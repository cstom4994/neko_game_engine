...
LuaImpl = Impl "lua" -- Functions in neko library impemented in lua instead of C (default)
...
local function to_function_obj(def, ns)
    ...
    for _i, x in ipairs(def.items) do
        ...
        if x._kind == "Arg" then
            ...
            local doc = nil
            if #x.items >= 2 and x.items[2]._kind == "Doc" then
                doc = x.items[2]._name
            end
            table.insert(fn.args, {
                name = x._name,
                type = typename,
                wrapped = iswrapped,
                doc = doc
        ...
        elseif x._kind == "Ret" then
            ...
            table.insert(fn.rets, {
                name = x._name,
                type = typename,
                wrapped = iswrapped,
                doc = doc
            })
        elseif x._kind == "Doc" then
            fn.doc = fn.doc .. x._name .. " "
        elseif x._kind == "Code" then
            fn.code = x._name
        elseif x._kind == "MapTo" then
            fn.mapto = functions[x._name];
            fn.doc = fn.doc .. fn.mapto.doc
        elseif x._kind == "MapWrapTo" then
            fn.mapwrapto = functions[x._name]; -- fn.doc = fn.doc .. fn.mapto.doc
        elseif x._kind == "Impl" then
            assert(fn.impl == "native") -- impl can only defined once per function
            fn.impl = x._name
        else
            error("Unknown kind: " .. x._kind .. " -> " .. x._name)
        end
    end
...
local function to_record_obj(def, ns)
    ...
    for _i, x in ipairs(def.items) do
            ...
            if #x.items >= 2 then
                if #x.items == 2 then
                    ...
                    elseif (x.items[2]._kind == "MapWrite") then
                        ...
                        error("Unsupported number of parts in MapWrite def:: " .. x._kind .. " -> ." .. x._name)
                    end
                elseif #x.items == 3 then
                    ...
                    error("Unsupported number of parts in definition: " .. x._kind .. " -> ." .. x._name)
                end
            end
        ...
        elseif x._kind == "Method" then
            ...
            table.insert(rec.methods, {
                name = x._name,
                mapto = mapto
            })
        elseif x._kind == "Doc" then
            rec.doc = rec.doc .. x._name .. " "
        else
            error("Unknown kind: " .. x._kind .. " -> " .. x._name)
        end
    end
...
local function to_list_obj(def, nsname)
    ...
    if def.items[1]._kind ~= "Type" then
        error "List definition needs to specify item(element) type as the first item"
    end
    ls.item_type = def.items[1]._name

    check_tag(def.items[2])
    if def.items[2]._kind ~= "MaxCount" then
        error("List definition needs to specify a max count as the second item" .. def.items[2]._kind)
    end
    ls.max_count = def.items[2]._name

    if (#def.items > 2) then
        check_tag(def.items[3])
        if def.items[3]._kind ~= "Doc" then
            error "List definition's thirt item needs to be a Doc if it exists"
        end
...
local function to_dict_obj(def, nsname)
    ...
    check_tag(def.items[1])
    if def.items[1]._kind ~= "Type" then
        error "Dict definition needs to specify key type as the first item"
    end
    dict.key_type = def.items[1]._name
    check_tag(def.items[2])
    if def.items[2]._kind ~= "Type" then
        error "Dict definition needs to specify value type as the second item"
    end
    dict.value_type = def.items[2]._name

    if (#def.items > 2) then
        check_tag(def.items[3])
        if def.items[2]._kind ~= "Doc" then
            error "Dict definition's third item needs to be a Doc if it exists"
        end
...
local function to_variant_obj(def, nsname)
    ...
    for _i, x in ipairs(def.items) do
        ...
        if x._kind == "Option" then
            ...
            table.insert(variant.options, {
                name = x._name,
                type = typename,
                doc = doc
            })
        elseif x._kind == "Doc" then
            variant.doc = variant.doc .. x._name .. " "
        else
            error("Unknown kind: " .. x._kind .. " -> " .. x._name)
        end
    end
...
local function to_enum_obj(def, nsname)
    ...
    for _, x in ipairs(def.items) do
        if type(x) == "string" then
            table.insert(_enum.values, x)
        elseif check_tag(x) and x._kind == "Doc" then
            _enum.doc = _enum.doc .. x._name
        else
            error("Unknown kind: " .. x._kind .. " -> " .. x._name)
        end
    end
...
local function to_namespace_obj(def, parent_ns_name)
    ...
    for _i, x in ipairs(def.items) do
        ...
        if x._kind == "Require" then
            ...
            -- -- Note: this doesn't handle a child's require's. Not needed for neko's current API shape
            -- Enums are shared between neko and neko_core namespaces. copy them over
            for _i, e in ipairs(req.enums) do
                table.insert(NS.enums, e)
            end
    ...
    for _i, x in ipairs(def.items) do
        check_tag(x)
        if x._kind == "Function" then
            table.insert(NS.functions, to_function_obj(x, NS))
        end
    end
    ...
    for _i, x in ipairs(def.items) do
        ...
        elseif x._kind == "Namespace" then
            table.insert(NS.namespaces, to_namespace_obj(x, NS.name))
        elseif x._kind == "Record" then
            table.insert(NS.records, to_record_obj(x, NS))
        elseif x._kind == "List" then
            table.insert(NS.lists, to_list_obj(x, NS.name))
        elseif x._kind == "Dict" then
            table.insert(NS.dicts, to_dict_obj(x, NS.name))
        elseif x._kind == "Variant" then
            table.insert(NS.variants, to_variant_obj(x, NS.name))
        elseif x._kind == "Enum" then
            table.insert(NS.enums, to_enum_obj(x, NS.name))
        elseif x._kind == "Doc" then
            NS.doc = NS.doc .. "\n" .. x._name
        else
            error("Unknown kind: " .. x._kind .. " -> " .. x._name)
