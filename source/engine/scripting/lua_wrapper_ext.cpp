...
LuaVector *LuaVector::CheckArg(lua_State *L, int arg) { return neko_s_cast<LuaVector *>(luaL_checkudata(L, arg, LUA_TYPE_NAME)); }

int LuaVector::New(lua_State *L) {
    auto p = neko_s_cast<LuaVector *>(lua_newuserdata(L, sizeof(LuaVector)));
    new (p) LuaVector;
...
int LuaVector::NewIndex(lua_State *L) {
    ...
    p->at(idx) = neko_s_cast<float>(value);
    return 0;
}
...
int LuaVector::Len(lua_State *L) {
    ...
    lua_pushinteger(L, neko_s_cast<lua_Integer>(p->size()));
    return 1;
}
...
int LuaVector::Append(lua_State *L) {
    ...
    p->push_back(neko_s_cast<float>(value));
...
int LuaVector::Insert(lua_State *L) {
    ...
    p->insert(std::next(p->begin(), idx), neko_s_cast<float>(value));
...
neko_private(void) luaSD_stackdumpvalue(lua_State *state, luaSD_printf luasdorintf, int stackIndex, int depth, int newline, int indentLevel, int performIndent) {
    int t;
