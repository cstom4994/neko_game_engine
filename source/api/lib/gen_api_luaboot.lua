...
local function generate_api_luaboot(NS)
    ...
    for _i, rec in ipairs(NS.records) do
        ...
        for _i, f in ipairs(rec.fields) do
            if f.mapread then
                out = out .. SPC2 .. "if k=='" .. f.name .. "' then return " .. NS.name .. "." .. f.mapread.name ..
                          "(self, k) end\n"
            end
        end
        -- non "meta" methods
        for _i, m in ipairs(rec.methods) do
            if m.name:sub(1, 2) ~= "__" then
                out = out .. SPC2 .. "if k=='" .. m.name .. "' then return " .. NS.name .. "." .. m.mapto.name ..
                          " end\n"
            end
        end
        ...
        out = out .. SPC .. "__tostring=function(self) return '(obj type = " .. NS.name .. "." .. rec.name ..
                  ")' end;\n"
        out = out .. "}\n"
    end
    out = out .. "\n"
