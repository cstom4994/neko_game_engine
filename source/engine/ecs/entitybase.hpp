...
// DECL_ENT(Camera, Float32 viewport_height;);
...
DECL_ENT(Transform, vec2 position; Float32 rotation; vec2 scale; NativeEntity parent;  // 如果entity_nil 则为 root
...
// bbox pool
DECL_ENT(BBoxPoolElem, mat3 wmat; BBox bbox; Float32 selected;  // > 0.5 if and only if selected
);