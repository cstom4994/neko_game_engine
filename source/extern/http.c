...
static void http_init(void) { internet = InternetOpen(L"NEKO", INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0); }

static void http_destroy(void) { InternetCloseHandle(internet); }
...
static int l_http_request(lua_State* L) {
    ...
    http_response_t response = {.header_cb = addheader, .userdata = L};
