...
static constexpr const char UnsyncRsrc[] = "UnsyncRsrc";

class LuaStackPopGuard {
public:
    LuaStackPopGuard(neko_lua_state_view L, int n) : L{L}, n{n} {}
...
struct ArgStack {
    void* argptr_buffer[MaxArgNum];
    std::aligned_storage_t<sizeof(Type), alignof(Type)> argType_buffer[MaxArgNum];
    std::uint64_t copied_args_buffer[MaxArgNum];
...
ObjectView* safe_get_Object(neko_lua_state_view L, int idx);

template <typename T>
T* auto_get_userdata_ptr(neko_lua_state_view L, int idx) {
...
template <typename T>
T auto_get(neko_lua_state_view L, int idx) {
    if constexpr (std::is_reference_v<T>)
        ...
        switch (type) {
            ...
            case LUA_TUSERDATA:
                return *(T*)L.checkudata(idx, type_name<T>().Data());
            default:
                L.error("auto_get : The %dth argument (%s) isn't convertible to %s.", idx, L.typename_(idx), type_name<T>().Data());
                return T{};
        }
    } else
...
void push(neko_lua_state_view L, Ret rst) {
...
int FillArgStack(neko_lua_state_view L, ArgStack& stack, int begin, int cnt);

MethodPtr::Func LuaFuncToMethodPtrFunc(Type object_type, MethodFlag flag, Type result_type, neko_lua_ref func_ref);

template <auto funcptr, typename Obj, std::size_t... Ns>
void caller(neko_lua_state_view L, Obj* obj, std::index_sequence<Ns...>) {
...
void caller(neko_lua_state_view L, std::index_sequence<Ns...>) {
    using FuncObj = decltype(funcptr);
    using Traits = FuncTraits<FuncObj>;
    using ArgList = typename Traits::ArgList;
    using Ret = typename Traits::Return;
    ...
    return static_cast<lua_CFunction>([](lua_State* L_) -> int {
        ...
        if constexpr (std::is_member_function_pointer_v<FuncObj>) {
            ...
            if (n != 1 + Length_v<ArgList>)
                return L.error("%s::%s : The number of arguments is invalid. The function needs (object + %I) arguments.", type_name<Obj>().Data(), FuncName::Data(),
            ...
            if (n != Length_v<ArgList>)
                return L.error("%s::%s : The number of arguments is invalid. The function needs %I arguments (no object).", type_name<Obj>().Data(), FuncName::Data(),
                               static_cast<lua_Integer>(Length_v<ArgList>));
...
extern void neko_refl_lua(lua_State* L);
