...
static FORMAT_ARGS(1) void print(const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
...
static FORMAT_ARGS(1) void println(const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    vfprintf(output_file, fmt, args);
    va_end(args);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (do_opts && arg[0] == '-') {
            if (0 == strcmp(arg, "--")) {
                ...
                i++;
                if (i >= argc) {
                    fatal_error("missing argument for -m");
                }
                module_name = argv[i];
...
static char *get_module_name_from_filename(const char *filename) {
...
static void check_module_name(const char *module_name) {
...
static void replace_dots(char *module_name) {
    for (size_t i = 0; module_name[i] != '\0'; i++) {
        if (module_name[i] == '.') {
            module_name[i] = '_';
        }
...
static void luaot_PrintOpcodeComment(Proto *f, int pc) {
...
static void create_functions(Proto *p) {
...
static void print_functions(Proto *p) {
    create_functions(p);
...
static void print_source_code() {
    ...
    if (!infile) {
        fatal_error("could not open input file a second time");
    }
