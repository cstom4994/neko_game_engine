...
function ng.c_save_load(ctype, c_save, c_load)
    return function (cdata)
        local cdump = ng.c_serialize(loadstring('return ' .. c_save)(),
                                        cdata)
        return 'ng.c_deserialize(' .. ctype .. ', ' .. c_load .. ', '
            .. ng.safestr(cdump) .. ')'
    end
end
...
_cge = ng._entity_resolve_saved_id
...
ng.Vec2 = ffi.metatype('CVec2',
{
    __add = ng.vec2_add,
    __sub = ng.vec2_sub,
    __unm = function (v) return ng.vec2_neg(v) end,
    __mul = function (a, b)
        if type(a) == 'number' then return ng.vec2_scalar_mul(b, a)
        elseif type(b) == 'number' then return ng.vec2_scalar_mul(a, b)
        else return ng.vec2_mul(a, b) end
    end,
    __div = function (a, b)
        if type(b) == 'number' then return ng.vec2_scalar_div(a, b)
        elseif type(a) == 'number' then return ng.scalar_vec2_div(a, b)
        else return ng.vec2_div(a, b) end
    end,
    __index =
    {
...
ng.BBox = ffi.metatype('BBox',
{
    __index =
    {
            __serialize = function (b)
                return string.format(
                    'ng.bbox(ng.vec2(%f, %f), ng.vec2(%f, %f))',
