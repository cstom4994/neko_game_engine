...
neko_render_batch_context_t* neko_render_batch_make_ctx(u32 max_draw_calls) {
    neko_render_batch_context_t* ctx = (neko_render_batch_context_t*)mem_alloc(sizeof(neko_render_batch_context_t));

    ctx->max_draw_calls = max_draw_calls;
    ctx->count = 0;
    ctx->calls = (neko_render_batch_draw_call_t*)mem_alloc(sizeof(neko_render_batch_draw_call_t) * max_draw_calls);
    if (!ctx->calls) {
        mem_free(ctx);
        return 0;
    }
