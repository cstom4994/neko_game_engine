...
#ifndef NEKO_PROPS_HPP
#define NEKO_PROPS_HPP
...
struct prop_holder {
    ...
    template <typename V>
    static data_type_t get_type_enum() {
        return unknowns_type_value;
    }
    template <>
    static data_type_t get_type_enum<int>() {
        return integer_value;
    }
    template <>
    static data_type_t get_type_enum<unsigned int>() {
        return uinteger_value;
    }
    template <>
    static data_type_t get_type_enum<wchar_t>() {
        return wchar_value;
    }
    template <>
    static data_type_t get_type_enum<char>() {
        return char_value;
    }
    template <>
    static data_type_t get_type_enum<unsigned char>() {
        return uchar_value;
    }
    template <>
    static data_type_t get_type_enum<short>() {
        return short_value;
    }
    template <>
    static data_type_t get_type_enum<unsigned short>() {
        return ushort_value;
    }
    template <>
    static data_type_t get_type_enum<float>() {
        return float_value;
    }
    template <>
    static data_type_t get_type_enum<double>() {
        return double_value;
    }
    template <>
    static data_type_t get_type_enum<bool>() {
        return bool_value;
    }
    template <>
    static data_type_t get_type_enum<long long>() {
...
struct prop_holder_m_const : prop_holder_t<T, V> {
    ...
    prop_holder_m_const(mptr_t p) : ptr(p) { __super::read_only = true; }
...
struct prop_holder_m : prop_holder_t<T, V> {
    ...
    prop_holder_m(mptr_t p) : ptr(p) { __super::read_only = false; }

    virtual V get(const T *po) const { return po ? po->*(ptr) : V(0); }

    virtual void set(T *po, const V &v) const {
        if (__super::read_only || !po) return;
        po->*(ptr) = v;
    }

    virtual void from_string(void *owner, const std::string &str) const {
        T *ot = (T *)owner;
        if (__super::read_only || !owner) return;
...
struct prop_holder_f : prop_holder_t<OwnerType, ValueType> {
    ...
    prop_holder_f(getter_t g, setter_t s) : getter(g), setter(s) { __super::read_only = s ? false : true; }

    virtual ValueType get(const OwnerType *po) const { return po ? getter(po) : ValueType(0); }
...
public:
    ...
    V get(const T *owner) const {
        ...
        if (typeid(V) != value_type) return aux::lexical_cast<V>(holder->to_string(owner));
