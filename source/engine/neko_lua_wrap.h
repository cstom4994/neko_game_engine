...
inline bool neko_lua_dofile(lua_State *_L, const std::string &file) {
...
template <typename T>
int neko_lua_get_global_variable(lua_State *_L, const std::string &field_name_, T &ret_);
template <typename T>
int neko_lua_get_global_variable(lua_State *_L, const char *field_name_, T &ret_);

template <typename T>
int neko_lua_set_global_variable(lua_State *_L, const std::string &field_name_, const T &value_);
template <typename T>
int neko_lua_set_global_variable(lua_State *_L, const char *field_name_, const T &value_);

inline void neko_lua_register_raw_function(lua_State *_L, const char *func_name_, lua_function_t func_) {
    lua_checkstack(_L, STACK_MIN_NUM);

    lua_pushcfunction(_L, func_);
    lua_setglobal(_L, func_name_);
}
...
inline int __neko_lua_getFuncByName(lua_State *_L, const char *func_name_) {
    // lua_getglobal(_L, func_name_);
    // return 0;
    ...
    for (unsigned int i = 0; i < sizeof(tmpBuff); ++i) {
        ...
        if (c == '.') {
            tmpBuff[i] = '\0';
            lua_getglobal(_L, begin);
            const char *begin2 = func_name_ + i + 1;
            lua_getfield(_L, -1, begin2);
            lua_remove(_L, -2);
            return 0;
        } else if (c == ':') {
            tmpBuff[i] = '\0';
...
int neko_lua_get_global_variable(lua_State *_L, const char *field_name_, T &ret_) {
    ...
    lua_getglobal(_L, field_name_);
    ret = __lua_op_t<T>::get_ret_value(_L, -1, ret_);
...
template <typename T>
int neko_lua_set_global_variable(lua_State *_L, const char *field_name_, const T &value_) {
    __lua_op_t<T>::push_stack(_L, value_);
    lua_setglobal(_L, field_name_);
    return 0;
}
...
ret(RET) neko_lua_call(lua_State *_L, const char *func_name_) {
    ...
    if (__lua_op_t<ret(RET)>::get_ret_value(_L, -1, ret)) {
        lua_pop(_L, 1);
        char buff[512];
...
ret(RET) neko_lua_call(lua_State *_L, const char *func_name_, const ARG1 &arg1_) {
    ret(RET) ret = init_value_traits_t<ret(RET)>::value();
...
ret(RET) neko_lua_call(lua_State *_L, const char *func_name_, const ARG1 &arg1_, const ARG2 &arg2_) {
    ret(RET) ret = init_value_traits_t<ret(RET)>::value();
...
ret(RET) neko_lua_call(lua_State *_L, const char *func_name_, const ARG1 &arg1_, const ARG2 &arg2_, const ARG3 &arg3_) {
    ret(RET) ret = init_value_traits_t<ret(RET)>::value();
...
ret(RET) neko_lua_call(lua_State *_L, const char *func_name_, const ARG1 &arg1_, const ARG2 &arg2_, const ARG3 &arg3_, const ARG4 &arg4_) {
    ret(RET) ret = init_value_traits_t<ret(RET)>::value();
...
ret(RET) neko_lua_call(lua_State *_L, const char *func_name_, const ARG1 &arg1_, const ARG2 &arg2_, const ARG3 &arg3_, const ARG4 &arg4_, const ARG5 &arg5_) {
    ret(RET) ret = init_value_traits_t<ret(RET)>::value();
...
ret(RET) neko_lua_call(lua_State *_L, const char *func_name_, const ARG1 &arg1_, const ARG2 &arg2_, const ARG3 &arg3_, const ARG4 &arg4_, const ARG5 &arg5_, const ARG6 &arg6_) {
    ret(RET) ret = init_value_traits_t<ret(RET)>::value();
...
ret(RET) neko_lua_call(lua_State *_L, const char *func_name_, const ARG1 &arg1_, const ARG2 &arg2_, const ARG3 &arg3_, const ARG4 &arg4_, const ARG5 &arg5_, const ARG6 &arg6_, const ARG7 &arg7_) {
    ret(RET) ret = init_value_traits_t<ret(RET)>::value();
