...
void neko_immediate_draw_static_data_init() {
    ...
    *tdesc.data = pixels;
    ...
    i32 compressed_ttf_size = (((i32)strlen(compressed_ttf_data_base85) + 4) / 5) * 4;
    ...
    i32 v = stbtt_BakeFontBitmap((u8*)buf_decompressed_data, 0, 13.f, alpha_bitmap, w, h, 32, 96, (stbtt_bakedchar*)f->glyphs);  // no guarantee this fits!
...
void neko_immediate_draw_free(neko_immediate_draw_t* ctx) {
...
static void neko_idraw_rect_2d(neko_immediate_draw_t* neko_idraw, neko_vec2 a, neko_vec2 b, neko_vec2 uv0, neko_vec2 uv1, Color256 color) {
...
void neko_idraw_pipeline_set(neko_immediate_draw_t* neko_idraw, neko_handle(gfx_pipeline_t) pipeline) {
    neko_idraw_flush(neko_idraw);
...
void neko_idraw_vattr_list(neko_immediate_draw_t* neko_idraw, neko_idraw_vattr_type* list, size_t sz) {
    neko_idraw_flush(neko_idraw);
...
void neko_idraw_vattr_list_mesh(neko_immediate_draw_t* neko_idraw, neko_asset_mesh_layout_t* layout, size_t sz) {
    neko_idraw_flush(neko_idraw);
...
void neko_idraw_c4ubv(neko_immediate_draw_t* neko_idraw, Color256 c) { neko_idraw_c4ub(neko_idraw, c.r, c.g, c.b, c.a); }
...
void neko_idraw_linev(neko_immediate_draw_t* neko_idraw, neko_vec2 v0, neko_vec2 v1, Color256 c) { neko_idraw_line(neko_idraw, v0.x, v0.y, v1.x, v1.y, c.r, c.g, c.b, c.a); }
...
void neko_idraw_rect(neko_immediate_draw_t* neko_idraw, f32 l, f32 b, f32 r, f32 t, u8 _r, u8 _g, u8 _b, u8 _a, gfx_primitive_type type) {
...
void neko_idraw_rect3Dv(neko_immediate_draw_t* neko_idraw, neko_vec3 min, neko_vec3 max, neko_vec2 uv0, neko_vec2 uv1, Color256 c, gfx_primitive_type type) {
...
void neko_idraw_box(neko_immediate_draw_t* neko_idraw, f32 x, f32 y, f32 z, f32 hx, f32 hy, f32 hz, u8 r, u8 g, u8 b, u8 a, gfx_primitive_type type) {
    f32 width = hx;
    f32 height = hy;
    f32 length = hz;
...
void neko_idraw_renderpass_submit(neko_immediate_draw_t* neko_idraw, neko_command_buffer_t* cb, neko_vec4 viewport, Color256 c) {
...
unsigned int neko_decompress_length(const unsigned char* input) { return (input[8] << 24) + (input[9] << 16) + (input[10] << 8) + input[11]; }
...
unsigned int neko_decompress(unsigned char* output, unsigned char* i, unsigned int length) {
    u32 olen;
...
unsigned int __neko_internal_Decode85Byte(char c) { return c >= '\\' ? c - 36 : c - 35; }
...
const char* __neko_internal_GetDefaultCompressedFontDataTTFBase85() { return __neko_proggy_clean_ttf_compressed_data_base85; }
...
neko_draw_pipeline_t neko_draw_pipeline_create(const neko_draw_pipeline_desc_t* desc) {
    ...
    pip.hndl = gfx_pipeline_create(desc->pip_desc);
    pip.ublock = neko_draw_uniform_block_create(&desc->ublock_desc);
    pip.desc = desc->pip_desc;
    pip.desc.layout.attrs = (gfx_vertex_attribute_desc_t*)mem_alloc(desc->pip_desc.layout.size);
...
neko_draw_uniform_block_t neko_draw_uniform_block_create(const neko_draw_uniform_block_desc_t* desc) {
    ...
    for (uint32_t i = 0; i < ct; ++i) {
        ...
        gfx_uniform_desc_t u_desc = NEKO_DEFAULT_VAL();
        gfx_uniform_layout_desc_t u_layout = NEKO_DEFAULT_VAL();
        ...
        switch (ud->type) {
            ...
            default: {
                u.hndl = gfx_uniform_create(u_desc);
                u.offset = offset;
            } break;
        }
        ...
        switch (ud->type) {
            ...
            case R_UNIFORM_SAMPLER2D:
                offset += sizeof(neko_handle(gfx_texture_t));
                break;
            case R_UNIFORM_USAMPLER2D:
                offset += sizeof(neko_handle(gfx_texture_t));
                break;
            case R_UNIFORM_IMAGE2D_RGBA32F: {
                image2D_offset += sizeof(neko_handle(gfx_texture_t));
            } break;
        }
...
neko_draw_texture_t neko_draw_texture_create(gfx_texture_desc_t desc) { return gfx_texture_create(desc); }
    ...
    for (uint32_t i = 0; i < mesh_count; ++i) {
        ...
        for (uint32_t p = 0; p < neko_dyn_array_size(m->primitives); ++p) {
            ...
            if (vdata->positions.data) {
                ...
                prim.stream.positions = gfx_vertex_buffer_create(vdesc);
                if (!desc->keep_data) {
                    mem_free(vdata->positions.data);
                }
            }
            ...
            if (vdata->normals.data) {
                ...
                prim.stream.normals = gfx_vertex_buffer_create(vdesc);
                if (!desc->keep_data) {
                    mem_free(vdata->normals.data);
                }
            }
            ...
            if (vdata->tangents.data) {
                ...
                prim.stream.tangents = gfx_vertex_buffer_create(vdesc);
                if (!desc->keep_data) {
                    mem_free(vdata->tangents.data);
                }
            }
            ...
            for (uint32_t j = 0; j < NEKO_GFXT_TEX_COORD_MAX; ++j) {
                if (vdata->tex_coords[j].data) {
                    ...
                    prim.stream.tex_coords[j] = gfx_vertex_buffer_create(vdesc);
                    if (!desc->keep_data) {
                        mem_free(vdata->tex_coords[j].data);
                    }
                }
            }
            ...
            for (uint32_t j = 0; j < NEKO_GFXT_COLOR_MAX; ++j) {
                if (vdata->colors[j].data) {
                    ...
                    prim.stream.colors[j] = gfx_vertex_buffer_create(vdesc);
                    if (!desc->keep_data) {
                        mem_free(vdata->colors[j].data);
                    }
                }
            }
            ...
            for (uint32_t j = 0; j < NEKO_GFXT_JOINT_MAX; ++j) {
                if (vdata->joints[j].data) {
                    ...
                    prim.stream.joints[j] = gfx_vertex_buffer_create(vdesc);
                    if (!desc->keep_data) {
                        mem_free(vdata->joints[j].data);
                    }
                }
            }
            ...
            for (uint32_t j = 0; j < NEKO_GFXT_WEIGHT_MAX; ++j) {
                if (vdata->weights[j].data) {
                    ...
                    prim.stream.weights[j] = gfx_vertex_buffer_create(vdesc);
                    if (!desc->keep_data) {
                        mem_free(vdata->weights[j].data);
                    }
                }
            }

            // Index buffer decl
            gfx_index_buffer_desc_t idesc = NEKO_DEFAULT_VAL();
            idesc.data = vdata->indices.data;
            idesc.size = vdata->indices.size;

            // Construct index buffer for primitive
            prim.indices = gfx_index_buffer_create(idesc);

            if (!desc->keep_data) {
                mem_free(vdata->indices.data);
            }
        ...
        for (uint32_t p = 0; p < neko_dyn_array_size(m->primitives); ++p) {
            ...
            if (vdata->positions.data) {
                ...
                if (prim->stream.positions.id) {
                    gfx_vertex_buffer_update(prim->stream.positions, &vdesc);
                }
                // Create
                else {
                    prim->stream.positions = gfx_vertex_buffer_create(vdesc);
            ...
            if (vdata->normals.data) {
                ...
                if (prim->stream.normals.id) {
                    gfx_vertex_buffer_update(prim->stream.normals, &vdesc);
                } else {
                    prim->stream.normals = gfx_vertex_buffer_create(vdesc);
                }
                if (!desc->keep_data) {
            ...
            if (vdata->tangents.data) {
                ...
                if (prim->stream.tangents.id) {
                    gfx_vertex_buffer_update(prim->stream.tangents, &vdesc);
                } else {
                    prim->stream.tangents = gfx_vertex_buffer_create(vdesc);
                }
                if (!desc->keep_data) {
            ...
            for (uint32_t j = 0; j < NEKO_GFXT_TEX_COORD_MAX; ++j) {
                if (vdata->tex_coords[j].data) {
                    ...
                    if (prim->stream.tex_coords[j].id) {
                        gfx_vertex_buffer_update(prim->stream.tex_coords[j], &vdesc);
                    } else {
                        prim->stream.tex_coords[j] = gfx_vertex_buffer_create(vdesc);
                    }
                    if (!desc->keep_data) {
            ...
            for (uint32_t j = 0; j < NEKO_GFXT_COLOR_MAX; ++j) {
                if (vdata->colors[j].data) {
                    ...
                    if (prim->stream.colors[j].id) {
                        gfx_vertex_buffer_update(prim->stream.colors[j], &vdesc);
                    } else {
                        prim->stream.colors[j] = gfx_vertex_buffer_create(vdesc);
                    }
                    if (!desc->keep_data) {
            ...
            for (uint32_t j = 0; j < NEKO_GFXT_JOINT_MAX; ++j) {
                if (vdata->joints[j].data) {
                    ...
                    if (prim->stream.joints[j].id) {
                        gfx_vertex_buffer_update(prim->stream.joints[j], &vdesc);
                    } else {
                        prim->stream.joints[j] = gfx_vertex_buffer_create(vdesc);
                    }
                    if (!desc->keep_data) {
            ...
            for (uint32_t j = 0; j < NEKO_GFXT_WEIGHT_MAX; ++j) {
                if (vdata->weights[j].data) {
                    ...
                    if (prim->stream.weights[j].id) {
                        gfx_vertex_buffer_update(prim->stream.weights[j], &vdesc);
                    } else {
                        prim->stream.weights[j] = gfx_vertex_buffer_create(vdesc);
                    }
                    if (!desc->keep_data) {
            ...
            for (uint32_t j = 0; j < NEKO_GFXT_CUSTOM_UINT_MAX; ++j) {
                if (vdata->custom_uint[j].data) {
                    ...
                    if (prim->stream.custom_uint[j].id) {
                        gfx_vertex_buffer_update(prim->stream.custom_uint[j], &vdesc);
                    } else {
                        prim->stream.custom_uint[j] = gfx_vertex_buffer_create(vdesc);
                    }
                    if (!desc->keep_data) {
                        mem_free(vdata->custom_uint[j].data);
                    }
                }
            }
            ...
            // Construct index buffer for primitive
            if (prim->indices.id) {
                gfx_index_buffer_update(prim->indices, &idesc);
            } else {
                prim->indices = gfx_index_buffer_create(idesc);
            }
...
//=== Destruction ===//
void neko_draw_texture_fini(neko_draw_texture_t* texture) { gfx_texture_fini(*texture); }
...
void neko_draw_mesh_fini(neko_draw_mesh_t* mesh) {
    ...
    for (uint32_t p = 0; p < neko_dyn_array_size(mesh->primitives); ++p) {
        ...
        // Free index buffer
        if (prim->indices.id) gfx_index_buffer_fini(prim->indices);

        // Free vertex stream
        if (prim->stream.positions.id) gfx_vertex_buffer_fini(prim->stream.positions);
        if (prim->stream.normals.id) gfx_vertex_buffer_fini(prim->stream.normals);
        if (prim->stream.tangents.id) gfx_vertex_buffer_fini(prim->stream.tangents);

        for (uint32_t i = 0; i < NEKO_GFXT_COLOR_MAX; ++i) {
            if (prim->stream.colors[i].id) gfx_vertex_buffer_fini(prim->stream.colors[i]);
        }

        for (uint32_t i = 0; i < NEKO_GFXT_TEX_COORD_MAX; ++i) {
            if (prim->stream.tex_coords[i].id) gfx_vertex_buffer_fini(prim->stream.tex_coords[i]);
        }

        for (uint32_t i = 0; i < NEKO_GFXT_JOINT_MAX; ++i) {
            if (prim->stream.joints[i].id) gfx_vertex_buffer_fini(prim->stream.joints[i]);
        }

        for (uint32_t i = 0; i < NEKO_GFXT_WEIGHT_MAX; ++i) {
            if (prim->stream.weights[i].id) gfx_vertex_buffer_fini(prim->stream.weights[i]);
        }
    }
}
...
void neko_draw_pipeline_fini(neko_draw_pipeline_t* pipeline) {
    ...
    gfx_shader_fini(pipeline->desc.raster.shader);

    // Destroy pipeline
    if (pipeline->desc.layout.attrs) mem_free(pipeline->desc.layout.attrs);
    if (pipeline->mesh_layout) neko_dyn_array_free(pipeline->mesh_layout);
    gfx_pipeline_fini(pipeline->hndl);
}
...
void neko_draw_material_set_uniform(neko_draw_material_t* mat, const char* name, const void* data) {
    ...
    uint64_t key = neko_hash_str64(name);
    if (!neko_hash_table_exists(pip->ublock.lookup, key)) {
        NEKO_TIMED_ACTION(60, { console_log("Unable to find uniform: %s", name); });
        return;
    }
    ...
    switch (u->type) {
        ...
        case R_UNIFORM_SAMPLER2D:
        case R_UNIFORM_USAMPLER2D: {
            neko_byte_buffer_write(&mat->uniform_data, neko_handle(gfx_texture_t), *(neko_handle(gfx_texture_t)*)data);
        } break;

        case R_UNIFORM_IMAGE2D_RGBA32F: {
            neko_byte_buffer_write(&mat->image_buffer_data, neko_handle(gfx_texture_t), *(neko_handle(gfx_texture_t)*)data);
        } break;
    }
}
...
void neko_draw_material_bind_uniforms(neko_command_buffer_t* cb, neko_draw_material_t* mat) {
    ...
    for (uint32_t i = 0; i < neko_dyn_array_size(pip->ublock.uniforms); ++i) {
        ...
        switch (u->type) {
            case R_UNIFORM_IMAGE2D_RGBA32F: {
                gfx_bind_image_buffer_desc_t ibuffer[1];
                ibuffer[0].tex = *(neko_handle(gfx_texture_t)*)(mat->image_buffer_data.data + u->offset);
                ...
                gfx_apply_bindings(cb, &bind);
            } break;

            default: {
                gfx_bind_uniform_desc_t uniforms[1];
...
// Mesh API
void neko_draw_mesh_draw(neko_command_buffer_t* cb, neko_draw_mesh_t* mp) {
    /*
    ...
    for (uint32_t i = 0; i < neko_dyn_array_size(mp->primitives); ++i)
    {
        ...
        gfx_bind_desc_t binds = NEKO_DEFAULT_VAL();
        gfx_bind_vertex_buffer_desc_t vdesc = NEKO_DEFAULT_VAL();
        gfx_bind_index_buffer_desc_t idesc = NEKO_DEFAULT_VAL();
        vdesc.buffer = prim->vbo;
        idesc.buffer = prim->indices;
        binds.vertex_buffers.desc = &vdesc;
        binds.index_buffers.desc = &idesc;
        gfx_draw_desc_t ddesc = NEKO_DEFAULT_VAL();
        ddesc.start = 0;
...
void neko_draw_mesh_primitive_draw_layout(neko_command_buffer_t* cb, neko_draw_mesh_primitive_t* prim, neko_draw_mesh_layout_t* layout, size_t layout_size, u32 instance_count) {
    if (!layout || !layout_size || !prim || !cb) {
        return;
    }

    gfx_bind_vertex_buffer_desc_t vbos[8] = {0};  // Make this a define
    uint32_t l = 0;
    ...
    gfx_bind_index_buffer_desc_t ibos = NEKO_DEFAULT_VAL();
    ...
    binds.vertex_buffers.size = l * sizeof(gfx_bind_vertex_buffer_desc_t);
...
neko_draw_mesh_t neko_draw_mesh_load_from_file(const char* path, neko_draw_mesh_import_options_t* options) {
    ...
    if (!neko_os_file_exists(path)) {
        console_log("[gfxt] Warning:GFXT:MeshLoadFromFile:File does not exist: %s", path);
        return mesh;
    }
    ...
    if (neko_string_compare_equal(file_ext, "gltf")) {  // GLTF
        ...
        console_log("[gfxt] Warning:GFXT:MeshLoadFromFile:File extension not supported: %s, file: %s", file_ext, path);
        return mesh;
    }
...
neko_draw_mesh_t neko_draw_mesh_unit_quad_generate(neko_draw_mesh_import_options_t* options) {
    ...
    Color256 v_color[] = {NEKO_COLOR_WHITE, NEKO_COLOR_WHITE, NEKO_COLOR_WHITE, NEKO_COLOR_WHITE};
...
#define NEKO_GFXT_ROW_COL_CT 5
    ...
    Color256 pixels[NEKO_GFXT_ROW_COL_CT * NEKO_GFXT_ROW_COL_CT] = NEKO_DEFAULT_VAL();
...
typedef struct neko_pipeline_parse_data_t {
    ...
    neko_dyn_array(gfx_vertex_attribute_type) vertex_layout;
    char* code[3];
    char dir[256];
} neko_ppd_t;
...
#define neko_parse_block(NAME, ...)                                                                            \
    do {                                                                                                       \
        console_log("[gfxt] neko_pipeline_load_from_file::parsing::%s", #NAME);                                \
        if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_LBRACE)) {                                        \
            console_log("[gfxt] error::neko_pipeline_load_from_file::error parsing raster from .sf resource"); \
            neko_assert(false);                                                                                \
        }                                                                                                      \
                                                                                                               \
        uint32_t bc = 1;                                                                                       \
...
const char* neko_get_vertex_attribute_string(gfx_vertex_attribute_type type) {
    switch (type) {
...
gfx_vertex_attribute_type neko_get_vertex_attribute_from_token(const neko_token_t* t) {
    ...
    return (gfx_vertex_attribute_type)0x00;
}

gfx_uniform_type neko_uniform_type_from_token(const neko_token_t* t) {
    if (neko_token_compare_text(t, "float"))
    ...
    return (gfx_uniform_type)0x00;
}

const char* neko_uniform_string_from_type(gfx_uniform_type type) {
    switch (type) {
        case R_UNIFORM_FLOAT:
            return "float";
            break;
...
bool neko_parse_uniform_special_keyword(neko_lexer_t* lex, neko_draw_pipeline_desc_t* desc, neko_ppd_t* ppd, gfx_shader_stage_type stage, neko_draw_uniform_desc_t* uniform) {
...
bool neko_parse_uniforms(neko_lexer_t* lex, neko_draw_pipeline_desc_t* desc, neko_ppd_t* ppd, gfx_shader_stage_type stage) {
    uint32_t image_binding = 0;

    if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_LBRACE)) {
        console_log("Unable to parsing uniforms from .sf resource");
        return false;
    }
            ...
            case NEKO_TOKEN_IDENTIFIER: {
                ...
                if (!special) {
                    ...
                    if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                        console_log("Unidentified token (Expected identifier)");
...
bool neko_parse_io(neko_lexer_t* lex, neko_draw_pipeline_desc_t* desc, neko_ppd_t* ppd, gfx_shader_stage_type type) {
    if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_LBRACE)) {
        console_log("Expected opening left brace. Unable to parse io from .sf resource");
        return false;
    }
            ...
            case NEKO_TOKEN_IDENTIFIER: {
                ...
                switch (type) {
                    case R_SHADER_STAGE_VERTEX: {
                        if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                            console_log("IO expected identifier name after type, shader stage vertex.");
                            neko_token_debug_print(&lex->current_token);
                            return false;
                        }
                        token = lex->current_token;
                    ...
                    case R_SHADER_STAGE_FRAGMENT: {
                        if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                            console_log("IO expected identifier name after type, shader stage fragment.");
                            neko_token_debug_print(&lex->current_token);
                            return false;
                        }
                        token = lex->current_token;
                    ...
                    case R_SHADER_STAGE_COMPUTE: {
                        if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_NUMBER)) {
                            console_log("IO expected number after type, shader stage compute.");
                            neko_token_debug_print(&lex->current_token);
                            return false;
                        }
                        token = lex->current_token;
...
bool neko_parse_code(neko_lexer_t* lex, neko_draw_pipeline_desc_t* desc, neko_ppd_t* ppd, gfx_shader_stage_type stage) {
    if (!neko_lexer_require_token_type(lex, NEKO_TOKEN_LBRACE)) {
        console_log("Expected opening left brace");
        return false;
    }
...
#define PUSH_ATTR(MESH_ATTR, VERT_ATTR)                           \
    do {                                                          \
        ...
        gfx_vertex_attribute_desc_t attr = NEKO_DEFAULT_VAL();    \
        ...
        /*console_log("[gfxt] %s: %s", #MESH_ATTR, #VERT_ATTR);*/ \
    } while (0)
                ...
                // else if (neko_token_compare_text(&token, "FLOAT4"))     PUSH_ATTR(TANGENT, FLOAT4);
                    ...
                    console_log("Unidentified vertex attribute: %.*s: %.*s", token.len, token.text, token_name.len, token_name.text);
                    return false;
                }
            }
        }
...
bool neko_parse_shader_stage(neko_lexer_t* lex, neko_draw_pipeline_desc_t* desc, neko_ppd_t* ppd, gfx_shader_stage_type stage) {
    if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_LBRACE)) {
        console_log("[gfxt] error::neko_pipeline_load_from_file::error parsing raster from .sf resource");
        neko_assert(false);
    }
        ...
        switch (token.type) {
            ...
            case NEKO_TOKEN_IDENTIFIER: {
                if (stage == R_SHADER_STAGE_VERTEX && neko_token_compare_text(&token, "attributes")) {
                    console_log("[gfxt] parsing attributes...");
                    if (!neko_parse_vertex_attributes(lex, desc, ppd)) {
                        console_log("Unable to parse vertex attributes.");
                        return false;
                    }
                }

                else if (neko_token_compare_text(&token, "uniforms")) {
                    console_log("[gfxt] parsing uniforms...");
                    if (!neko_parse_uniforms(lex, desc, ppd, stage)) {
                        console_log("Unable to parse 'uniforms' for stage: %zu.", (u32)stage);
                        return false;
                    }
                }

                else if (neko_token_compare_text(&token, "out")) {
                    console_log("[gfxt] parsing out...");
                    if (!neko_parse_io(lex, desc, ppd, stage)) {
                        console_log("Unable to parse 'out' for stage: %zu.", (u32)stage);
                        return false;
                    }
                }

                else if (neko_token_compare_text(&token, "in")) {
                    console_log("[gfxt] parsing in...");
                    if (!neko_parse_io(lex, desc, ppd, stage)) {
                        console_log("Unable to parse 'in' for stage: %zu.", (u32)stage);
                        return false;
                    }
                }

                else if (neko_token_compare_text(&token, "code")) {
                    console_log("[gfxt] parsing code...");
                    if (!neko_parse_code(lex, desc, ppd, stage)) {
                        console_log("Unable to parse 'code' for stage: %zu.", (u32)stage);
                        return false;
                    }
                }
            } break;
    ...
    neko_parse_block(PIPELINE::COMPUTE_SHADER_STAGE, {
        if (neko_token_compare_text(&token, "uniforms")) {
            if (!neko_parse_uniforms(lex, desc, ppd, R_SHADER_STAGE_COMPUTE)) {
                console_log("Unable to parse 'uniforms' for compute shader");
                return false;
            }
        }
            ...
            if (!neko_parse_io(lex, desc, ppd, R_SHADER_STAGE_COMPUTE)) {
                console_log("Unable to parse 'in' for compute shader");
                return false;
            }
        }
            ...
            if (!neko_parse_code(lex, desc, ppd, R_SHADER_STAGE_COMPUTE)) {
                console_log("Unable to parse 'code' for compute shader");
                return false;
            }
        }
    });
...
bool neko_parse_shader(neko_lexer_t* lex, neko_draw_pipeline_desc_t* desc, neko_ppd_t* ppd) {
    if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_LBRACE)) {
        console_log("Unable to parse shader from .sf resource. Expected opening left brace.");
        return false;
    }
        ...
        switch (token.type) {
            ...
            case NEKO_TOKEN_IDENTIFIER: {
                ...
                if (neko_token_compare_text(&token, "vertex")) {
                    console_log("[gfxt] parsing vertex shader");
                    if (!neko_parse_shader_stage(lex, desc, ppd, R_SHADER_STAGE_VERTEX)) {
                        console_log("Unable to parse shader stage: Vertex");
                        return false;
                    }
                }
                ...
                // Fragment shader
                    ...
                    console_log("[gfxt] parsing fragment shader");
                    if (!neko_parse_shader_stage(lex, desc, ppd, R_SHADER_STAGE_FRAGMENT)) {
                        console_log("Unable to parse shader stage: Fragment");
                        return false;
                    }
                }
                ...
                // Compute shader
                    ...
                    console_log("[gfxt] parsing compute shader");
                    if (!neko_parse_shader_stage(lex, desc, ppd, R_SHADER_STAGE_COMPUTE)) {
                        console_log("Unable to parse shader stage: Compute");
                        return false;
                    }
                }
...
bool neko_parse_depth(neko_lexer_t* lex, neko_draw_pipeline_desc_t* pdesc, neko_ppd_t* ppd) {
    neko_parse_block(PIPELINE::DEPTH, {
        ...
        if (neko_token_compare_text(&token, "func")) {
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                token = lex->current_token;
                console_log("Depth func type not found after function decl: %.*s", token.len, token.text);
                return false;
            }
            ...
            if (neko_token_compare_text(&token, "LESS"))
                ...
                token = lex->current_token;
                console_log("Func type %.*s not valid.", token.len, token.text);
                return false;
            }
        }
        if (neko_token_compare_text(&token, "mask")) {
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                token = lex->current_token;
                console_log("Depth mask type not found after function decl: %.*s", token.len, token.text);
                return false;
            }
            ...
            if (neko_token_compare_text(&token, "ENABLED"))
                ...
                token = lex->current_token;
                console_log("Mask type %.*s not valid.", token.len, token.text);
                return false;
            }
            console_log("[gfxt] MASK: %zu", (uint32_t)pdesc->pip_desc.depth.mask);
        }
    });
    return true;
}
...
bool neko_parse_blend(neko_lexer_t* lex, neko_draw_pipeline_desc_t* pdesc, neko_ppd_t* ppd) {
    neko_parse_block(PIPELINE::BLEND, {
        ...
        if (neko_token_compare_text(&token, "func")) {
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                console_log("Blend func type not found after function decl.");
                return false;
            }
            ...
            if (neko_token_compare_text(&token, "ADD"))
                ...
                console_log("Blend func type %.*s not valid.", token.len, token.text);
                return false;
            }
        }
        ...
        // Source blend
            ...
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                console_log("Blend src type not found after decl.");
                return false;
            }
            ...
            if (neko_token_compare_text(&token, "ZERO"))
                ...
                console_log("Blend src type %.*s not valid.", token.len, token.text);
                return false;
            }
        }
        ...
        // Dest blend
            ...
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                console_log("Blend dst type not found after decl.");
                return false;
            }
            ...
            if (neko_token_compare_text(&token, "ZERO"))
                ...
                console_log("Blend dst type %.*s not valid.", token.len, token.text);
                return false;
            }
        }
    });
...
bool neko_parse_stencil(neko_lexer_t* lex, neko_draw_pipeline_desc_t* pdesc, neko_ppd_t* ppd) {
    neko_parse_block(PIPELINE::STENCIL, {
        ...
        if (neko_token_compare_text(&token, "func")) {
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                console_log("Stencil func type not found after decl.");
                return false;
            }
                ...
                if (neko_token_compare_text(&token, "LESS"))
                    ...
                    console_log("Stencil func type %.*s not valid.", token.len, token.text);
                    return false;
                }
            }
        ...
        // Reference value
            ...
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_NUMBER)) {
                console_log("Stencil reference value not found after decl.");
                return false;
            }
        ...
        // Component mask
            ...
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_NUMBER)) {
                console_log("Stencil component mask value not found after decl.");
                return false;
            }
        ...
        // Write mask
            ...
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_NUMBER)) {
                console_log("Stencil write mask value not found after decl.");
                return false;
            }
        ...
        // Stencil test failure
            ...
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                console_log("Stencil sfail value not found after decl.");
                return false;
            }
                ...
                if (neko_token_compare_text(&token, "KEEP"))
                    ...
                    console_log("Stencil sfail type %.*s not valid.", token.len, token.text);
                    return false;
                }
            }
        }
        ...
        // Stencil test pass, Depth fail
            ...
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                console_log("Stencil dpfail value not found after decl.");
                return false;
            }
                ...
                if (neko_token_compare_text(&token, "KEEP"))
                    ...
                    console_log("Stencil dpfail type %.*s not valid.", token.len, token.text);
                    return false;
                }
            }
        }
        ...
        // Stencil test pass, Depth pass
            ...
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                console_log("Stencil dppass value not found after decl.");
                return false;
            }
                ...
                if (neko_token_compare_text(&token, "KEEP"))
                    ...
                    console_log("Stencil dppass type %.*s not valid.", token.len, token.text);
                    return false;
                }
            }
        }
...
bool neko_parse_raster(neko_lexer_t* lex, neko_draw_pipeline_desc_t* pdesc, neko_ppd_t* ppd) {
    neko_parse_block(PIPELINE::RASTER, {
        ...
        if (neko_token_compare_text(&token, "index_buffer_element_size")) {
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                console_log("Raster index buffer element size not found.", token.len, token.text);
            }
        ...
        if (neko_token_compare_text(&token, "face_culling")) {
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                console_log("Raster face culling type not found.");
                return false;
            }
            ...
            if (neko_token_compare_text(&token, "FRONT"))
                ...
                console_log("Raster face culling type %.*s not valid.", token.len, token.text);
                return false;
            }
        }
        ...
        if (neko_token_compare_text(&token, "winding_order")) {
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                console_log("Raster winding order type not found.");
                return false;
            }
            ...
            if (neko_token_compare_text(&token, "CW"))
                ...
                console_log("Raster winding order type %.*s not valid.", token.len, token.text);
                return false;
            }
        }
        ...
        if (neko_token_compare_text(&token, "primitive")) {
            if (!neko_lexer_find_next_token_type(lex, NEKO_TOKEN_IDENTIFIER)) {
                console_log("Raster primitive type not found.");
                return false;
            }
            ...
            if (neko_token_compare_text(&token, "LINES"))
                ...
                console_log("Raster primitive type %.*s not valid.", token.len, token.text);
                return false;
            }
        }
    });
        ...
        switch (token.type) {
            case NEKO_TOKEN_IDENTIFIER: {
                if (neko_token_compare_text(&token, "shader")) {
                    console_log("[gfxt] parsing shader");
                    if (!neko_parse_shader(lex, desc, ppd)) {
                        console_log("Unable to parse shader descriptor");
                        return false;
                    }
                }
                    ...
                    if (!neko_parse_raster(lex, desc, ppd)) {
                        console_log("Unable to parse raster descriptor");
                        return false;
                    }
                }
                    ...
                    if (!neko_parse_depth(lex, desc, ppd)) {
                        console_log("Unable to parse depth descriptor");
                        return false;
                    }
                }
                    ...
                    if (!neko_parse_stencil(lex, desc, ppd)) {
                        console_log("Unable to parse stencil descriptor");
                        return false;
                    }
                }
                    ...
                    if (!neko_parse_blend(lex, desc, ppd)) {
                        console_log("Unable to parse blend descriptor");
                        return false;
                    }
                }
...
char* neko_pipeline_generate_shader_code(neko_draw_pipeline_desc_t* pdesc, neko_ppd_t* ppd, gfx_shader_stage_type stage) {
    console_log("[gfxt] GENERATING CODE...");

    // Get major/minor version of shader
    gfx_info_t* ginfo = gfx_info();
...
#define _NEKO_VERSION_STR MAJMINSTR  // should be "#version 430\n" or newer
 ...
 neko_draw_pipeline_t neko_draw_pipeline_load_from_file(const char* path) {
    ...
    console_log("Parsing pipeline: %s", path);
    neko_draw_pipeline_t pip = neko_draw_pipeline_load_from_memory_ext(file_data, len, path);
    mem_free(file_data);
    return pip;
}

 neko_draw_pipeline_t neko_draw_pipeline_load_from_memory(const char* file_data, size_t sz) { return neko_draw_pipeline_load_from_memory_ext(file_data, sz, "."); }

 neko_draw_pipeline_t neko_draw_pipeline_load_from_memory_ext(const char* file_data, size_t sz, const char* file_path) {
    // Cast to pip
    neko_draw_pipeline_t pip = NEKO_DEFAULT_VAL();
    ...
    if (file_path) {
        ...
        // Now save dir
        console_log("[gfxt] HERE: %zu", tparen.text - file_path);
        memcpy(ppd.dir, file_path, tparen.text - file_path);
        console_log("[gfxt] PPD_DIR: %s", ppd.dir);
    }
            ...
            case NEKO_TOKEN_IDENTIFIER: {
                if (neko_token_compare_text(&token, "pipeline")) {
                    if (!neko_parse_pipeline(&lex, &pdesc, &ppd)) {
                        console_log("Unable to parse pipeline");
                        return pip;
                    }
                }
            } break;
    ...
    // console_log("[gfxt] %s", c_src);

    // Construct compute shader
    if (c_src) {
        gfx_shader_desc_t sdesc = NEKO_DEFAULT_VAL();
        gfx_shader_source_desc_t source_desc[1] = NEKO_DEFAULT_VAL();
        source_desc[0].type = R_SHADER_STAGE_COMPUTE;
        source_desc[0].source = c_src;
        sdesc.sources = source_desc;
        sdesc.size = 1 * sizeof(gfx_shader_source_desc_t);

        pdesc.pip_desc.compute.shader = gfx_shader_create(sdesc);
    }
    // Construct raster shader
    else {
        gfx_shader_desc_t sdesc = NEKO_DEFAULT_VAL();
        gfx_shader_source_desc_t source_desc[2] = NEKO_DEFAULT_VAL();
        ...
        sdesc.sources = source_desc;
        sdesc.size = 2 * sizeof(gfx_shader_source_desc_t);

        pdesc.pip_desc.raster.shader = gfx_shader_create(sdesc);
    }

    // Set up layout
    pdesc.pip_desc.layout.size = neko_dyn_array_size(pdesc.pip_desc.layout.attrs) * sizeof(gfx_vertex_attribute_desc_t);
    ...
    if (v_src) mem_free(v_src);
    if (f_src) mem_free(f_src);
    if (c_src) mem_free(c_src);
    ...
    for (uint32_t i = 0; i < 3; ++i) {
        if (ppd.code[i]) mem_free(ppd.code[i]);
...
bool neko_asset_mesh_load_from_file(const_str path, void* out, neko_asset_mesh_decl_t* decl, void* data_out, size_t data_size) {
    ...
    if (!neko_os_file_exists(path)) {
        console_log("mesh loader:file does not exist: %s", path);
        return false;
    }
    ...
    if (neko_string_compare_equal(file_ext, "gltf")) {
        ...
        console_log("mesh loader:file extension not supported: %s, file: %s", file_ext, path);
        return false;
    }
    ...
    for (u32 i = 0; i < mesh_count; ++i) {
        ...
        for (u32 p = 0; p < m->prim_count; ++p) {
            ...
            prim.vbo = gfx_vertex_buffer_create(vdesc);
            ...
            // Construct index buffer for primitive
            prim.ibo = gfx_index_buffer_create(idesc);
