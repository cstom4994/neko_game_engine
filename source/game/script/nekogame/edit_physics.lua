    ...
    local function add_r(rs)
        local function add_midx(midxs)
            local function add_midy(midys)
                ns.physics.shape_add_circle(ent, tonumber(rs) or 0,
                                            ng.vec2(tonumber(midxs) or 0,
                                                    tonumber(midys) or 0))
            end
            ns.edit.command_start('offset y: ', add_midy, nil, false,
                                  tostring(mid.y))
        end
        ns.edit.command_start('offset x: ', add_midx, nil, false,
                              tostring(mid.x))
    end
...
local function add_box(ent, bbox)
    ...
    local function add(s)
        ...
        local bbox2 = ng.bbox(ng.vec2(bbox.min.x + r, bbox.min.y + r),
                              ng.vec2(bbox.max.x - r, bbox.max.y - r))
...
ns.edit_inspector.custom['physics'] = {
    add = function (inspector)
        ...
        ns.gui_text.set_str(ns.gui_textbox.get_text(inspector.add_box),
                            'add box')
        ...
        ns.gui_text.set_str(ns.gui_textbox.get_text(inspector.add_poly),
                            'add poly')
        ...
        ns.gui_text.set_str(ns.gui_textbox.get_text(inspector.add_circle),
                            'add circle')
    ...
    post_update = function (inspector)
        ...
        for i = #inspector.shapes, 1, -1 do -- backwards for safe remove
            ...
            if ns.entity.destroyed(shape.window) then
                ...
                ns.gui_window.set_title(shape.window,
                                        ng.enum_tostring('PhysicsShape', t))
                if t == ng.PS_POLYGON then
                    local n = ns.physics.poly_get_num_verts(inspector.ent,
                                                            i - 1)
...
function ns.edit.modes.phypoly.update_all()
    if not ns.physics.has(phypoly_ent) then ns.edit.phypoly_cancel() end

    if #phypoly_verts > 1 then
        for i = 1, #phypoly_verts do
            local j = i < #phypoly_verts and i + 1 or 1
            local a = ns.transform.local_to_world(phypoly_ent,
                                                  phypoly_verts[i])
            local b = ns.transform.local_to_world(phypoly_ent,
                                                  phypoly_verts[j])
