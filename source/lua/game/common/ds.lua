local core = require "neko_lua_ds.core"
local ds = {};
ds.__index = ds
...
local function dumpdata(self, n, name, indent)
    local content = self._content
    local c = content[n]
    if c == nil then
        print(indent .. "[" .. name .. "]")
        ...
        if children then
            ...
            for k, v in pairs(children) do
                ...
                dumpdata(self, v, keyname, indent)
            end
        else
...
local function debug_dump_pack(self, pack)
    ...
    for _, item in ipairs(pack) do
        ...
        print("nodeid:", item[1], key, "-> " .. item[2], value)
    end
end
...
function ds:pack()
    ...
    local function pack_table(root)
        ...
        for keyid, v in pairs(root) do
            ...
            if c ~= nil then
                local nodeid = node[v]
                local item = {nodeid, sibling, keys[keyid], c}
                pack[index] = item
                sibling = nodeid
                index = index + 1
                if content[c] then
