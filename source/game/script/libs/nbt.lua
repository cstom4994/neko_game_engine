...
if isLuaJIT then
    ...
    if ffi.abi("be") then
        ...
        function encodeLongInt(v)
            i64.a = v
            return string.char(i64.b[7], i64.b[6], i64.b[5], i64.b[4], i64.b[3], i64.b[2], i64.b[1], i64.b[0])
        end
        ...
        function encodeFloat(v)
            i32f.a = v
            return string.char(i32f.b[3], i32f.b[2], i32f.b[1], i32f.b[0])
        end
...
elseif isLua53 then
    ...
    function encodeLongInt(v)
        ...
        return string.char(v < 0 and 255 or 0, -- only 53 bits from double
        math.floor(v / 0x1000000000000) % 0x100, math.floor(v / 0x10000000000) % 0x100,
            math.floor(v / 0x100000000) % 0x100, math.floor(v / 0x1000000) % 0x100, math.floor(v / 0x10000) % 0x100,
    ...
    function encodeDouble(v)
        ...
        elseif (mant == 0 and expo == 0) or expo < -1022 then
            ...
            return string.char(sign + math.floor(expo / 16), (expo % 16) * 16 + math.floor(mant / 0x1000000000000),
                math.floor(mant / 0x10000000000) % 256, math.floor(mant / 4294967296) % 256,
                math.floor(mant / 16777216) % 256, math.floor(mant / 65536) % 256, math.floor(mant / 256) % 256,
                mant % 256)
        end
    end
    ...
    function encodeFloat(v)
        ...
        elseif (mant == 0 and expo == 0) or expo < -126 then
            ...
            return string.char(sign + math.floor(expo / 2), (expo % 2) * 128 + math.floor(mant / 65536),
                math.floor(mant / 256) % 256, mant % 256)
        end
    end
...
local function isCorrectTypeID(typeID)
    return type(typeID) == "number" and
               (typeID == TAG_BYTE or typeID == TAG_SHORT or typeID == TAG_INT or typeID == TAG_LONG or typeID ==
                   TAG_FLOAT or typeID == TAG_DOUBLE or typeID == TAG_BYTE_ARRAY or typeID == TAG_STRING or typeID ==
                   TAG_LIST or typeID == TAG_COMPOUND or typeID == TAG_INT_ARRAY or typeID == TAG_LONG_ARRAY)
end
...
if isLua53 then
    ...
    function encodeInt(v)
        v = v % 4294967296
        return string.char(math.floor(v / 16777216), math.floor(v / 65536) % 256, math.floor(v / 256) % 256,
            math.floor(v) % 256)
    end
...
-- NBT TAG_* class
local TagClass = { -- Names, no need to use separate table
"TAG_Byte", "TAG_Short", "TAG_Int", "TAG_Long", "TAG_Float", "TAG_Double", "TAG_Byte_Array", "TAG_String", "TAG_List",
...
function TagClass:__tostring(indent, noname)
    ...
    if self._type == TAG_BYTE or self._type == TAG_SHORT or self._type == TAG_INT then
        return string.format("%s%d", prefix, self._value)
    elseif self._type == TAG_FLOAT or self._type == TAG_DOUBLE then
    ...
    elseif self._type == TAG_LIST or self._type == TAG_COMPOUND then
        ...
        local len = #strb
        return string.format("%s%d entr%s\n%s{\n%s\n%s}", -- uh crazy string formats
        prefix, len, len == 1 and "y" or "ies", ind, table.concat(strb, "\n"), ind)
    elseif self._type == TAG_BYTE_ARRAY or self._type == TAG_INT_ARRAY then
...
function TagClass:getString()
    if self._type == TAG_BYTE_ARRAY or self._type == TAG_LIST or self._type == TAG_COMPOUND or self._type ==
        TAG_INT_ARRAY or self._type == TAG_LONG_ARRAY then
...
function TagClass:getNumber()
    if self._type == TAG_BYTE or self._type == TAG_SHORT or self._type == TAG_INT or self._type == TAG_LONG or
        self._type == TAG_FLOAT or self._type == TAG_DOUBLE then
...
function TagClass:getInteger()
    if self._type == TAG_BYTE or self._type == TAG_SHORT or self._type == TAG_INT or self._type == TAG_FLOAT or
        self._type == TAG_DOUBLE then
...
function TagClass:copy(shallow)
    ...
    elseif self._type == TAG_BYTE_ARRAY or self._type == TAG_INT_ARRAY or self._type == TAG_LONG_ARRAY then
        -- Copy elements
        local new = {}
    ...
    elseif self._type == TAG_LIST then
        ...
        if #self._value == 0 then
            ...
            if noprefix then
                ...
                return "\9" .. encodeShort(#name) .. name .. string.char(typeID) .. encodeInt(#strb) ..
                           table.concat(strb)
            end
        end
    ...
    if self.string and #self.string < n then
        if self.buffer then
            ...
            if not (str) then
                error(msg)
            elseif #str == 0 then
                error("returned buffer is empty string")
            elseif #str < minsize then
                error("returned buffer is less than minsize")
            end
