...
local function cimgui_json(dir, json_file)
    local json = dofile(dir .. "/lib/json.lua")
    local function readall(path)
        local f<close> = assert(io.open(path, "rb"))
        return f:read "a"
    end
    return json.decode(readall(json_file))
end

local m = {}

function m.init(status, json_file)
    local meta = cimgui_json(status.GenDir, json_file)
    local types = {}
    local flags = {}
    local enums = {}
    local funcs = {}
    ...
    for _, typedef_meta in ipairs(meta.typedefs) do
        if conditionals(typedef_meta) and not TodoType[typedef_meta.name] and not flags[typedef_meta.name] and
            not enums[typedef_meta.name] and not is_function_pointer(typedef_meta) then
