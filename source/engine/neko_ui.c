...
NEKO_API_DECL void neko_ui_set_pipeline(neko_ui_context_t* ctx, neko_handle(neko_graphics_pipeline_t) pip, void* layout, size_t sz, neko_idraw_layout_type type) {
    neko_ui_command_t* cmd;
...
typedef struct {
    b32 hit;
    neko_vec3 point;
} neko_ui_gizmo_line_intersection_result_t;

enum { NEKO_UI_AXIS_RIGHT = 0x01, NEKO_UI_AXIS_UP, NEKO_UI_AXIS_FORWARD };
...
typedef struct {
    neko_ui_axis_t right;
    neko_ui_axis_t up;
    neko_ui_axis_t forward;
...
#define NEKO_UI_GIZMO_AXIS_DEFINE_SCALE(MEMBER, OFFSET, DEG, AXIS)                                  \
    do {                                                                                            \
        /* Axis */                                                                                  \
        {                                                                                           \
    ...
    NEKO_UI_GIZMO_AXIS_DEFINE_SCALE(right, neko_v3(-off, 0.f, 0.f), 90.f, NEKO_ZAXIS);
    NEKO_UI_GIZMO_AXIS_DEFINE_SCALE(up, neko_v3(0.f, off, 0.f), 0.f, NEKO_YAXIS);
    NEKO_UI_GIZMO_AXIS_DEFINE_SCALE(forward, neko_v3(0.f, 0.f, off), 90.f, NEKO_XAXIS);
...
#define NEKO_UI_GIZMO_AXIS_DEFINE_TRANSLATE(MEMBER, OFFSET, DEG, AXIS)                              \
    do {                                                                                            \
        /* Axis */                                                                                  \
        {                                                                                           \
            local = neko_vqs_ctor(OFFSET, neko_quat_angle_axis(neko_deg2rad(DEG), AXIS), ax_scl);   \
    ...
    const float off = 0.6f;
    NEKO_UI_GIZMO_AXIS_DEFINE_TRANSLATE(right, neko_v3(-off, 0.f, 0.f), 90.f, NEKO_ZAXIS);
    NEKO_UI_GIZMO_AXIS_DEFINE_TRANSLATE(up, neko_v3(0.f, off, 0.f), 0.f, NEKO_YAXIS);
    NEKO_UI_GIZMO_AXIS_DEFINE_TRANSLATE(forward, neko_v3(0.f, 0.f, off), 90.f, NEKO_XAXIS);
...
#define NEKO_UI_GIZMO_AXIS_DEFINE_ROTATE(MEMBER, OFFSET, DEG, AXIS)                               \
    do {                                                                                          \
        /* Axis */                                                                                \
        {                                                                                         \
            local = neko_vqs_ctor(OFFSET, neko_quat_angle_axis(neko_deg2rad(DEG), AXIS), ax_scl); \
    ...
    NEKO_UI_GIZMO_AXIS_DEFINE_ROTATE(right, neko_v3(0.f, 0.f, 0.f), 90.f, NEKO_YAXIS);
    NEKO_UI_GIZMO_AXIS_DEFINE_ROTATE(up, neko_v3(0.f, 0.f, 0.f), -90.f, NEKO_XAXIS);
    NEKO_UI_GIZMO_AXIS_DEFINE_ROTATE(forward, neko_v3(0.f, 0.f, 0.f), 0.f, NEKO_ZAXIS);
...
typedef struct {
    ...
    neko_ui_rect_t viewport;
    neko_ui_gizmo_line_intersection_result_t li;
} neko_gizmo_desc_t;

static void neko_ui_gizmo_render(neko_ui_context_t* ctx, neko_ui_customcommand_t* cmd) {
    ...
    neko_ui_rect_t clip = cmd->clip;
    neko_ui_rect_t viewport = desc->viewport;
    neko_camera_t cam = desc->camera;
    const u16 segments = 4;
    const neko_ui_gizmo_line_intersection_result_t* li = &desc->li;
...
#define NEKO_UI_GIZMO_AXIS_TRANSLATE(ID, AXIS, COLOR)                                                                                                                            \
    do {                                                                                                                                                                         \
        neko_ui_id id = neko_ui_get_id_hash(ctx, ID, strlen(ID), cmd->hash);                                                                                                     \
...
#define NEKO_UI_GIZMO_AXIS_SCALE(ID, AXIS, COLOR)                                                                                                                                \
    do {                                                                                                                                                                         \
        neko_ui_id id = neko_ui_get_id_hash(ctx, ID, strlen(ID), cmd->hash);                                                                                                     \
        bool hover = cmd->hover == id;                                                                                                                                           \
        bool focus = cmd->focus == id;                                                                                                                                           \
...
#define NEKO_UI_GIZMO_AXIS_ROTATE(ID, AXIS, COLOR)                                                                                                      \
    do {                                                                                                                                                \
    ...
    switch (desc->op) {
        case NEKO_UI_GIZMO_TRANSLATE: {
            NEKO_UI_GIZMO_AXIS_TRANSLATE("#gizmo_trans_right", right, NEKO_COLOR_RED);
            NEKO_UI_GIZMO_AXIS_TRANSLATE("#gizmo_trans_up", up, NEKO_COLOR_GREEN);
            NEKO_UI_GIZMO_AXIS_TRANSLATE("#gizmo_trans_forward", forward, NEKO_COLOR_BLUE);
        } break;

        case NEKO_UI_GIZMO_SCALE: {
            NEKO_UI_GIZMO_AXIS_SCALE("#gizmo_scale_right", right, NEKO_COLOR_RED);
            NEKO_UI_GIZMO_AXIS_SCALE("#gizmo_scale_up", up, NEKO_COLOR_GREEN);
            NEKO_UI_GIZMO_AXIS_SCALE("#gizmo_scale_forward", forward, NEKO_COLOR_BLUE);
        } break;

        case NEKO_UI_GIZMO_ROTATE: {
            neko_ui_id id_r = neko_ui_get_id_hash(ctx, "#gizmo_rotate_right", strlen("#gizmo_rotate_right"), cmd->hash);
            neko_ui_id id_u = neko_ui_get_id_hash(ctx, "#gizmo_rotate_up", strlen("#gizmo_rotate_up"), cmd->hash);
            neko_ui_id id_f = neko_ui_get_id_hash(ctx, "#gizmo_rotate_forward", strlen("#gizmo_rotate_forward"), cmd->hash);

            NEKO_UI_GIZMO_AXIS_ROTATE("#gizmo_rotate_right", right, (cmd->focus == id_u || cmd->focus == id_f) ? neko_color_alpha(NEKO_COLOR_RED, 25) : neko_color_alpha(NEKO_COLOR_RED, alpha));
            NEKO_UI_GIZMO_AXIS_ROTATE("#gizmo_rotate_up", up, (cmd->focus == id_r || cmd->focus == id_f) ? neko_color_alpha(NEKO_COLOR_GREEN, 25) : neko_color_alpha(NEKO_COLOR_GREEN, alpha));
            NEKO_UI_GIZMO_AXIS_ROTATE("#gizmo_rotate_forward", forward, (cmd->focus == id_r || cmd->focus == id_u) ? neko_color_alpha(NEKO_COLOR_BLUE, 25) : neko_color_alpha(NEKO_COLOR_BLUE, alpha));
        } break;
    }
...
static neko_ui_gizmo_line_intersection_result_t neko_ui_gizmo_get_line_intersection(const neko_vqs* model, neko_vec3 axis_a, neko_vec3 axis_b, neko_vec3 axis_c, const neko_camera_t* camera,
                                                                                    const neko_ray_t* ray, neko_vec3 plane_normal_axis, bool compare_supporting_axes, bool override_axis) {
    neko_ui_gizmo_line_intersection_result_t res = neko_default_val();
...
NEKO_API_DECL s32 neko_ui_gizmo(neko_ui_context_t* ctx, neko_camera_t* camera, neko_vqs* model, neko_ui_rect_t viewport, bool invert_view_y, float snap, s32 op, s32 mode, u64 opt) {
    ...
    const neko_vec2 fbs = ctx->framebuffer_size;
    const float t = neko_platform_elapsed_time();
    const bool in_hover_root = neko_ui_in_hover_root(ctx);

    neko_immediate_draw_t* dl = &ctx->overlay_draw_list;

    // This doesn't actually work for the clip...
    neko_ui_rect_t clip = viewport;
    if (invert_view_y) {
    ...
    // Check for scale and bias
    s32 flags = 0x00;
    if (flags & NEKO_UI_HINT_FLAG_NO_SCALE_BIAS_MOUSE) {
    ...
    */
    ...
    desc.xform.rotation = (mode == NEKO_UI_TRANSFORM_LOCAL || op == NEKO_UI_GIZMO_SCALE) ? model->rotation : neko_quat_default();  // This depends on the mode (local/world)
...
#define UPDATE_GIZMO_CONTROL(ID, RAY, SHAPE, MODEL, FUNC, CAP_SHAPE, CAP_MODEL, CAP_FUNC, INFO)                                                \
    ...
    switch (op) {
        case NEKO_UI_GIZMO_TRANSLATE: {
            ...
            neko_ui_id id_r = neko_ui_get_id(ctx, "#gizmo_trans_right", strlen("#gizmo_trans_right"));
            neko_ui_id id_u = neko_ui_get_id(ctx, "#gizmo_trans_up", strlen("#gizmo_trans_up"));
            neko_ui_id id_f = neko_ui_get_id(ctx, "#gizmo_trans_forward", strlen("#gizmo_trans_forward"));
            ...
            // Control
            if (ctx->focus == id_r) {
                desc.li = neko_ui_gizmo_get_line_intersection(&desc.xform, NEKO_XAXIS, NEKO_YAXIS, NEKO_ZAXIS, camera, &ray, neko_v3s(0.f), true, false);
                ...
                desc.li = neko_ui_gizmo_get_line_intersection(&desc.xform, NEKO_YAXIS, NEKO_XAXIS, NEKO_ZAXIS, camera, &ray, neko_v3s(0.f), true, false);
                ...
                desc.li = neko_ui_gizmo_get_line_intersection(&desc.xform, NEKO_ZAXIS, NEKO_XAXIS, NEKO_YAXIS, camera, &ray, neko_v3s(0.f), true, false);
        ...
        case NEKO_UI_GIZMO_SCALE: {
            ...
            neko_ui_id id_r = neko_ui_get_id(ctx, "#gizmo_scale_right", strlen("#gizmo_scale_right"));
            neko_ui_id id_u = neko_ui_get_id(ctx, "#gizmo_scale_up", strlen("#gizmo_scale_up"));
            neko_ui_id id_f = neko_ui_get_id(ctx, "#gizmo_scale_forward", strlen("#gizmo_scale_forward"));
...
#define UPDATE_GIZMO_CONTROL_ROTATE(ID, RAY, SHAPE, MODEL, AXIS, INFO)                                                                         \
    do {                                                                                                                                       \
        ...
        case NEKO_UI_GIZMO_ROTATE: {
            ...
            neko_ui_id id_r = neko_ui_get_id(ctx, "#gizmo_rotate_right", strlen("#gizmo_rotate_right"));
            neko_ui_id id_u = neko_ui_get_id(ctx, "#gizmo_rotate_up", strlen("#gizmo_rotate_up"));
            neko_ui_id id_f = neko_ui_get_id(ctx, "#gizmo_rotate_forward", strlen("#gizmo_rotate_forward"));
            ...
            if (ctx->focus == id_r) {
                desc.li = op == NEKO_UI_TRANSFORM_LOCAL ? neko_ui_gizmo_get_line_intersection(&desc.xform, NEKO_XAXIS, NEKO_YAXIS, NEKO_ZAXIS, camera, &ray, neko_v3s(0.f), false, false)
                                                        : neko_ui_gizmo_get_line_intersection(&desc.xform, NEKO_XAXIS, NEKO_YAXIS, NEKO_ZAXIS, camera, &ray, neko_v3s(0.f), false, true);
                ...
                if (desc.li.hit) {
                    ...
                    if (snap > 0.f) {
                        ...
                        switch (mode) {
                            case NEKO_UI_TRANSFORM_WORLD:
                                model->rotation = neko_quat_mul(s_delta.rotation, orot);
                                break;
                            case NEKO_UI_TRANSFORM_LOCAL:
                ...
                desc.li = op == NEKO_UI_TRANSFORM_LOCAL ? neko_ui_gizmo_get_line_intersection(&desc.xform, NEKO_YAXIS, NEKO_XAXIS, NEKO_ZAXIS, camera, &ray, neko_v3s(0.f), false, false)
                                                        : neko_ui_gizmo_get_line_intersection(&desc.xform, NEKO_YAXIS, NEKO_XAXIS, NEKO_ZAXIS, camera, &ray, neko_v3s(0.f), false, true);
                ...
                desc.li = op == NEKO_UI_TRANSFORM_LOCAL ? neko_ui_gizmo_get_line_intersection(&desc.xform, NEKO_ZAXIS, NEKO_XAXIS, NEKO_YAXIS, camera, &ray, neko_v3s(0.f), false, false)
                                                        : neko_ui_gizmo_get_line_intersection(&desc.xform, NEKO_ZAXIS, NEKO_XAXIS, NEKO_YAXIS, camera, &ray, neko_v3s(0.f), false, true);
    ...
    neko_ui_draw_custom(ctx, clip, neko_ui_gizmo_render, &desc, sizeof(desc));
...
typedef enum { NEKO_UI_SS_DEF_NUMBER = 0x00, NEKO_UI_SS_DEF_ENUM, NEKO_UI_SS_DEF_COLOR, NEKO_UI_SS_DEF_STR } neko_ui_ss_var_def_type;

typedef struct {
    neko_ui_ss_var_def_type type;
    union {
        s32 number;
        neko_color_t color;
        char str[64];
    } val;
} neko_ui_ss_var_def_t;

typedef struct {
    neko_hash_table(u64, neko_ui_ss_var_def_t) variables;
} neko_ui_ss_variables_t;

#define _NEKO_UI_SS_GET_TO_VALUES(T0, T1)                  \
    do {                                                   \
...
bool __neko_ui_style_sheet_parse_attribute_transition(neko_ui_context_t* ctx, neko_lexer_t* lex, neko_ui_style_sheet_t* ss, const u64 id_tag, s32 elementid, s32 state,
                                                      neko_ui_ss_variables_t* variables) {
    ...
    if (id_tag) {
        if (!neko_hash_table_exists(ss->cid_animations, id_tag)) {
            neko_ui_animation_property_list_t sl = neko_default_val();
...
#define PARSE_TRANSITION(T)                                                                                                      \
    do {                                                                                                                         \
        ...
        neko_ui_animation_property_t prop = neko_default_val();                                                                  \
        prop.type = T;                                                                                                           \
    ...
    _NEKO_UI_SS_GET_TO_VALUES(NEKO_TOKEN_COLON, NEKO_TOKEN_LBRACE);

    if (!neko_hash_table_exists(ss->animations, (neko_ui_element_type)elementid)) {
        neko_ui_animation_property_list_t list = neko_default_val();
        neko_hash_table_insert(ss->animations, (neko_ui_element_type)elementid, list);
    }
    neko_ui_animation_property_list_t* list = id_tag ? neko_hash_table_getp(ss->cid_animations, id_tag) : neko_hash_table_getp(ss->animations, (neko_ui_element_type)elementid);
...
bool __neko_ui_style_sheet_parse_attribute_font(neko_ui_context_t* ctx, neko_lexer_t* lex, neko_ui_style_sheet_t* ss, u64 id_tag, s32 elementid, s32 state, neko_ui_ss_variables_t* variables) {
    // the_name of enum attribute
    neko_token_t token = neko_lexer_current_token(lex);
    // neko_token_debug_print(&token);
    //
    neko_ui_style_list_t* idsl = NULL;
    if (id_tag) {
        const u64 idhash = id_tag;
        if (!neko_hash_table_exists(ss->cid_styles, idhash)) {
            neko_ui_style_list_t sl = neko_default_val();
...
bool __neko_ui_style_sheet_parse_attribute_enum(neko_ui_context_t* ctx, neko_lexer_t* lex, neko_ui_style_sheet_t* ss, u64 id_tag, s32 elementid, s32 state, neko_ui_ss_variables_t* variables) {
    // the_name of enum attribute
    neko_token_t token = neko_lexer_current_token(lex);
    // neko_token_debug_print(&token);
...
#define SET_ENUM(COMP, VAL)                                       \
    ...
    if (neko_token_compare_text(&token, "justify_content")) {
        neko_ui_style_element_t se = neko_default_val();
        se.type = NEKO_UI_STYLE_JUSTIFY_CONTENT;
        bool ret = neko_lexer_require_token_type(lex, NEKO_TOKEN_COLON);
        ...
        switch (token.type) {
            case NEKO_TOKEN_IDENTIFIER: {
                if (neko_token_compare_text(&token, "start"))
                    SET_ENUM(justify_content, NEKO_UI_JUSTIFY_START);
                else if (neko_token_compare_text(&token, "end"))
                    SET_ENUM(justify_content, NEKO_UI_JUSTIFY_END);
                else if (neko_token_compare_text(&token, "center"))
                    SET_ENUM(justify_content, NEKO_UI_JUSTIFY_CENTER);
            } break;
        ...
        neko_ui_style_element_t se = neko_default_val();
        se.type = NEKO_UI_STYLE_ALIGN_CONTENT;
        bool ret = neko_lexer_require_token_type(lex, NEKO_TOKEN_COLON);
        ret &= (neko_lexer_require_token_type(lex, NEKO_TOKEN_IDENTIFIER) || neko_lexer_require_token_type(lex, NEKO_TOKEN_DOLLAR));
        if (!ret) {
            neko_log_warning("Missing either identifier value or variable.");
        ...
        switch (token.type) {
            case NEKO_TOKEN_IDENTIFIER: {
                if (neko_token_compare_text(&token, "start"))
                    SET_ENUM(align_content, NEKO_UI_ALIGN_START);
                else if (neko_token_compare_text(&token, "end"))
                    SET_ENUM(align_content, NEKO_UI_ALIGN_END);
                else if (neko_token_compare_text(&token, "center"))
                    SET_ENUM(align_content, NEKO_UI_ALIGN_CENTER);
            } break;
...
bool __neko_ui_style_sheet_parse_attribute_val(neko_ui_context_t* ctx, neko_lexer_t* lex, neko_ui_style_sheet_t* ss, u64 id_tag, s32 elementid, s32 state, neko_ui_ss_variables_t* variables) {
        ...
        switch (token.type) {                                                                                                                             \
            case NEKO_TOKEN_NUMBER: {                                                                                                                     \
                ...
                switch (state) {                                                                                                                          \
                    case NEKO_UI_ELEMENT_STATE_DEFAULT: {                                                                                                 \
                        if (idsl) {                                                                                                                       \
                            neko_dyn_array_push(idsl->styles[NEKO_UI_ELEMENT_STATE_DEFAULT], se);                                                         \
                            neko_dyn_array_push(idsl->styles[NEKO_UI_ELEMENT_STATE_HOVER], se);                                                           \
                            neko_dyn_array_push(idsl->styles[NEKO_UI_ELEMENT_STATE_FOCUS], se);                                                           \
                        } else {                                                                                                                          \
                            for (u32 p = 0; p < 4; ++p) {                                                                                                 \
                                ss->styles[elementid][NEKO_UI_ELEMENT_STATE_DEFAULT].COMP[p] = (T)val;                                                    \
                                ss->styles[elementid][NEKO_UI_ELEMENT_STATE_HOVER].COMP[p] = (T)val;                                                      \
                                ss->styles[elementid][NEKO_UI_ELEMENT_STATE_FOCUS].COMP[p] = (T)val;                                                      \
                            }                                                                                                                             \
                        }                                                                                                                                 \
                    } break;                                                                                                                              \
                    default: {                                                                                                                            \
                ...
                switch (state) {                                                                                                                          \
                    case NEKO_UI_ELEMENT_STATE_DEFAULT: {                                                                                                 \
                        if (idsl) {                                                                                                                       \
                            ...
                            for (u32 p = 0; p < 4; ++p) {                                                                                                 \
                                ss->styles[elementid][NEKO_UI_ELEMENT_STATE_DEFAULT].COMP[p] = val;                                                       \
                                ss->styles[elementid][NEKO_UI_ELEMENT_STATE_HOVER].COMP[p] = val;                                                         \
                                ss->styles[elementid][NEKO_UI_ELEMENT_STATE_FOCUS].COMP[p] = val;                                                         \
                            }                                                                                                                             \
                        }                                                                                                                                 \
                    } break;                                                                                                                              \
                    default: {                                                                                                                            \
...
#define SET_VAL2(COMP0, COMP1, T, SE0, SE1)                                                                                                               \
    do {                                                                                                                                                  \
        neko_ui_style_element_t se0 = neko_default_val();                                                                                                 \
        neko_ui_style_element_t se1 = neko_default_val();                                                                                                 \
        se0.type = SE0;                                                                                                                                   \
        se1.type = SE1;                                                                                                                                   \
        bool ret = neko_lexer_require_token_type(lex, NEKO_TOKEN_COLON);                                                                                  \
        ret &= (neko_lexer_require_token_type(lex, NEKO_TOKEN_DOLLAR) || neko_lexer_require_token_type(lex, NEKO_TOKEN_NUMBER));                          \
        ...
        switch (state) {                                                                                                                                  \
            case NEKO_UI_ELEMENT_STATE_DEFAULT: {                                                                                                         \
                if (idsl) {                                                                                                                               \
                    se0.value = val;                                                                                                                      \
                    se1.value = val;                                                                                                                      \
                    neko_dyn_array_push(idsl->styles[NEKO_UI_ELEMENT_STATE_DEFAULT], se0);                                                                \
                    neko_dyn_array_push(idsl->styles[NEKO_UI_ELEMENT_STATE_HOVER], se0);                                                                  \
                    neko_dyn_array_push(idsl->styles[NEKO_UI_ELEMENT_STATE_FOCUS], se0);                                                                  \
                    neko_dyn_array_push(idsl->styles[NEKO_UI_ELEMENT_STATE_DEFAULT], se1);                                                                \
                    neko_dyn_array_push(idsl->styles[NEKO_UI_ELEMENT_STATE_HOVER], se1);                                                                  \
                    neko_dyn_array_push(idsl->styles[NEKO_UI_ELEMENT_STATE_FOCUS], se1);                                                                  \
                } else {                                                                                                                                  \
                    ss->styles[elementid][NEKO_UI_ELEMENT_STATE_DEFAULT].COMP0 = val;                                                                     \
                    ss->styles[elementid][NEKO_UI_ELEMENT_STATE_HOVER].COMP0 = val;                                                                       \
                    ss->styles[elementid][NEKO_UI_ELEMENT_STATE_FOCUS].COMP0 = val;                                                                       \
                    ss->styles[elementid][NEKO_UI_ELEMENT_STATE_DEFAULT].COMP1 = val;                                                                     \
                    ss->styles[elementid][NEKO_UI_ELEMENT_STATE_HOVER].COMP1 = val;                                                                       \
                    ss->styles[elementid][NEKO_UI_ELEMENT_STATE_FOCUS].COMP1 = val;                                                                       \
                }                                                                                                                                         \
            } break;                                                                                                                                      \
            default: {                                                                                                                                    \
                if (idsl) {                                                                                                                               \
...
#define SET_VAL(COMP, T, SE)                                                                                                                              \
        ...
        switch (state) {                                                                                                                                  \
            case NEKO_UI_ELEMENT_STATE_DEFAULT: {                                                                                                         \
                if (idsl) {                                                                                                                               \
                    ...
                    ss->styles[elementid][NEKO_UI_ELEMENT_STATE_DEFAULT].COMP = val;                                                                      \
                    ss->styles[elementid][NEKO_UI_ELEMENT_STATE_HOVER].COMP = val;                                                                        \
                    ss->styles[elementid][NEKO_UI_ELEMENT_STATE_FOCUS].COMP = val;                                                                        \
                }                                                                                                                                         \
            } break;                                                                                                                                      \
            default: {                                                                                                                                    \
                if (idsl) {                                                                                                                               \
    ...
    if (neko_token_compare_text(&token, "width"))
        SET_VAL(size[0], float, NEKO_UI_STYLE_WIDTH);
    else if (neko_token_compare_text(&token, "height"))
        SET_VAL(size[1], float, NEKO_UI_STYLE_HEIGHT);
    else if (neko_token_compare_text(&token, "padding"))
        SET_VAL4(padding, s16, NEKO_UI_STYLE_PADDING);
    else if (neko_token_compare_text(&token, "padding_left"))
        SET_VAL(padding[NEKO_UI_PADDING_LEFT], s16, NEKO_UI_STYLE_PADDING_LEFT);
    else if (neko_token_compare_text(&token, "padding_right"))
        SET_VAL(padding[NEKO_UI_PADDING_RIGHT], s16, NEKO_UI_STYLE_PADDING_RIGHT);
    else if (neko_token_compare_text(&token, "padding_top"))
        SET_VAL(padding[NEKO_UI_PADDING_TOP], s16, NEKO_UI_STYLE_PADDING_TOP);
    else if (neko_token_compare_text(&token, "padding_bottom"))
        SET_VAL(padding[NEKO_UI_PADDING_BOTTOM], s16, NEKO_UI_STYLE_PADDING_BOTTOM);
    else if (neko_token_compare_text(&token, "margin"))
        SET_VAL4(margin, s16, NEKO_UI_STYLE_MARGIN);
    else if (neko_token_compare_text(&token, "margin_left"))
        SET_VAL(margin[NEKO_UI_MARGIN_LEFT], s16, NEKO_UI_STYLE_MARGIN_LEFT);
    else if (neko_token_compare_text(&token, "margin_right"))
        SET_VAL(margin[NEKO_UI_MARGIN_RIGHT], s16, NEKO_UI_STYLE_MARGIN_RIGHT);
    else if (neko_token_compare_text(&token, "margin_top"))
        SET_VAL(margin[NEKO_UI_MARGIN_TOP], s16, NEKO_UI_STYLE_MARGIN_TOP);
    else if (neko_token_compare_text(&token, "margin_bottom"))
        SET_VAL(margin[NEKO_UI_MARGIN_BOTTOM], s16, NEKO_UI_STYLE_MARGIN_BOTTOM);
    else if (neko_token_compare_text(&token, "border"))
        SET_VAL4(border_width, s16, NEKO_UI_STYLE_BORDER_WIDTH);
    else if (neko_token_compare_text(&token, "border_left"))
        SET_VAL(border_width[0], s16, NEKO_UI_STYLE_BORDER_WIDTH_LEFT);
    else if (neko_token_compare_text(&token, "border_right"))
        SET_VAL(border_width[1], s16, NEKO_UI_STYLE_BORDER_WIDTH_RIGHT);
    else if (neko_token_compare_text(&token, "border_top"))
        SET_VAL(border_width[2], s16, NEKO_UI_STYLE_BORDER_WIDTH_TOP);
    else if (neko_token_compare_text(&token, "border_bottom"))
        SET_VAL(border_width[3], s16, NEKO_UI_STYLE_BORDER_WIDTH_BOTTOM);
    else if (neko_token_compare_text(&token, "shadow"))
        SET_VAL2(shadow_x, shadow_y, s16, NEKO_UI_STYLE_SHADOW_X, NEKO_UI_STYLE_SHADOW_Y);

    return true;
}

bool __neko_ui_style_sheet_parse_attribute_color(neko_ui_context_t* ctx, neko_lexer_t* lex, neko_ui_style_sheet_t* ss, u64 id_tag, s32 elementid, s32 state, neko_ui_ss_variables_t* variables) {
    ...
    neko_ui_style_element_type type = (neko_ui_style_element_type)0x00;
    s32 color = NEKO_UI_COLOR_BACKGROUND;
    if (neko_token_compare_text(&token, "color_background")) {
        color = NEKO_UI_COLOR_BACKGROUND;
        type = NEKO_UI_STYLE_COLOR_BACKGROUND;
    } else if (neko_token_compare_text(&token, "color_border")) {
        color = NEKO_UI_COLOR_BORDER;
        type = NEKO_UI_STYLE_COLOR_BORDER;
    } else if (neko_token_compare_text(&token, "color_shadow")) {
        color = NEKO_UI_COLOR_SHADOW;
        type = NEKO_UI_STYLE_COLOR_SHADOW;
    } else if (neko_token_compare_text(&token, "color_content")) {
        color = NEKO_UI_COLOR_CONTENT;
        type = NEKO_UI_STYLE_COLOR_CONTENT;
    } else if (neko_token_compare_text(&token, "color_content_background")) {
        color = NEKO_UI_COLOR_CONTENT_BACKGROUND;
        type = NEKO_UI_STYLE_COLOR_CONTENT_BACKGROUND;
    } else if (neko_token_compare_text(&token, "color_content_border")) {
        color = NEKO_UI_COLOR_CONTENT_BORDER;
        type = NEKO_UI_STYLE_COLOR_CONTENT_BORDER;
    } else if (neko_token_compare_text(&token, "color_content_shadow")) {
...
bool __neko_ui_style_sheet_parse_attribute(neko_ui_context_t* ctx, neko_lexer_t* lex, neko_ui_style_sheet_t* ss, u64 id_tag, s32 elementid, s32 state, neko_ui_ss_variables_t* variables) {
...
bool __neko_ui_style_sheet_parse_element(neko_ui_context_t* ctx, neko_lexer_t* lex, neko_ui_style_sheet_t* ss, s32 elementid, neko_ui_ss_variables_t* variables) {
    s32 state = 0x00;
    s32 bc = 0;
    neko_token_t token = neko_lexer_next_token(lex);
    if (token.type == NEKO_TOKEN_COLON) {
        ...
        if (neko_token_compare_text(&token, "focus"))
            state = NEKO_UI_ELEMENT_STATE_FOCUS;
...
bool __neko_ui_style_sheet_parse_cid_tag(neko_ui_context_t* ctx, neko_lexer_t* lex, neko_ui_style_sheet_t* ss, const u64 cid_tag, neko_ui_ss_variables_t* variables) {
    s32 state = 0x00;
    s32 bc = 0;
    neko_token_t token = neko_lexer_next_token(lex);
    if (token.type == NEKO_TOKEN_COLON) {
...
NEKO_API_DECL neko_ui_style_sheet_t neko_ui_style_sheet_load_from_file(neko_ui_context_t* ctx, const char* file_path) {
    // Generate new style sheet based on default element styles
    neko_ui_style_sheet_t ss = neko_default_val();
    bool success = true;
...
static bool __neko_ui_style_sheet_parse_variable(neko_ui_context_t* ctx, neko_lexer_t* lex, neko_ui_style_sheet_t* ss, char* name_buf, size_t sz, neko_ui_ss_var_def_t* out) {
        ...
        switch (token.type) {
            case NEKO_TOKEN_IDENTIFIER: {
                if (neko_token_compare_text(&token, "rgb")) {
                    ...
                    out->val.number = (s32)NEKO_UI_JUSTIFY_CENTER;
                } else if (neko_token_compare_text(&token, "start")) {
                    out->type = NEKO_UI_SS_DEF_ENUM;
                    out->val.number = (s32)NEKO_UI_JUSTIFY_START;
                } else if (neko_token_compare_text(&token, "end")) {
                    out->type = NEKO_UI_SS_DEF_ENUM;
            ...
            case NEKO_TOKEN_NUMBER: {
                neko_snprintfc(VAL, 32, "%.*s", token.len, token.text);
                s32 v = (s32)atoi(VAL);
                out->type = NEKO_UI_SS_DEF_NUMBER;
                out->val.number = v;
            } break;

            case NEKO_TOKEN_STRING: {
                neko_snprintfc(VAL, 64, "%.*s", token.len - 2, token.text + 1);
                out->type = NEKO_UI_SS_DEF_STR;
...
NEKO_API_DECL neko_ui_style_sheet_t neko_ui_style_sheet_load_from_memory(neko_ui_context_t* ctx, const char* fd, size_t sz, bool* sp) {
    // Generate new style sheet based on default element styles
    neko_ui_style_sheet_t ss = neko_default_val();
    bool success = true;

    neko_ui_ss_variables_t variables = neko_default_val();

    // Copy all default styles
    NEKO_UI_COPY_STYLES(ss.styles, neko_ui_default_style_sheet.styles, NEKO_UI_ELEMENT_CONTAINER);
    NEKO_UI_COPY_STYLES(ss.styles, neko_ui_default_style_sheet.styles, NEKO_UI_ELEMENT_LABEL);
    NEKO_UI_COPY_STYLES(ss.styles, neko_ui_default_style_sheet.styles, NEKO_UI_ELEMENT_TEXT);
    NEKO_UI_COPY_STYLES(ss.styles, neko_ui_default_style_sheet.styles, NEKO_UI_ELEMENT_PANEL);
    NEKO_UI_COPY_STYLES(ss.styles, neko_ui_default_style_sheet.styles, NEKO_UI_ELEMENT_INPUT);
    NEKO_UI_COPY_STYLES(ss.styles, neko_ui_default_style_sheet.styles, NEKO_UI_ELEMENT_BUTTON);
    NEKO_UI_COPY_STYLES(ss.styles, neko_ui_default_style_sheet.styles, NEKO_UI_ELEMENT_SCROLL);
    NEKO_UI_COPY_STYLES(ss.styles, neko_ui_default_style_sheet.styles, NEKO_UI_ELEMENT_IMAGE);
        ...
        switch (token.type) {
            case NEKO_TOKEN_IDENTIFIER: {
                if (neko_token_compare_text(&token, "button"))
                    PARSE_ELEMENT(NEKO_UI_ELEMENT_BUTTON, "button");
                else if (neko_token_compare_text(&token, "text"))
                    PARSE_ELEMENT(NEKO_UI_ELEMENT_TEXT, "text");
                else if (neko_token_compare_text(&token, "label"))
                    PARSE_ELEMENT(NEKO_UI_ELEMENT_LABEL, "label");
                else if (neko_token_compare_text(&token, "image"))
                    PARSE_ELEMENT(NEKO_UI_ELEMENT_IMAGE, "image");
                else if (neko_token_compare_text(&token, "scroll"))
                    PARSE_ELEMENT(NEKO_UI_ELEMENT_SCROLL, "scroll");
                else if (neko_token_compare_text(&token, "panel"))
                    PARSE_ELEMENT(NEKO_UI_ELEMENT_PANEL, "panel");
                else if (neko_token_compare_text(&token, "container"))
                    PARSE_ELEMENT(NEKO_UI_ELEMENT_CONTAINER, "container");
            ...
            case NEKO_TOKEN_DOLLAR: {
                ...
                typedef enum
                {
                    NEKO_UI_SS_DEF_VALUE = 0x00,
                    NEKO_UI_SS_DEF_COLOR,
                    NEKO_UI_SS_DEF_STR
                } neko_ui_ss_var_def_type;
