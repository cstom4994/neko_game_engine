...
local types = require "imgui_types"
...
write_arg["void*"] = function(type_meta, context)
    ...
    elseif type_meta.default_value == "NULL" then
        writeln("    auto %s = lua_isnoneornil(L, %d)? NULL: lua_touserdata(L, %d);", type_meta.name, context.idx,
            context.idx)
    else
        assert(false)
    end
...
write_arg["ImVec4"] = function(type_meta, context)
    if type_meta.default_value == nil then
        ...
        local def_x, def_y, def_z, def_w =
            type_meta.default_value:match "^ImVec4%(([^,]+), ([^,]+), ([^,]+), ([^,]+)%)$"
        writeln("    auto %s = ImVec4 {", type_meta.name)
        writeln("        (float)luaL_optnumber(L, %d, %s),", context.idx + 1, def_x)
        writeln("        (float)luaL_optnumber(L, %d, %s),", context.idx + 2, def_y)
...
write_arg["const ImWchar*"] = function(type_meta, context)
    ...
    if type_meta.default_value == "NULL" then
        ...
        writeln("    case LUA_TLIGHTUSERDATA: %s = (const ImWchar*)lua_touserdata(L, %d); break;", type_meta.name,
            context.idx)
...
write_arg["bool"] = function(type_meta, context)
    ...
    context.arguments[#context.arguments + 1] = type_meta.name
    if type_meta.default_value then
        writeln("    auto %s = lua_isnoneornil(L, %d)? %s: !!lua_toboolean(L, %d);", type_meta.name, context.idx,
            type_meta.default_value, context.idx)
    else
...
for _, type_name in ipairs {"int", "unsigned int", "size_t", "ImU32", "ImWchar", "ImWchar16", "ImDrawIdx", "ImGuiID",
                            "ImGuiKeyChord"} do
    write_arg[type_name] = function(type_meta, context)
        context.idx = context.idx + 1
        if type_meta.default_value then
            writeln("    auto %s = (%s)luaL_optinteger(L, %d, %s);", type_meta.name, type_name, context.idx,
                type_meta.default_value)
        else
...
local function write_flags_and_enums()
    ...
    local function init_enum(realname, elements)
        ...
        write_arg[realname] = function(type_meta, context)
            context.idx = context.idx + 1
            if type_meta.default_value then
                writeln("    auto %s = (%s)luaL_optinteger(L, %d, lua_Integer(%s));", type_meta.name, realname,
                    context.idx, find_name(type_meta.default_value))
            else
    ...
    for _, v in ipairs(status.structs) do
        ...
        for _, nametype in ipairs {"const " .. name .. "*", name .. "*"} do
            write_arg[nametype] = function(type_meta, context)
                ...
                elseif type_meta.default_value == "NULL" then
                    writeln("    auto %s = lua_isnoneornil(L, %d)? NULL: *(%s*)lua_touserdata(L, %d);", type_meta.name,
                        context.idx, nametype, context.idx)
                else
                    assert(false)
                end
            end
...
writeln "#include \"engine/neko_ui.h\""
writeln ""
writeln "namespace neko::imgui {"
writeln ""
...
writeln ""
-- writeln "extern \"C\""
writeln "int open_imgui(lua_State *L) {"
writeln "    neko::imgui::init(L);"
writeln "    return 1;"
writeln "}"
