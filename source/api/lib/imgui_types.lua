...
local builtin_get<const> = {
    ["boolean"] = {"{RET}!!lua_toboolean(L, {IDX});"},
    ["integer"] = {"{RET}luaL_checkinteger(L, {IDX});"},
    ["number"] = {"{RET}luaL_checknumber(L, {IDX});"},
    ["lightuserdata"] = {"luaL_checktype(L, {IDX}, LUA_TLIGHTUSERDATA);", "{RET}lua_touserdata(L, {IDX});"}
}
