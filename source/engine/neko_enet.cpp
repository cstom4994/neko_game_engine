...
// neko_enet based on lua-enet by Leaf Corcoran (https://leafo.net/lua-enet)
...
#define check_host(l, idx) *(ENetHost **)luaL_checkudata(l, idx, "enet_host")

#define check_peer(l, idx) *(ENetPeer **)luaL_checkudata(l, idx, "enet_peer")
...
static size_t find_peer_index(lua_State *l, ENetHost *enet_host, ENetPeer *peer) {
    ...
    for (peer_index = 0; peer_index < enet_host->peerCount; peer_index++) {
        if (peer == &(enet_host->peers[peer_index])) return peer_index;
    }
...
static int checked_userdata(lua_State *L) {
    // 尝试在所有位都已设置的情况下推动指针
...
static bool supports_full_lightuserdata(lua_State *L) {
    ...
    if (!checked) {
        lua_pushcclosure(L, checked_userdata, 0);
...
// // 确保 max_align_t 的对齐要求是最大的
// _Static_assert(sizeof(max_align_t) % _Alignof(max_align_t) == 0, "Alignment requirement not met");
...
static uintptr_t compute_peer_key(lua_State *L, ENetPeer *peer) {
    ...
    const size_t minalign = sizeof(void *) == 8 ? NEKO_MIN(__alignof(ENetPeer), __alignof(max_align_t)) : 1;
    uintptr_t key = (uintptr_t)peer;

    if ((key & (minalign - 1)) != 0) {
        luaL_error(L,
                   "Cannot push enet peer to Lua: unexpected alignment "
                   "(pointer is %p but alignment should be %d)",
                   peer, minalign);
    }

    size_t shift = (size_t)log2((double)minalign);
...
static void push_peer_key(lua_State *L, uintptr_t key) {
    ...
    if (supports_full_lightuserdata(L)) lua_pushlightuserdata(L, (void *)key);
...
static int linked_version(lua_State *l) {
    lua_pushfstring(l, "%d.%d.%d", ENET_VERSION_GET_MAJOR(enet_linked_version()), ENET_VERSION_GET_MINOR(enet_linked_version()), ENET_VERSION_GET_PATCH(enet_linked_version()));
    return 1;
}
...
static int host_service(lua_State *l) {
    ...
    if (lua_gettop(l) > 1) timeout = (int)luaL_checknumber(l, 2);
...
static int host_get_socket_address(lua_State *l) {
    ...
    lua_pushfstring(l, "%d.%d.%d.%d:%d", ((address.host) & 0xFF), ((address.host >> 8) & 0xFF), ((address.host >> 16) & 0xFF), (address.host >> 24 & 0xFF), address.port);
    return 1;
}
...
static const struct luaL_Reg enet_funcs[] = {{"host_create", host_create}, {"linked_version", linked_version}, {NULL, NULL}};

static const struct luaL_Reg enet_host_funcs[] = {{"service", host_service},
...
static const struct luaL_Reg enet_peer_funcs[] = {{"disconnect", peer_disconnect},
...
extern "C" int luaopen_enet(lua_State *l) {
    ...
    luax_register(l, NULL, enet_funcs);

    // 返回使用 luaL_register 创建的 enet 表
    return 1;
}
