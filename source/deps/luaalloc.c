...
inline static ubitmap *getbitmap(Block *b) { return &b->bitmap[0]; }

inline static void *getdata(Block *b) { return ((char *)getbitmap(b)) + (b->bitmapInts * sizeof(ubitmap)); }

inline static void *getdataend(Block *b) { return ((char *)getdata(b)) + ((size_t)b->elemSize * b->elemstotal); }
...
inline static unsigned bsizeindex(const Block *b) { return sizeindex(b->elemSize); }

static int contains(Block *b, const void *p) { return getdata(b) <= p && p < getdataend(b); }
...
inline static size_t blocksize(Block *b) { return (char *)getdataend(b) - (char *)b; }

inline static u16 nextblockelems(Block *b) {
    if (!b) return LA_ELEMS_MIN;
...
typedef enum { LA_TYPE_LARGELUA = 0, LA_TYPE_BLOCK = 1, LA_TYPE_INTERNAL = 2 } AllocType;
...
static void freeblock(LuaAlloc *LA_RESTRICT LA, Block **LA_RESTRICT spot) {
    ...
    if (LA->active[si] == b) LA->active[si] = NULL;
...
static void *_Balloc(Block *b) {
    ...
    unsigned i = 0, bm;
    for (; !(bm = bitmap[i]); ++i) {
    }                                       /* as soon as one isn't all zero, there's a free slot */
...
static Block *getfreeblock(LuaAlloc *LA, u16 size) {
    ...
    b = LA->chain[si];
    while (b && !b->elemsfree) b = b->prev;

    /* Still no good? Allocate new block */
    if (!b || !b->elemsfree) b = newblock(LA, nextblockelems(LA->chain[si]), size); /* Use newest block in chain to compute size */
...
static void *_Realloc(LuaAlloc *LA_RESTRICT LA, void *LA_RESTRICT p, size_t newsize, size_t oldsize) {
    ...
    if (!newptr) return newsize <= oldsize ? p : NULL;
...
void luaalloc_delete(LuaAlloc *LA) {
    ...
    if (LA->all) sysfree(LA, LA->all, LA->allcap * sizeof(Block *));
...
unsigned luaalloc_getstats(const LuaAlloc *LA, const size_t **alive, const size_t **total, const size_t **blocks, unsigned *pbinstep) {
    if (pbinstep) *pbinstep = LA_ALLOC_STEP;

#ifdef LA_TRACK_STATS
    if (alive) *alive = LA->stats.alive;
    if (total) *total = LA->stats.total;
    if (blocks) *blocks = LA->stats.blocks_alive;
