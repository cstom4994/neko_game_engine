...
local NATIVE_FN = "neko_api_"
local NATIVE_ENUM = "NEKO"
...
static inline void *_checklightuserdata(lua_State *L, int narg) {
    ...
    if (ret == NULL && !lua_islightuserdata(L, narg)) {
        luaL_typeerror(L, narg, lua_typename(L, LUA_TLIGHTUSERDATA));
    }
    return ret;
}
...
static inline size_t _checktable_getcount(lua_State *L, int narg) {
    ...
    size_t cnt = lua_rawlen(L, narg);
    return cnt;
}

static inline void luax_setfuncs(lua_State *L, const luaL_Reg *l) {
    if (l == NULL) return;
...
static inline void luax_register(lua_State *L, const char *name, const luaL_Reg *l) {
...
local CTypeMaps = {
    ["bool"] = {
        c = "bool",
        check_fn = "lua_toboolean",
        push_fn = "lua_pushboolean"
    },
    ["int"] = {
        c = "int",
        check_fn = "luaL_checknumber",
        push_fn = "lua_pushinteger"
    },
    ["float"] = {
        c = "float",
        check_fn = "luaL_checknumber",
        push_fn = "lua_pushnumber"
    },
    ["double"] = {
        c = "double",
        check_fn = "luaL_checknumber",
    ...
    ["string"] = {
        c = "const char *",
        check_fn = "luaL_checkstring",
        push_fn = "lua_pushstring"
    },
    ["pointer"] = {
        c = "void *",
        check_fn = "_checklightuserdata",
        push_fn = "lua_pushlightuserdata"
    }
}

local function get_register_fn_name(NS)
    return "register_" .. NS.name .. "_open"
end
...
local function get_include_guard_name(filename)
    return "_" .. filename:gsub("[.]", "_"):upper() .. "_INCLUDED_"
end
...
local function get_list_item_type(NS, name)
    if not is_type_list(NS, name) then
...
local function get_c_typed_var(NS, t, varname, is_ptr, is_arglist, is_ret)
    ...
    elseif is_list then
        if is_arglist then
            ...
            if not is_ret then
                ret =
                    get_list_item_type(NS, t) .. " " .. varname .. "[" .. get_list_item_max_count(NS, t) .. "] = {0}" ..
                        "; " .. "size_t " .. varname .. "_count"
            else
                ret = get_list_item_type(NS, t) .. " *" .. varname .. "; " .. "size_t " .. varname .. "_count"
            end
        end
...
local function get_c_typed_args_and_rets(NS, fn, out_kind)
    ...
    if #fn.args > 0 or #fn.rets > 0 then
        ...
        for _i, a in ipairs(fn.args) do
            ...
            --     out = out .. " = {0};"
            -- end
            if _i < #fn.args or #fn.rets > 0 then
                out = out .. sep
            end
        end
        -- rets
...
local function get_gen_func_base(NS, fn)
    ...
    for _i, a in ipairs(fn.args) do
        ...
        elseif is_type_enum(NS, a.type) then
            ...
            out =
                out .. SPC .. a.name .. " = enumstring_to_int(" .. NS.name .. "_" .. a.type .. "_strings, " .. a.name ..
                    "_str);\n"
                ...
                if lst.name == r.type then
                    ...
                    if CTypeMaps[item_type] or is_type_enum(NS, item_type) then
                        if CTypeMaps[item_type] then
                            ...
                            push_fn = CTypeMaps["string"].push_fn .. "(L, int_to_enumstring(" .. map_name .. ", " ..
                                          r.name .. "[i]" .. "));\n"
                        end
...
local function get_gen_func_impl(NS, fn)
    ...
    out = out .. "int err = 0;\n"
    out = out .. SPC .. "err = " .. NATIVE_FN .. fn.name .. "("
    for _i, a in ipairs(fn.args) do
        out = out .. a.name
        if is_type_list(NS, a.type) then -- add count variable in list special case
            out = out .. ", " .. a.name .. "_count"
...
local function gen_api_base(NS, filename, header_file_name, impl_file_name)
    ...
    for _i, x in ipairs(NS.enums) do
        -- EnumStrInt neko_UniformType_strings[] = {
    ...
    out = out .. SPC .. 'luax_register(L, "' .. NS.name .. '", ' .. NS.name .. "_api_functions);\n"
    out = out .. SPC .. "lua_settop(L, 0);\n"
    -- out = out .. SPC .. "lua_settop(L, lua_gettop(L) - 1); // 保留底部的 luax_msgh\n"
...
local function gen_api_impl(NS, filename)
    ...
    for _i, x in ipairs(NS.enums) do
        ...
        for _j, y in ipairs(x.values) do
            ...
            local enum_value = NATIVE_ENUM .. enum_name -- hardcoding these.. changable with simple copy paste
...
local function generate_api_native(NS, output_folder)
    ...
    local header_file_name = "gen_" .. NS.name .. ".h"
    local base_file_name = "gen_" .. NS.name .. ".cpp"
    local wrap_file_name_edit = "gen_" .. NS.name .. "_warp__EDIT__.hpp"
    local wrap_file_name = wrap_file_name_edit:gsub("__EDIT__", "")

    local header_src = gen_header(NS, header_file_name)
    local base_src = gen_api_base(NS, base_file_name, header_file_name, wrap_file_name)
    local impl_src = gen_api_impl(NS, wrap_file_name)
    ...
    local impl_file = io.open(output_folder .. "/" .. wrap_file_name, "w")
