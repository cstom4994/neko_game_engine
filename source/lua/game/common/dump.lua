local M = {
    ...
    identifier = "^[a-zA-Z_][a-zA-Z0-9_]*$", -- 不支持UTF8标识符
    reserved = {
        ["and"] = true,
        ["break"] = true,
        ["do"] = true,
...
local function internal_mini_dump(t, lvl, cfg)
    ...
    if type(t) == "table" then
        ...
        for k, v in pairs(t) do
            r[#r + 1] = (cfg.indent or ""):rep(lvl) .. "[" .. internal_mini_dump(k, lvl, cfg) .. "]" ..
                            (cfg.assign or "=") .. internal_mini_dump(v, lvl, cfg) .. ","
        end
        lvl = lvl - 1
...
local function doublequote_dump_string(s, nonprintable_pat, nonprintable_names)
    ...
    nonprintable_names = nonprintable_names or {
        ["\0"] = "0",
        ["\a"] = "a",
        ["\b"] = "b",
        ["\t"] = "t",
    ...
    return '"' .. (s:gsub("[\"\\]", function(cap)
        return "\\" .. cap
    end):gsub(nonprintable_pat, function(cap)
    ...
    if type(t) == "table" then
        ...
        for k, v in pairs(t) do
            if not skipassign or not skipassign(t, k, v) then
                local line = ""
                -- 如果k是一个数字索引
                -- 检查是否真的是当前索引 : it could also be a number <= 0 or a float, ...
                -- 仅在未通过配置拒绝的情况下
                if type(k) == "number" and i == k and (cfg.ishort ~= false) then
                    i = i + 1 -- 增加隐式索引
                    --	"two",
                    --	AST: `Id{ "two" }
                    -- 它是关键字/散列索引 k是有效的标识符，而不是保留字
                elseif type(k) == "string" and cfg.kshort ~= false and
                    (type(identifier) ~= "string" or k:find(identifier)) and (not reserved or not reserved[k]) then
                    line = k .. assign
                    --	foo="FOO",
