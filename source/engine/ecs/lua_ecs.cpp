...
struct EcsLuaWrap {
    ...
    static int l_ecs_match_component(lua_State* L) {
        ...
        EcsWorld* world = (struct EcsWorld*)luaL_checkudata(L, ECS_WORLD, ECS_WORLD_OLD_UDATA_NAME);
...
int l_ecs_create_world(lua_State* L) {
    ...
    if (luaL_getmetatable(L, ECS_WORLD_OLD_UDATA_NAME) == LUA_TNIL) {
        luaL_Reg world_mt[] = {
                {"__index", NULL},
                {"__name", NULL},
        ...
        lua_pushliteral(L, ECS_WORLD_OLD_UDATA_NAME);
        lua_setfield(L, -2, "__name");
        lua_pushvalue(L, -1);
        lua_setfield(L, LUA_REGISTRYINDEX, ECS_WORLD_OLD_UDATA_NAME);
    }
