...
#define neko_core_ui_unused(x) ((void)(x))

#define neko_core_ui_stack_push(stk, val)                                           \
    do {                                                                            \
...
#define neko_core_ui_stack_pop(stk) \
    do {                            \
...
/* 32bit fnv-1a hash */
#define NEKO_CORE_UI_HASH_INITIAL 2166136261

static void neko_core_ui_hash(neko_core_ui_id* hash, const void* data, s32 size) {
...
static neko_core_ui_rect_t neko_core_ui_unclipped_rect = {0, 0, 0x1000000, 0x1000000};

// Default styles
static neko_core_ui_style_t neko_core_ui_default_container_style[3] = neko_default_val();
static neko_core_ui_style_t neko_core_ui_default_button_style[3] = neko_default_val();
static neko_core_ui_style_t neko_core_ui_default_text_style[3] = neko_default_val();
static neko_core_ui_style_t neko_core_ui_default_label_style[3] = neko_default_val();
static neko_core_ui_style_t neko_core_ui_default_panel_style[3] = neko_default_val();
static neko_core_ui_style_t neko_core_ui_default_input_style[3] = neko_default_val();
static neko_core_ui_style_t neko_core_ui_default_scroll_style[3] = neko_default_val();
static neko_core_ui_style_t neko_core_ui_default_image_style[3] = neko_default_val();

static neko_core_ui_style_t neko_core_ui_default_style = {
        ...
        // colors
        {
                {25, 25, 25, 255},     // NEKO_CORE_UI_COLOR_BACKGROUND
                {255, 255, 255, 255},  // NEKO_CORE_UI_COLOR_CONTENT
                {29, 29, 29, 76},      // NEKO_CORE_UI_COLOR_BORDER
                {0, 0, 0, 31},         // NEKO_CORE_UI_COLOR_SHADOW
                {0, 0, 0, 0},          // NEKO_CORE_UI_COLOR_CONTENT_BACKGROUND
                {0, 0, 0, 0},          // NEKO_CORE_UI_COLOR_CONTENT_SHADOW
                {0, 0, 0, 0}           // NEKO_CORE_UI_COLOR_CONTENT_BORDER
        },
        ...
        NEKO_CORE_UI_DIRECTION_COLUMN,
        NEKO_CORE_UI_JUSTIFY_START,
        NEKO_CORE_UI_ALIGN_CENTER,

        // shadow x, y
        1,
        1};

static neko_core_ui_style_sheet_t neko_core_ui_default_style_sheet = neko_default_val();
...
#define NEKO_CORE_UI_APPLY_STYLE(SE)                                               \
    ...
    neko_core_ui_style_list_t* id_styles = NULL;
    neko_core_ui_style_list_t* cls_styles[NEKO_CORE_UI_CLS_SELECTOR_MAX] = neko_default_val();
    ...
    if (desc) {
        ...
        // Class selectors
        for (u32 i = 0; i < NEKO_CORE_UI_CLS_SELECTOR_MAX; ++i) {
            if (desc->classes[i]) {
    ...
    if (*cls_styles) {
        for (u32 i = 0; i < NEKO_CORE_UI_CLS_SELECTOR_MAX; ++i) {
            ...
            for (u32 s = 0; s < neko_dyn_array_size(cls_styles[i]->styles[state]); ++s) {
                neko_core_ui_style_element_t* se = &cls_styles[i]->styles[state][s];
                NEKO_CORE_UI_APPLY_STYLE(se);
            }
        }
    }
    ...
    if (id_styles) {
        for (u32 i = 0; i < neko_dyn_array_size(id_styles->styles[state]); ++i) {
            neko_core_ui_style_element_t* se = &id_styles->styles[state][i];
            NEKO_CORE_UI_APPLY_STYLE(se);
        }
    }

    if (neko_hash_table_exists(ctx->inline_styles, (neko_core_ui_element_type)elementid)) {
        neko_core_ui_inline_style_stack_t* iss = neko_hash_table_getp(ctx->inline_styles, (neko_core_ui_element_type)elementid);
        if (neko_dyn_array_size(iss->styles[state])) {
            ...
            for (u32 i = 0; i < ct; ++i) {
                u32 idx = (ssz - ct + i);
                neko_core_ui_style_element_t* se = &iss->styles[state][idx];
                NEKO_CORE_UI_APPLY_STYLE(se);
            }
        }
    }
...
NEKO_API_DECL neko_core_ui_style_t neko_core_ui_animation_get_blend_style(neko_core_ui_context_t* ctx, neko_core_ui_animation_t* anim, const neko_core_ui_selector_desc_t* desc, s32 elementid) {
    neko_core_ui_style_t ret = neko_default_val();

    s32 focus_state = anim->focus_state;
    s32 hover_state = anim->hover_state;

    neko_core_ui_style_t s0 = neko_core_ui_get_current_element_style(ctx, desc, elementid, anim->start_state);
    neko_core_ui_style_t s1 = neko_core_ui_get_current_element_style(ctx, desc, elementid, anim->end_state);

    neko_core_ui_inline_style_stack_t* iss = NULL;
    if (neko_hash_table_exists(ctx->inline_styles, (neko_core_ui_element_type)elementid)) {
        iss = neko_hash_table_getp(ctx->inline_styles, (neko_core_ui_element_type)elementid);
    }

    if (anim->direction == NEKO_CORE_UI_ANIMATION_DIRECTION_FORWARD) {
        ret = s1;
    } else {
        ret = s0;
    }

    const neko_core_ui_animation_property_list_t* list = NULL;
    if (neko_hash_table_exists(ctx->style_sheet->animations, (neko_core_ui_element_type)elementid)) {
        list = neko_hash_table_getp(ctx->style_sheet->animations, (neko_core_ui_element_type)elementid);
    }

    const neko_core_ui_animation_property_list_t* id_list = NULL;
    const neko_core_ui_animation_property_list_t* cls_list[NEKO_CORE_UI_CLS_SELECTOR_MAX] = neko_default_val();
...
#define NEKO_CORE_UI_BLEND_PROPERTIES(LIST)                                                                                                            \
    ...
    if (list && !neko_dyn_array_empty(list->properties[anim->end_state])) {
        NEKO_CORE_UI_BLEND_PROPERTIES(list->properties[anim->end_state]);
    }

    // Class list
    if (has_class_animations) {
        for (u32 c = 0; c < NEKO_CORE_UI_CLS_SELECTOR_MAX; ++c) {
            if (!cls_list[c]) continue;
            if (!neko_dyn_array_empty(cls_list[c]->properties[anim->end_state])) {
                NEKO_CORE_UI_BLEND_PROPERTIES(cls_list[c]->properties[anim->end_state]);
            }
        }
    }
    ...
    if (id_list && !neko_dyn_array_empty(id_list->properties[anim->end_state])) {
        NEKO_CORE_UI_BLEND_PROPERTIES(id_list->properties[anim->end_state]);
    }

    if (iss) {
        NEKO_CORE_UI_BLEND_PROPERTIES(iss->animations[anim->end_state]);
    }

    return ret;
}

static void _neko_core_ui_animation_get_time(neko_core_ui_context_t* ctx, neko_core_ui_id id, s32 elementid, const neko_core_ui_selector_desc_t* desc, neko_core_ui_inline_style_stack_t* iss,
                                             s32 state, neko_core_ui_animation_t* anim) {
    u32 act = 0, ssz = 0;
    ...
    neko_core_ui_animation_property_list_t* cls_list[NEKO_CORE_UI_CLS_SELECTOR_MAX] = neko_default_val();
    const neko_core_ui_animation_property_list_t* id_list = NULL;
    const neko_core_ui_animation_property_list_t* list = NULL;
    bool has_class_animations = false;
    ...
    neko_core_ui_animation_property_t properties[NEKO_CORE_UI_STYLE_COUNT] = neko_default_val();
    for (u32 i = 0; i < NEKO_CORE_UI_STYLE_COUNT; ++i) {
        properties[i].type = (neko_core_ui_style_element_type)i;
    }

#define GUI_SET_PROPERTY_TIMES(PROP_LIST)                              \
    do {                                                               \
        for (u32 p = 0; p < neko_dyn_array_size((PROP_LIST)); ++p) {   \
            neko_core_ui_animation_property_t* prop = &(PROP_LIST)[p]; \
    ...
    // Element type list
    if (list) {
        neko_dyn_array(neko_core_ui_animation_property_t) props = list->properties[state];
    ...
    if (has_class_animations) {
        for (u32 c = 0; c < NEKO_CORE_UI_CLS_SELECTOR_MAX; ++c) {
            if (!cls_list[c]) continue;
            neko_dyn_array(neko_core_ui_animation_property_t) props = cls_list[c]->properties[state];
            GUI_SET_PROPERTY_TIMES(props);
        }
    }
    ...
    // Id list
    if (id_list) {
        neko_dyn_array(neko_core_ui_animation_property_t) props = id_list->properties[state];
        GUI_SET_PROPERTY_TIMES(props);
    }
    ...
    if (act && iss) {
        for (u32 a = 0; a < act; ++a) {
            u32 idx = ssz - act + a;
            neko_core_ui_animation_property_t* ap = &iss->animations[state][idx];
...
NEKO_API_DECL neko_core_ui_animation_t* neko_core_ui_get_animation(neko_core_ui_context_t* ctx, neko_core_ui_id id, const neko_core_ui_selector_desc_t* desc, s32 elementid) {
    neko_core_ui_animation_t* anim = NULL;

    const b32 valid_eid = (elementid >= 0 && elementid < NEKO_CORE_UI_ELEMENT_COUNT);

    // Construct new animation if necessary to insert
    if (ctx->state_switch_id == id) {
        if (!neko_hash_table_exists(ctx->animations, id)) {
            neko_core_ui_animation_t val = neko_default_val();
...
#define ANIM_GET_TIME(STATE)
        ...
        switch (ctx->switch_state) {
            case NEKO_CORE_UI_ELEMENT_STATE_OFF_FOCUS: {
                if (ctx->hover == id) {
                    anim->direction = NEKO_CORE_UI_ANIMATION_DIRECTION_BACKWARD;
                    anim->start_state = NEKO_CORE_UI_ELEMENT_STATE_HOVER;
                    anim->end_state = NEKO_CORE_UI_ELEMENT_STATE_FOCUS;
                    time_state = NEKO_CORE_UI_ELEMENT_STATE_HOVER;
                    if (valid_eid) _neko_core_ui_animation_get_time(ctx, id, elementid, desc, iss, time_state, anim);
                    anim->time = anim->max;
                } else {
                    anim->direction = NEKO_CORE_UI_ANIMATION_DIRECTION_BACKWARD;
                    anim->start_state = NEKO_CORE_UI_ELEMENT_STATE_DEFAULT;
                    anim->end_state = NEKO_CORE_UI_ELEMENT_STATE_FOCUS;
                    time_state = NEKO_CORE_UI_ELEMENT_STATE_DEFAULT;
                    if (valid_eid) _neko_core_ui_animation_get_time(ctx, id, elementid, desc, iss, time_state, anim);
                    anim->time = anim->max;
                }
            } break;

            case NEKO_CORE_UI_ELEMENT_STATE_ON_FOCUS: {
                anim->direction = NEKO_CORE_UI_ANIMATION_DIRECTION_FORWARD;
                anim->start_state = NEKO_CORE_UI_ELEMENT_STATE_HOVER;
                anim->end_state = NEKO_CORE_UI_ELEMENT_STATE_FOCUS;
                time_state = NEKO_CORE_UI_ELEMENT_STATE_FOCUS;
                if (valid_eid) _neko_core_ui_animation_get_time(ctx, id, elementid, desc, iss, time_state, anim);
                anim->time = 0;
            } break;

            case NEKO_CORE_UI_ELEMENT_STATE_OFF_HOVER: {
                anim->direction = NEKO_CORE_UI_ANIMATION_DIRECTION_BACKWARD;
                anim->start_state = NEKO_CORE_UI_ELEMENT_STATE_DEFAULT;
                anim->end_state = NEKO_CORE_UI_ELEMENT_STATE_HOVER;
                time_state = NEKO_CORE_UI_ELEMENT_STATE_DEFAULT;
                if (valid_eid) _neko_core_ui_animation_get_time(ctx, id, elementid, desc, iss, time_state, anim);
                anim->time = anim->max;
            } break;

            case NEKO_CORE_UI_ELEMENT_STATE_ON_HOVER: {
                anim->direction = NEKO_CORE_UI_ANIMATION_DIRECTION_FORWARD;
                anim->start_state = NEKO_CORE_UI_ELEMENT_STATE_DEFAULT;
                anim->end_state = NEKO_CORE_UI_ELEMENT_STATE_HOVER;
                time_state = NEKO_CORE_UI_ELEMENT_STATE_HOVER;
...
NEKO_API_DECL void neko_core_ui_animation_update(neko_core_ui_context_t* ctx, neko_core_ui_animation_t* anim) {
...
static neko_core_ui_rect_t neko_core_ui_expand_rect(neko_core_ui_rect_t rect, s16 v[4]) { return neko_core_ui_rect(rect.x - v[0], rect.y - v[2], rect.w + v[0] + v[1], rect.h + v[2] + v[3]); }

static neko_core_ui_rect_t neko_core_ui_intersect_rects(neko_core_ui_rect_t r1, neko_core_ui_rect_t r2) {
    ...
    return neko_core_ui_rect((float)x1, (float)y1, (float)x2 - (float)x1, (float)y2 - (float)y1);
}

static s32 neko_core_ui_rect_overlaps_vec2(neko_core_ui_rect_t r, neko_vec2 p) { return p.x >= r.x && p.x < r.x + r.w && p.y >= r.y && p.y < r.y + r.h; }
...
NEKO_API_DECL neko_core_ui_container_t* neko_core_ui_get_top_most_container(neko_core_ui_context_t* ctx, neko_core_ui_split_t* split) {
    ...
    if (split->children[0].type == NEKO_CORE_UI_SPLIT_NODE_CONTAINER) return split->children[0].container;
    if (split->children[1].type == NEKO_CORE_UI_SPLIT_NODE_CONTAINER) return split->children[1].container;
    neko_core_ui_container_t* c0 = neko_core_ui_get_top_most_container(ctx, neko_slot_array_getp(ctx->splits, split->children[0].split));
    neko_core_ui_container_t* c1 = neko_core_ui_get_top_most_container(ctx, neko_slot_array_getp(ctx->splits, split->children[1].split));
...
NEKO_API_DECL void neko_core_ui_bring_split_to_front(neko_core_ui_context_t* ctx, neko_core_ui_split_t* split) {
    ...
    if (!split->parent) {
        ...
        neko_core_ui_id id = neko_core_ui_get_id(ctx, TMP, 256);
        neko_core_ui_container_t* cnt = neko_core_ui_get_container(ctx, TMP);
        // if (cnt) neko_core_ui_bring_to_front(ctx, cnt);
        // cnt->zindex = 0;
    }

    neko_core_ui_split_node_t* c0 = &split->children[0];
    neko_core_ui_split_node_t* c1 = &split->children[1];

    if (c0->type == NEKO_CORE_UI_SPLIT_NODE_CONTAINER) {
        neko_core_ui_bring_to_front(ctx, c0->container);
        // ctx->hover = c0;
    } else {
        neko_core_ui_split_t* s = neko_slot_array_getp(ctx->splits, c0->split);
        neko_core_ui_bring_split_to_front(ctx, s);
    }

    if (c1->type == NEKO_CORE_UI_SPLIT_NODE_CONTAINER) {
        neko_core_ui_bring_to_front(ctx, c1->container);
        // ctx->hover = c1;
    } else {
        neko_core_ui_split_t* s = neko_slot_array_getp(ctx->splits, c1->split);
        neko_core_ui_bring_split_to_front(ctx, s);
    }
}

static void neko_core_ui_update_split(neko_core_ui_context_t* ctx, neko_core_ui_split_t* split) {
    ...
    switch (split->type) {
        case NEKO_CORE_UI_SPLIT_LEFT: {
            if (split->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].type == NEKO_CORE_UI_SPLIT_NODE_SPLIT) {
                // Update split
                neko_core_ui_split_t* s = neko_slot_array_getp(ctx->splits, split->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].split);
                s->rect = neko_core_ui_rect(sr->x + sr->w * ratio, sr->y, sr->w * (1.f - ratio), sr->h);
                neko_core_ui_update_split(ctx, s);
            }

            if (split->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].type == NEKO_CORE_UI_SPLIT_NODE_SPLIT) {
                neko_core_ui_split_t* s = neko_slot_array_getp(ctx->splits, split->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].split);
                s->rect = neko_core_ui_rect(sr->x, sr->y, sr->w * (ratio), sr->h);
                neko_core_ui_update_split(ctx, s);
            }
        } break;
        ...
        case NEKO_CORE_UI_SPLIT_RIGHT: {
            if (split->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].type == NEKO_CORE_UI_SPLIT_NODE_SPLIT) {
                // Update split
                neko_core_ui_split_t* s = neko_slot_array_getp(ctx->splits, split->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].split);
                s->rect = neko_core_ui_rect(sr->x + sr->w * (1.f - ratio), sr->y, sr->w * (ratio), sr->h);
                neko_core_ui_update_split(ctx, s);
            }

            if (split->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].type == NEKO_CORE_UI_SPLIT_NODE_SPLIT) {
                neko_core_ui_split_t* s = neko_slot_array_getp(ctx->splits, split->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].split);
                s->rect = neko_core_ui_rect(sr->x, sr->y, sr->w * (1.f - ratio), sr->h);
                neko_core_ui_update_split(ctx, s);
            }
        ...
        case NEKO_CORE_UI_SPLIT_TOP: {
            if (split->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].type == NEKO_CORE_UI_SPLIT_NODE_SPLIT) {
                // Update split
                neko_core_ui_split_t* s = neko_slot_array_getp(ctx->splits, split->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].split);
                s->rect = neko_core_ui_rect(sr->x, sr->y + sr->h * (ratio), sr->w, sr->h * (1.f - ratio));
                neko_core_ui_update_split(ctx, s);
            }

            if (split->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].type == NEKO_CORE_UI_SPLIT_NODE_SPLIT) {
                neko_core_ui_split_t* s = neko_slot_array_getp(ctx->splits, split->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].split);
                s->rect = neko_core_ui_rect(sr->x, sr->y, sr->w, sr->h * (ratio));
                neko_core_ui_update_split(ctx, s);
            }
        } break;
        ...
        case NEKO_CORE_UI_SPLIT_BOTTOM: {
            if (split->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].type == NEKO_CORE_UI_SPLIT_NODE_SPLIT) {
                // Update split
                neko_core_ui_split_t* s = neko_slot_array_getp(ctx->splits, split->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].split);
                s->rect = neko_core_ui_rect(sr->x, sr->y + sr->h * (1.f - ratio), sr->w, sr->h * (ratio));
                neko_core_ui_update_split(ctx, s);
            }

            if (split->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].type == NEKO_CORE_UI_SPLIT_NODE_SPLIT) {
                neko_core_ui_split_t* s = neko_slot_array_getp(ctx->splits, split->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].split);
                s->rect = neko_core_ui_rect(sr->x, sr->y, sr->w, sr->h * (1.f - ratio));
                neko_core_ui_update_split(ctx, s);
            }
        } break;
    }
...
static neko_core_ui_split_t* neko_core_ui_get_root_split_from_split(neko_core_ui_context_t* ctx, neko_core_ui_split_t* split) {
    if (!split) return NULL;

    // Cache top root level split
    neko_core_ui_split_t* root_split = split && split->parent ? neko_slot_array_getp(ctx->splits, split->parent) : split ? split : NULL;
...
static neko_core_ui_split_t* neko_core_ui_get_root_split(neko_core_ui_context_t* ctx, neko_core_ui_container_t* cnt) {
    neko_core_ui_split_t* split = neko_core_ui_get_split(ctx, cnt);
    if (split)
        return neko_core_ui_get_root_split_from_split(ctx, split);
    else
        return NULL;
}
...
NEKO_API_DECL neko_core_ui_container_t* neko_core_ui_get_root_container_from_split(neko_core_ui_context_t* ctx, neko_core_ui_split_t* split) {
    neko_core_ui_split_t* root = neko_core_ui_get_root_split_from_split(ctx, split);
    neko_core_ui_split_t* s = root;
    neko_core_ui_container_t* c = NULL;
    while (s && !c) {
        if (s->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].type == NEKO_CORE_UI_SPLIT_NODE_SPLIT) {
            s = neko_slot_array_getp(ctx->splits, s->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].split);
        } else if (s->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].type == NEKO_CORE_UI_SPLIT_NODE_CONTAINER) {
            c = s->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].container;
        } else {
            s = NULL;
        }
    }
...
NEKO_API_DECL neko_core_ui_container_t* neko_core_ui_get_root_container(neko_core_ui_context_t* ctx, neko_core_ui_container_t* cnt) {
    neko_core_ui_container_t* parent = neko_core_ui_get_parent(ctx, cnt);
    if (parent->split) {
        neko_core_ui_split_t* root = neko_core_ui_get_root_split(ctx, parent);
        neko_core_ui_split_t* s = root;
        neko_core_ui_container_t* c = NULL;
        while (s && !c) {
            if (s->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].type == NEKO_CORE_UI_SPLIT_NODE_SPLIT) {
...
NEKO_API_DECL neko_core_ui_split_t* neko_core_ui_get_split(neko_core_ui_context_t* ctx, neko_core_ui_container_t* cnt) {
    neko_core_ui_tab_bar_t* tab_bar = neko_core_ui_get_tab_bar(ctx, cnt);
    neko_core_ui_tab_item_t* tab_item = tab_bar ? &tab_bar->items[cnt->tab_item] : NULL;
    neko_core_ui_split_t* split = cnt->split ? neko_slot_array_getp(ctx->splits, cnt->split) : NULL;

    // Look at split if in tab group
    if (!split && tab_bar) {
        for (u32 i = 0; i < tab_bar->size; ++i) {
            if (((neko_core_ui_container_t*)tab_bar->items[i].data)->split) {
                split = neko_slot_array_getp(ctx->splits, ((neko_core_ui_container_t*)tab_bar->items[i].data)->split);
            }
        }
    }
...
static neko_core_ui_command_t* neko_core_ui_push_jump(neko_core_ui_context_t* ctx, neko_core_ui_command_t* dst) {
    neko_core_ui_command_t* cmd;
    cmd = neko_core_ui_push_command(ctx, NEKO_CORE_UI_COMMAND_JUMP, sizeof(neko_core_ui_jumpcommand_t));
    cmd->jump.dst = dst;
    return cmd;
}

static void neko_core_ui_draw_frame(neko_core_ui_context_t* ctx, neko_core_ui_rect_t rect, neko_core_ui_style_t* style) {
    neko_core_ui_draw_rect(ctx, rect, style->colors[NEKO_CORE_UI_COLOR_BACKGROUND]);

    // draw border
    if (style->colors[NEKO_CORE_UI_COLOR_BORDER].a) {
        neko_core_ui_draw_box(ctx, neko_core_ui_expand_rect(rect, (s16*)style->border_width), (s16*)style->border_width, style->colors[NEKO_CORE_UI_COLOR_BORDER]);
    }
}
...
void neko_core_ui_push_inline_style(neko_core_ui_context_t* ctx, neko_core_ui_element_type elementid, neko_core_ui_inline_style_desc_t* desc) {
    ...
    if (!neko_hash_table_exists(ctx->inline_styles, elementid)) {
        neko_core_ui_inline_style_stack_t v = neko_default_val();
        neko_hash_table_insert(ctx->inline_styles, elementid, v);
    }

    neko_core_ui_inline_style_stack_t* iss = neko_hash_table_getp(ctx->inline_styles, elementid);
    neko_assert(iss);
    ...
    if (desc->all.style.data && desc->all.style.size) {
        ...
        u32 ct = desc->all.style.size / sizeof(neko_core_ui_style_element_t);
        style_ct[0] += ct;
        style_ct[1] += ct;
        style_ct[2] += ct;
    ...
    if (desc->all.animation.data && desc->all.animation.size) {
        // Total amount to write for each section
        u32 ct = desc->all.animation.size / sizeof(neko_core_ui_animation_property_t);
        anim_ct[0] += ct;
        anim_ct[1] += ct;
        anim_ct[2] += ct;
...
#define NEKO_CORE_UI_COPY_INLINE_STYLE(TYPE, INDEX)                                         \
    do {                                                                                    \
        ...
        if (desc->TYPE.animation.data && desc->TYPE.animation.size) {                       \
            u32 ct = desc->TYPE.animation.size / sizeof(neko_core_ui_animation_property_t); \
            anim_ct[INDEX] += ct;                                                           \
                                                                                            \
            for (u32 i = 0; i < ct; ++i) {                                                  \
    ...
    NEKO_CORE_UI_COPY_INLINE_STYLE(def, 0);
    NEKO_CORE_UI_COPY_INLINE_STYLE(hover, 1);
    NEKO_CORE_UI_COPY_INLINE_STYLE(focus, 2);
...
static void neko_core_ui_push_layout(neko_core_ui_context_t* ctx, neko_core_ui_rect_t body, neko_vec2 scroll) {
    ...
    layout.body = neko_core_ui_rect(body.x - scroll.x, body.y - scroll.y, body.w, body.h);
    ...
    neko_core_ui_stack_push(ctx->layout_stack, layout);
    neko_core_ui_layout_row(ctx, 1, &width, 0);
}

static void neko_core_ui_pop_layout(neko_core_ui_context_t* ctx) { neko_core_ui_stack_pop(ctx->layout_stack); }

NEKO_API_DECL neko_core_ui_layout_t* neko_core_ui_get_layout(neko_core_ui_context_t* ctx) { return &ctx->layout_stack.items[ctx->layout_stack.idx - 1]; }

static void neko_core_ui_pop_container(neko_core_ui_context_t* ctx) {
    neko_core_ui_container_t* cnt = neko_core_ui_get_current_container(ctx);
    neko_core_ui_layout_t* layout = neko_core_ui_get_layout(ctx);
    cnt->content_size.x = layout->max.x - layout->body.x;
    cnt->content_size.y = layout->max.y - layout->body.y;

    /* pop container, layout and id */
    neko_core_ui_stack_pop(ctx->container_stack);
    neko_core_ui_stack_pop(ctx->layout_stack);
...
static void neko_core_ui_scrollbars(neko_core_ui_context_t* ctx, neko_core_ui_container_t* cnt, neko_core_ui_rect_t* body, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    s32 sz = (s32)ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_SCROLL][0x00].size[0];
    neko_vec2 cs = cnt->content_size;
    cs.x += ctx->style->padding[NEKO_CORE_UI_PADDING_LEFT] * 2;
    cs.y += ctx->style->padding[NEKO_CORE_UI_PADDING_TOP] * 2;
    neko_core_ui_push_clip_rect(ctx, *body);
    ...
    ** used; only the references to `x|y` `w|h` need to be switched */
    neko_core_ui_scrollbar(ctx, cnt, body, cs, x, y, w, h);

    if (~opt & NEKO_CORE_UI_OPT_NOSCROLLHORIZONTAL) {
        neko_core_ui_scrollbar(ctx, cnt, body, cs, y, x, h, w);
    }
    ...
    neko_core_ui_pop_clip_rect(ctx);
}

static void neko_core_ui_push_container_body(neko_core_ui_context_t* ctx, neko_core_ui_container_t* cnt, neko_core_ui_rect_t body, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    if (~opt & NEKO_CORE_UI_OPT_NOSCROLL) {
        neko_core_ui_scrollbars(ctx, cnt, &body, desc, opt);
    }
    s32* padding = ctx->style->padding;
    float l = body.x + padding[NEKO_CORE_UI_PADDING_LEFT];
    float t = body.y + padding[NEKO_CORE_UI_PADDING_TOP];
    float r = body.x + body.w - padding[NEKO_CORE_UI_PADDING_RIGHT];
    float b = body.y + body.h - padding[NEKO_CORE_UI_PADDING_BOTTOM];

    neko_core_ui_rect_t rect = neko_core_ui_rect(l, t, r - l, b - t);
    neko_core_ui_push_layout(ctx, rect, cnt->scroll);
    cnt->body = body;
}

static void neko_core_ui_begin_root_container(neko_core_ui_context_t* ctx, neko_core_ui_container_t* cnt, u64 opt) {
    neko_core_ui_stack_push(ctx->container_stack, cnt);

    /* push container to roots list and push head command */
    neko_core_ui_stack_push(ctx->root_list, cnt);
    cnt->head = neko_core_ui_push_jump(ctx, NULL);

    /* set as hover root if the mouse is overlapping this container and it has a
    ** higher zindex than the current hover root */
    if (neko_core_ui_rect_overlaps_vec2(cnt->rect, ctx->mouse_pos) && (!ctx->next_hover_root || cnt->zindex > ctx->next_hover_root->zindex) && ~opt & NEKO_CORE_UI_OPT_NOHOVER &&
        cnt->flags & NEKO_CORE_UI_WINDOW_FLAGS_VISIBLE) {
    ...
    ** root-container being clipped to the outer */
    neko_core_ui_stack_push(ctx->clip_stack, neko_core_ui_unclipped_rect);
}

static void neko_core_ui_root_container_end(neko_core_ui_context_t* ctx) {
    /* push tail 'goto' jump command and set head 'skip' command. the final steps
    ** on initing these are done in neko_core_ui_end() */
    neko_core_ui_container_t* cnt = neko_core_ui_get_current_container(ctx);
    cnt->tail = neko_core_ui_push_jump(ctx, NULL);
    cnt->head->jump.dst = ctx->command_list.items + ctx->command_list.idx;

    /* pop base clip rect and container */
    neko_core_ui_pop_clip_rect(ctx);
    neko_core_ui_pop_container(ctx);
}
...
NEKO_API_DECL neko_core_ui_style_sheet_t neko_core_ui_style_sheet_create(neko_core_ui_context_t* ctx, neko_core_ui_style_sheet_desc_t* desc) {
    ...
    neko_core_ui_style_sheet_t style_sheet = neko_default_val();

    // Copy all default styles
    NEKO_CORE_UI_COPY_STYLES(style_sheet.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_CONTAINER);
    NEKO_CORE_UI_COPY_STYLES(style_sheet.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_LABEL);
    NEKO_CORE_UI_COPY_STYLES(style_sheet.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_TEXT);
    NEKO_CORE_UI_COPY_STYLES(style_sheet.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_PANEL);
    NEKO_CORE_UI_COPY_STYLES(style_sheet.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_INPUT);
    NEKO_CORE_UI_COPY_STYLES(style_sheet.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_BUTTON);
    NEKO_CORE_UI_COPY_STYLES(style_sheet.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_SCROLL);
    NEKO_CORE_UI_COPY_STYLES(style_sheet.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_IMAGE);
...
#define NEKO_CORE_UI_APPLY_STYLE_ELEMENT(ELEMENT, TYPE)                                                                                                                \
    ...
    if (desc) {
        NEKO_CORE_UI_APPLY_STYLE_ELEMENT(desc->button, NEKO_CORE_UI_ELEMENT_BUTTON);
        NEKO_CORE_UI_APPLY_STYLE_ELEMENT(desc->container, NEKO_CORE_UI_ELEMENT_CONTAINER);
        NEKO_CORE_UI_APPLY_STYLE_ELEMENT(desc->panel, NEKO_CORE_UI_ELEMENT_PANEL);
        NEKO_CORE_UI_APPLY_STYLE_ELEMENT(desc->scroll, NEKO_CORE_UI_ELEMENT_SCROLL);
        NEKO_CORE_UI_APPLY_STYLE_ELEMENT(desc->image, NEKO_CORE_UI_ELEMENT_IMAGE);
        NEKO_CORE_UI_APPLY_STYLE_ELEMENT(desc->label, NEKO_CORE_UI_ELEMENT_LABEL);
        NEKO_CORE_UI_APPLY_STYLE_ELEMENT(desc->text, NEKO_CORE_UI_ELEMENT_TEXT);
    }
...
#define COPY_ANIM_DATA(TYPE, ELEMENT)                                                                             \
    do {                                                                                                          \
        ...
        if (desc->TYPE.all.animation.data) {                                                                      \
            s32 cnt = desc->TYPE.all.animation.size / sizeof(neko_core_ui_animation_property_t);                  \
            if (!neko_hash_table_exists(style_sheet.animations, ELEMENT)) {                                       \
                neko_core_ui_animation_property_list_t v = neko_default_val();                                    \
                neko_hash_table_insert(style_sheet.animations, ELEMENT, v);                                       \
            }                                                                                                     \
            neko_core_ui_animation_property_list_t* list = neko_hash_table_getp(style_sheet.animations, ELEMENT); \
            neko_assert(list);                                                                                    \
    ...
    // Copy animations
    COPY_ANIM_DATA(button, NEKO_CORE_UI_ELEMENT_BUTTON);
    COPY_ANIM_DATA(label, NEKO_CORE_UI_ELEMENT_LABEL);
    COPY_ANIM_DATA(scroll, NEKO_CORE_UI_ELEMENT_SCROLL);
    COPY_ANIM_DATA(image, NEKO_CORE_UI_ELEMENT_IMAGE);
    COPY_ANIM_DATA(panel, NEKO_CORE_UI_ELEMENT_PANEL);
    COPY_ANIM_DATA(text, NEKO_CORE_UI_ELEMENT_TEXT);
    COPY_ANIM_DATA(container, NEKO_CORE_UI_ELEMENT_CONTAINER);
...
NEKO_API_DECL void neko_core_ui_style_sheet_destroy(neko_core_ui_style_sheet_t* ss) {
    ...
    for (neko_hash_table_iter it = neko_hash_table_iter_new(ss->animations); neko_hash_table_iter_valid(ss->animations, it); neko_hash_table_iter_advance(ss->animations, it)) {
        neko_core_ui_animation_property_list_t* list = neko_hash_table_iter_getp(ss->animations, it);
        for (u32 i = 0; i < 3; ++i) {
...
NEKO_API_DECL void neko_core_ui_set_style_sheet(neko_core_ui_context_t* ctx, neko_core_ui_style_sheet_t* style_sheet) {
    ctx->style_sheet = style_sheet ? style_sheet : &neko_core_ui_default_style_sheet;
}

NEKO_API_DECL void neko_core_ui_style_sheet_set_element_styles(neko_core_ui_style_sheet_t* ss, neko_core_ui_element_type element, neko_core_ui_element_state state,
                                                               neko_core_ui_style_element_t* styles, size_t size) {
    const u32 count = size / sizeof(neko_core_ui_style_element_t);
    u32 idx_cnt = 1;
    u32 idx = 0;
    ...
    switch (state) {
        ...
        case NEKO_CORE_UI_ELEMENT_STATE_DEFAULT:
            idx = 0;
            break;
        case NEKO_CORE_UI_ELEMENT_STATE_HOVER:
            idx = 1;
            break;
        case NEKO_CORE_UI_ELEMENT_STATE_FOCUS:
            idx = 2;
            break;
    }

    for (u32 s = idx, c = 0; c < idx_cnt; ++s, ++c) {
        neko_core_ui_style_t* cs = &ss->styles[element][s];
        for (u32 i = 0; i < count; ++i) {
            neko_core_ui_style_element_t* se = &styles[i];
            ...
            switch (se->type) {
                ...
                // Padding
                case NEKO_CORE_UI_STYLE_PADDING: {
                    cs->padding[NEKO_CORE_UI_PADDING_LEFT] = (s32)se->value;
                    cs->padding[NEKO_CORE_UI_PADDING_TOP] = (s32)se->value;
                    cs->padding[NEKO_CORE_UI_PADDING_RIGHT] = (s32)se->value;
                    cs->padding[NEKO_CORE_UI_PADDING_BOTTOM] = (s32)se->value;
                }
                case NEKO_CORE_UI_STYLE_PADDING_LEFT:
                    cs->padding[NEKO_CORE_UI_PADDING_LEFT] = (s32)se->value;
                    break;
                ...
                case NEKO_CORE_UI_STYLE_MARGIN: {
                    cs->margin[NEKO_CORE_UI_MARGIN_LEFT] = (s32)se->value;
                    cs->margin[NEKO_CORE_UI_MARGIN_TOP] = (s32)se->value;
                    cs->margin[NEKO_CORE_UI_MARGIN_RIGHT] = (s32)se->value;
                    cs->margin[NEKO_CORE_UI_MARGIN_BOTTOM] = (s32)se->value;
                } break;
                ...
                // Colors
                case NEKO_CORE_UI_STYLE_COLOR_BACKGROUND:
                    cs->colors[NEKO_CORE_UI_COLOR_BACKGROUND] = se->color;
                    break;
                case NEKO_CORE_UI_STYLE_COLOR_BORDER:
                    cs->colors[NEKO_CORE_UI_COLOR_BORDER] = se->color;
                    break;
                case NEKO_CORE_UI_STYLE_COLOR_SHADOW:
                    cs->colors[NEKO_CORE_UI_COLOR_SHADOW] = se->color;
                    break;
                case NEKO_CORE_UI_STYLE_COLOR_CONTENT:
                    cs->colors[NEKO_CORE_UI_COLOR_CONTENT] = se->color;
                    break;
                case NEKO_CORE_UI_STYLE_COLOR_CONTENT_BACKGROUND:
                    cs->colors[NEKO_CORE_UI_COLOR_CONTENT_BACKGROUND] = se->color;
                    break;
                case NEKO_CORE_UI_STYLE_COLOR_CONTENT_BORDER:
                    cs->colors[NEKO_CORE_UI_COLOR_CONTENT_BORDER] = se->color;
                    break;
                case NEKO_CORE_UI_STYLE_COLOR_CONTENT_SHADOW:
                    cs->colors[NEKO_CORE_UI_COLOR_CONTENT_SHADOW] = se->color;
                    break;
...
NEKO_API_DECL void neko_core_ui_set_element_style(neko_core_ui_context_t* ctx, neko_core_ui_element_type element, neko_core_ui_element_state state, neko_core_ui_style_element_t* style, size_t size) {
    ...
    for (u32 s = idx, c = 0; c < idx_cnt; ++s, ++c) {
        neko_core_ui_style_t* cs = &ctx->style_sheet->styles[element][s];
        for (u32 i = 0; i < count; ++i) {
            neko_core_ui_style_element_t* se = &style[i];
...
NEKO_API_DECL neko_core_ui_container_t* neko_core_ui_get_container_ex(neko_core_ui_context_t* ctx, neko_core_ui_id id, u64 opt) {
    ...
    s32 idx = neko_core_ui_pool_get(ctx, ctx->container_pool, NEKO_CORE_UI_CONTAINERPOOL_SIZE, id);

    if (idx >= 0) {
        if (ctx->containers[idx].open || ~opt & NEKO_CORE_UI_OPT_CLOSED) {
            neko_core_ui_pool_update(ctx, ctx->container_pool, idx);
        }
        return &ctx->containers[idx];
    }

    if (opt & NEKO_CORE_UI_OPT_CLOSED) {
        return NULL;
    }

    /* container not found in pool: init new container */
    idx = neko_core_ui_pool_init(ctx, ctx->container_pool, NEKO_CORE_UI_CONTAINERPOOL_SIZE, id);
    cnt = &ctx->containers[idx];
    memset(cnt, 0, sizeof(*cnt));
    cnt->open = 1;
    cnt->id = id;
    cnt->flags |= NEKO_CORE_UI_WINDOW_FLAGS_VISIBLE | NEKO_CORE_UI_WINDOW_FLAGS_FIRST_INIT;
    neko_core_ui_bring_to_front(ctx, cnt);
...
static s32 neko_core_ui_text_width(neko_asset_ascii_font_t* font, const char* text, s32 len) {
    neko_vec2 td = neko_asset_ascii_font_text_dimensions(font, text, len);
    return (s32)td.x;
}

static s32 neko_core_ui_text_height(neko_asset_ascii_font_t* font, const char* text, s32 len) {
...
static s32 neko_core_ui_font_height(neko_asset_ascii_font_t* font) { return (s32)neko_asset_ascii_font_max_height(font); }

static neko_vec2 neko_core_ui_text_dimensions(neko_asset_ascii_font_t* font, const char* text, s32 len) {
    neko_vec2 td = neko_asset_ascii_font_text_dimensions(font, text, len);
    return td;
}
...
NEKO_API_DECL void neko_core_ui_dock_ex(neko_core_ui_context_t* ctx, const char* dst, const char* src, s32 split_type, float ratio) {
    neko_core_ui_hints_t hints = neko_default_val();
    hints.framebuffer_size = neko_platform_framebuffer_sizev(ctx->window_hndl);
    hints.viewport = neko_core_ui_rect(0.f, 0.f, 0.f, 0.f);
    u32 f = ctx->frame;
    if (!f) neko_core_ui_begin(ctx, &hints);
    neko_core_ui_container_t* dst_cnt = neko_core_ui_get_container(ctx, dst);
    neko_core_ui_container_t* src_cnt = neko_core_ui_get_container(ctx, src);
    neko_core_ui_dock_ex_cnt(ctx, dst_cnt, src_cnt, split_type, ratio);
    if (f != ctx->frame) neko_core_ui_end(ctx, true);
}

NEKO_API_DECL void neko_core_ui_undock_ex(neko_core_ui_context_t* ctx, const char* name) {
    neko_core_ui_container_t* cnt = neko_core_ui_get_container(ctx, name);
    neko_core_ui_undock_ex_cnt(ctx, cnt);
}

void neko_core_ui_set_split(neko_core_ui_context_t* ctx, neko_core_ui_container_t* cnt, u32 id) {
    cnt->split = id;
    neko_core_ui_tab_bar_t* tb = neko_core_ui_get_tab_bar(ctx, cnt);
    if (tb) {
        for (u32 i = 0; i < tb->size; ++i) {
            ((neko_core_ui_container_t*)tb->items[i].data)->split = id;
        }
    }
}

NEKO_API_DECL neko_core_ui_container_t* neko_core_ui_get_parent(neko_core_ui_context_t* ctx, neko_core_ui_container_t* cnt) { return (cnt->parent ? cnt->parent : cnt); }

NEKO_API_DECL void neko_core_ui_dock_ex_cnt(neko_core_ui_context_t* ctx, neko_core_ui_container_t* child, neko_core_ui_container_t* parent, s32 split_type, float ratio) {
    // Get top-level parent windows
    parent = neko_core_ui_get_parent(ctx, parent);
    child = neko_core_ui_get_parent(ctx, child);

    if (!child || !parent) {
        return;
    }

    if (split_type == NEKO_CORE_UI_SPLIT_TAB) {
        // If the parent window has a tab bar, then need to get that tab bar item and add it
        if (parent->tab_bar) {
            neko_println("add to tab bar");

            neko_core_ui_tab_bar_t* tab_bar = neko_slot_array_getp(ctx->tab_bars, parent->tab_bar);
            neko_assert(tab_bar);

            // Set all tab bar children to this as well, if has children, then release previous tab bar
            if (child->tab_bar) {
                u32 tbid = child->tab_bar;
                neko_core_ui_tab_bar_t* ctb = neko_slot_array_getp(ctx->tab_bars, child->tab_bar);
                for (u32 i = 0; i < ctb->size; ++i) {
                    neko_core_ui_tab_item_t* cti = &tab_bar->items[tab_bar->size];
                    neko_core_ui_container_t* c = (neko_core_ui_container_t*)ctb->items[i].data;
                ...
                neko_core_ui_tab_item_t* tab_item = &tab_bar->items[tab_bar->size];
        ...
        // Otherwise, create new tab bar
            ...
            // Create tab bar
            neko_core_ui_tab_bar_t tb = neko_default_val();
            u32 hndl = neko_slot_array_insert(ctx->tab_bars, tb);
            neko_core_ui_tab_bar_t* tab_bar = neko_slot_array_getp(ctx->tab_bars, hndl);
            neko_assert(tab_bar);

            // Create tab items
            neko_core_ui_tab_item_t* parent_tab_item = &tab_bar->items[tab_bar->size];
            ...
            if (child->tab_bar) {
                ...
                neko_core_ui_tab_item_t* child_tab_item = &tab_bar->items[tab_bar->size];
        ...
        neko_core_ui_split_t* root_split = neko_core_ui_get_root_split(ctx, parent);
        if (root_split) {
            neko_core_ui_update_split(ctx, root_split);
            neko_core_ui_bring_split_to_front(ctx, root_split);
        }
    } else {
        // Cache previous root splits
        neko_core_ui_split_t* ps = neko_core_ui_get_root_split(ctx, parent);
        neko_core_ui_split_t* cs = neko_core_ui_get_root_split(ctx, child);

        neko_core_ui_tab_bar_t* tab_bar = neko_core_ui_get_tab_bar(ctx, parent);

        neko_core_ui_split_t split = neko_default_val();
        split.type = (neko_core_ui_split_type)split_type;
        split.ratio = ratio;
        neko_core_ui_split_node_t c0 = neko_default_val();
        c0.type = NEKO_CORE_UI_SPLIT_NODE_CONTAINER;
        c0.container = child;
        neko_core_ui_split_node_t c1 = neko_default_val();
        c1.type = NEKO_CORE_UI_SPLIT_NODE_CONTAINER;
        c1.container = parent;
        split.children[NEKO_CORE_UI_SPLIT_NODE_CHILD] = c0;
        split.children[NEKO_CORE_UI_SPLIT_NODE_PARENT] = c1;
        ...
        neko_core_ui_split_t* sp = neko_slot_array_getp(ctx->splits, hndl);
        sp->id = hndl;
        ...
        // Child has split
            ...
            sp->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].type = NEKO_CORE_UI_SPLIT_NODE_SPLIT;
            sp->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].split = cs->id;
        ...
        // Parent has split
            ...
            // Fix up references
            if (ps->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].container == parent) {
                ps->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].type = NEKO_CORE_UI_SPLIT_NODE_SPLIT;
                ps->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].split = hndl;
            } else {
                ps->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].type = NEKO_CORE_UI_SPLIT_NODE_SPLIT;
                ps->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].split = hndl;
            }
        ...
        // Both have splits
            ...
            if (ps->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].container == parent) {
                ps->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].type = NEKO_CORE_UI_SPLIT_NODE_SPLIT;
                ps->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].split = hndl;
            } else {
                ps->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].type = NEKO_CORE_UI_SPLIT_NODE_SPLIT;
...
NEKO_API_DECL void neko_core_ui_undock_ex_cnt(neko_core_ui_context_t* ctx, neko_core_ui_container_t* cnt) {
    ...
    neko_core_ui_split_t* root_split = NULL;

    // Get parent split of this owning split
    neko_core_ui_split_t* ps = split && split->parent ? neko_slot_array_getp(ctx->splits, split->parent) : NULL;
    ...
    if (cnt->tab_bar) {
        ...
        if (parent->split) {
            ...
            neko_core_ui_tab_bar_t* tab_bar = neko_slot_array_getp(ctx->tab_bars, cnt->tab_bar);
            neko_core_ui_tab_item_t* tab_item = &tab_bar->items[cnt->tab_item];
            ...
            if (tab_bar->size > 2) {
                ...
                for (u32 i = idx; i < tab_bar->size; ++i) {
                    neko_core_ui_tab_item_swap(ctx, (neko_core_ui_container_t*)tab_bar->items[i].data, +1);
                }

                // Swap windows as well
                ((neko_core_ui_container_t*)(tab_item->data))->tab_item = tab_item->idx;
                ...
                if (parent == cnt) {
                    ...
                    for (u32 i = 0; i < tab_bar->size; ++i) {
                        neko_core_ui_container_t* c = (neko_core_ui_container_t*)tab_bar->items[i].data;
                        c->parent = (neko_core_ui_container_t*)tab_bar->items[tab_bar->focus].data;
                        tab_bar->items[i].idx = i;
                        tab_bar->items[i].zindex = i;
                    }

                    neko_core_ui_container_t* fcnt = (neko_core_ui_container_t*)tab_bar->items[tab_bar->focus].data;
                    fcnt->split = parent->split;
                    fcnt->flags |= NEKO_CORE_UI_WINDOW_FLAGS_VISIBLE;

                    // Fix up split reference
                    split = neko_slot_array_getp(ctx->splits, fcnt->split);
                    if (split->children[0].type == NEKO_CORE_UI_SPLIT_NODE_CONTAINER && split->children[0].container == cnt) {
            ...
            // Destroy tab
                ...
                for (u32 i = 0; i < tab_bar->size; ++i) {
                    neko_core_ui_container_t* fcnt = (neko_core_ui_container_t*)tab_bar->items[i].data;
                ...
                if (parent == cnt) {
                    ...
                    neko_assert((neko_core_ui_container_t*)tab_bar->items[tab_bar->focus].data != cnt);
                    ...
                    // Fix up split reference
                    split = neko_slot_array_getp(ctx->splits, fcnt->split);
                    if (split->children[0].type == NEKO_CORE_UI_SPLIT_NODE_CONTAINER && split->children[0].container == parent) {
                        split->children[0].container = fcnt;
                    } else {
                        split->children[1].container = fcnt;
                    }
        ...
        if (ps) {
            u32 node_id = ps->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].split == split->id ? NEKO_CORE_UI_SPLIT_NODE_CHILD : NEKO_CORE_UI_SPLIT_NODE_PARENT;
            neko_core_ui_split_node_t* fix_node = &ps->children[node_id];
            *fix_node = *remain_node;
            switch (fix_node->type) {
                case NEKO_CORE_UI_SPLIT_NODE_CONTAINER: {
                    neko_core_ui_container_t* remcnt = fix_node->container;
                    remcnt->split = ps->id;
                } break;

                case NEKO_CORE_UI_SPLIT_NODE_SPLIT: {
                    neko_core_ui_split_t* remsplit = neko_slot_array_getp(ctx->splits, fix_node->split);
                    remsplit->parent = ps->id;
                } break;
            }

            root_split = neko_core_ui_get_root_split_from_split(ctx, ps);
        }
        // Otherwise, we were root dock and have to treat that case for remaining nodes
        else {
            switch (remain_node->type) {
                case NEKO_CORE_UI_SPLIT_NODE_CONTAINER: {
                    neko_core_ui_container_t* remcnt = remain_node->container;
                    remcnt->rect = split->rect;
                    remcnt->split = 0x00;
                    root_split = neko_core_ui_get_root_split(ctx, remcnt);
                } break;

                case NEKO_CORE_UI_SPLIT_NODE_SPLIT: {
                    neko_core_ui_split_t* remsplit = neko_slot_array_getp(ctx->splits, remain_node->split);
                    remsplit->rect = split->rect;
                    remsplit->parent = 0x00;
                    root_split = neko_core_ui_get_root_split_from_split(ctx, remsplit);
                } break;
            }
        }
        ...
        // Update
        if (root_split) neko_core_ui_update_split(ctx, root_split);
        if (root_split) neko_core_ui_bring_split_to_front(ctx, root_split);
        neko_core_ui_bring_to_front(ctx, cnt);
    }
}
...
static void neko_core_ui_init_default_styles(neko_core_ui_context_t* ctx) {
    // Set up main default style
    neko_core_ui_default_style.font = neko_idraw_default_font();
    neko_core_ui_default_style.size[0] = 68.f;
    neko_core_ui_default_style.size[1] = 18.f;
    neko_core_ui_default_style.spacing = 2;
    neko_core_ui_default_style.indent = 10;
    neko_core_ui_default_style.title_height = 20;
    neko_core_ui_default_style.scrollbar_size = 5;
    neko_core_ui_default_style.thumb_size = 5;

    // Initialize all default styles
    NEKO_CORE_UI_COPY_STYLE(neko_core_ui_default_container_style, neko_core_ui_default_style);
    NEKO_CORE_UI_COPY_STYLE(neko_core_ui_default_button_style, neko_core_ui_default_style);
    NEKO_CORE_UI_COPY_STYLE(neko_core_ui_default_text_style, neko_core_ui_default_style);
    NEKO_CORE_UI_COPY_STYLE(neko_core_ui_default_label_style, neko_core_ui_default_style);
    NEKO_CORE_UI_COPY_STYLE(neko_core_ui_default_panel_style, neko_core_ui_default_style);
    NEKO_CORE_UI_COPY_STYLE(neko_core_ui_default_input_style, neko_core_ui_default_style);
    NEKO_CORE_UI_COPY_STYLE(neko_core_ui_default_scroll_style, neko_core_ui_default_style);
    NEKO_CORE_UI_COPY_STYLE(neko_core_ui_default_image_style, neko_core_ui_default_style);

    neko_core_ui_style_t* style = NULL;

    // button
    for (u32 i = 0; i < 3; ++i) {
        style = &neko_core_ui_default_button_style[i];
        style->justify_content = NEKO_CORE_UI_JUSTIFY_CENTER;
    }
    neko_core_ui_default_button_style[NEKO_CORE_UI_ELEMENT_STATE_DEFAULT].colors[NEKO_CORE_UI_COLOR_BACKGROUND] = neko_color(35, 35, 35, 255);
    neko_core_ui_default_button_style[NEKO_CORE_UI_ELEMENT_STATE_HOVER].colors[NEKO_CORE_UI_COLOR_BACKGROUND] = neko_color(40, 40, 40, 255);
    neko_core_ui_default_button_style[NEKO_CORE_UI_ELEMENT_STATE_FOCUS].colors[NEKO_CORE_UI_COLOR_BACKGROUND] = neko_color(0, 214, 121, 255);
    ...
    // input
    for (u32 i = 0; i < 3; ++i) {
        style = &neko_core_ui_default_input_style[i];
        style->colors[NEKO_CORE_UI_COLOR_BACKGROUND] = neko_color(20, 20, 20, 255);
        style->size[1] = 19;
    }

    // text
    for (u32 i = 0; i < 3; ++i) {
        style = &neko_core_ui_default_text_style[i];
        style->colors[NEKO_CORE_UI_COLOR_BACKGROUND] = neko_color(0, 0, 0, 0);
        style->colors[NEKO_CORE_UI_COLOR_CONTENT] = NEKO_COLOR_WHITE;
    }

    // label
    for (u32 i = 0; i < 3; ++i) {
        style = &neko_core_ui_default_label_style[i];
        style->colors[NEKO_CORE_UI_COLOR_BACKGROUND] = neko_color(0, 0, 0, 0);
        style->colors[NEKO_CORE_UI_COLOR_CONTENT] = NEKO_COLOR_WHITE;
        style->size[1] = 19;
    }
    ...
    // scroll
    for (u32 i = 0; i < 3; ++i) {
        style = &neko_core_ui_default_scroll_style[i];
        style->size[0] = 10;
        style->padding[NEKO_CORE_UI_PADDING_RIGHT] = 4;
    }

    style = &neko_core_ui_default_scroll_style[NEKO_CORE_UI_ELEMENT_STATE_DEFAULT];
    style->colors[NEKO_CORE_UI_COLOR_BACKGROUND] = neko_color(22, 22, 22, 255);
...
#define NEKO_CORE_UI_COPY(DST, SRC) \
    ...
    NEKO_CORE_UI_COPY(neko_core_ui_default_style_sheet.styles[NEKO_CORE_UI_ELEMENT_LABEL], neko_core_ui_default_label_style);
    NEKO_CORE_UI_COPY(neko_core_ui_default_style_sheet.styles[NEKO_CORE_UI_ELEMENT_TEXT], neko_core_ui_default_text_style);
    NEKO_CORE_UI_COPY(neko_core_ui_default_style_sheet.styles[NEKO_CORE_UI_ELEMENT_PANEL], neko_core_ui_default_panel_style);
    NEKO_CORE_UI_COPY(neko_core_ui_default_style_sheet.styles[NEKO_CORE_UI_ELEMENT_INPUT], neko_core_ui_default_input_style);
    NEKO_CORE_UI_COPY(neko_core_ui_default_style_sheet.styles[NEKO_CORE_UI_ELEMENT_BUTTON], neko_core_ui_default_button_style);
    NEKO_CORE_UI_COPY(neko_core_ui_default_style_sheet.styles[NEKO_CORE_UI_ELEMENT_SCROLL], neko_core_ui_default_scroll_style);
    NEKO_CORE_UI_COPY(neko_core_ui_default_style_sheet.styles[NEKO_CORE_UI_ELEMENT_IMAGE], neko_core_ui_default_image_style);

    ctx->style_sheet = &neko_core_ui_default_style_sheet;
    ctx->style = &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_CONTAINER][0x00];
}
...
NEKO_API_DECL neko_core_ui_context_t neko_core_ui_new(u32 window_hndl) {
    neko_core_ui_context_t ctx = neko_default_val();
    neko_core_ui_init(&ctx, window_hndl);
    return ctx;
}
...
NEKO_API_DECL void neko_core_ui_init(neko_core_ui_context_t* ctx, u32 window_hndl) {
    ...
    neko_core_ui_init_default_styles(ctx);
    ctx->window_hndl = window_hndl;
    ctx->last_zindex = 1000;
    neko_slot_array_reserve(ctx->splits, NEKO_CORE_UI_NEKO_CORE_UI_SPLIT_SIZE);
    neko_core_ui_split_t split = neko_default_val();
    neko_slot_array_insert(ctx->splits, split);  // First item is set for 0x00 invalid
    neko_slot_array_reserve(ctx->tab_bars, NEKO_CORE_UI_NEKO_CORE_UI_TAB_SIZE);
    neko_core_ui_tab_bar_t tb = neko_default_val();
    neko_slot_array_insert(ctx->tab_bars, tb);

    button_map[NEKO_MOUSE_LBUTTON & 0xff] = NEKO_CORE_UI_MOUSE_LEFT;
    button_map[NEKO_MOUSE_RBUTTON & 0xff] = NEKO_CORE_UI_MOUSE_RIGHT;
    button_map[NEKO_MOUSE_MBUTTON & 0xff] = NEKO_CORE_UI_MOUSE_MIDDLE;

    key_map[NEKO_KEYCODE_LEFT_SHIFT & 0xff] = NEKO_CORE_UI_KEY_SHIFT;
    key_map[NEKO_KEYCODE_RIGHT_SHIFT & 0xff] = NEKO_CORE_UI_KEY_SHIFT;
    key_map[NEKO_KEYCODE_LEFT_CONTROL & 0xff] = NEKO_CORE_UI_KEY_CTRL;
    key_map[NEKO_KEYCODE_RIGHT_CONTROL & 0xff] = NEKO_CORE_UI_KEY_CTRL;
    key_map[NEKO_KEYCODE_LEFT_ALT & 0xff] = NEKO_CORE_UI_KEY_ALT;
    key_map[NEKO_KEYCODE_RIGHT_ALT & 0xff] = NEKO_CORE_UI_KEY_ALT;
    key_map[NEKO_KEYCODE_ENTER & 0xff] = NEKO_CORE_UI_KEY_RETURN;
    key_map[NEKO_KEYCODE_BACKSPACE & 0xff] = NEKO_CORE_UI_KEY_BACKSPACE;
}
...
NEKO_API_DECL void neko_core_ui_init_font_stash(neko_core_ui_context_t* ctx, neko_core_ui_font_stash_desc_t* desc) {
    ...
    u32 ct = sizeof(neko_core_ui_font_desc_t) / desc->size;
    for (u32 i = 0; i < ct; ++i) {
...
NEKO_API_DECL neko_core_ui_context_t neko_core_ui_context_new(u32 window_hndl) {
    neko_core_ui_context_t gui = neko_default_val();
    neko_core_ui_init(&gui, window_hndl);
    return gui;
}
...
*/
    ...
    for (neko_hash_table_iter it = neko_hash_table_iter_new(ctx->inline_styles); neko_hash_table_iter_valid(ctx->inline_styles, it); neko_hash_table_iter_advance(ctx->inline_styles, it)) {
        neko_core_ui_inline_style_stack_t* stack = neko_hash_table_iter_getp(ctx->inline_styles, it);
        for (u32 i = 0; i < 3; ++i) {
...
static void neko_core_ui_draw_splits(neko_core_ui_context_t* ctx, neko_core_ui_split_t* split) {
    if (!split) return;

    neko_core_ui_split_t* root_split = neko_core_ui_get_root_split_from_split(ctx, split);

    // Draw split
    const neko_core_ui_rect_t* sr = &split->rect;
    neko_vec2 hd = neko_v2(sr->w * 0.5f, sr->h * 0.5f);
    neko_core_ui_rect_t r = neko_default_val();
    neko_color_t c = neko_color(0, 0, 0, 0);
    const float ratio = split->ratio;
    neko_core_ui_container_t* top = neko_core_ui_get_top_most_container(ctx, root_split);
    ...
    bool can_draw = true;
    for (u32 i = 0; i < 2; ++i) {
        if (split->children[i].type == NEKO_CORE_UI_SPLIT_NODE_CONTAINER && can_draw) {
            neko_core_ui_container_t* cnt = split->children[i].container;

            // Don't draw split if this container belongs to a editor_dockspace
            if (cnt->opt & NEKO_CORE_UI_OPT_DOCKSPACE) {
                can_draw = false;
                continue;
            }
            ...
            switch (split->type) {
                case NEKO_CORE_UI_SPLIT_LEFT: {
                    r = neko_core_ui_rect(sr->x + sr->w * ratio - NEKO_CORE_UI_SPLIT_SIZE * 0.5f, sr->y + NEKO_CORE_UI_SPLIT_SIZE, NEKO_CORE_UI_SPLIT_SIZE, sr->h - 2.f * NEKO_CORE_UI_SPLIT_SIZE);
                } break;
                ...
                case NEKO_CORE_UI_SPLIT_TOP: {
                    r = neko_core_ui_rect(sr->x + NEKO_CORE_UI_SPLIT_SIZE, sr->y + sr->h * (ratio)-NEKO_CORE_UI_SPLIT_SIZE * 0.5f, sr->w - 2.f * NEKO_CORE_UI_SPLIT_SIZE, NEKO_CORE_UI_SPLIT_SIZE);
                } break;
            ...
            s16 exp[4] = {1, 1, 1, 1};
            neko_core_ui_rect_t expand = neko_core_ui_expand_rect(r, exp);
            bool hover = !valid_hover && !ctx->focus && !ctx->prev_dockable_root && neko_core_ui_rect_overlaps_vec2(expand, ctx->mouse_pos) && !ctx->lock_hover_id;
            if (hover) ctx->next_hover_split = split;
            if (hover && ctx->mouse_down == NEKO_CORE_UI_MOUSE_LEFT) {
            ...
            if (active && valid) {
                ...
                req.type = NEKO_CORE_UI_SPLIT_RATIO;
                req.split = split;
                neko_dyn_array_push(ctx->requests, req);
            }

            if ((hover && (ctx->focus_split == split)) || (hover && !ctx->focus_split) || active) {
                neko_color_t bc = ctx->focus_split == split ? ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_BUTTON][NEKO_CORE_UI_ELEMENT_STATE_FOCUS].colors[NEKO_CORE_UI_COLOR_BACKGROUND]
                                                            : ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_BUTTON][NEKO_CORE_UI_ELEMENT_STATE_HOVER].colors[NEKO_CORE_UI_COLOR_BACKGROUND];
                neko_core_ui_draw_rect(ctx, r, bc);
                can_draw = false;
            }
        } else if (split->children[i].type == NEKO_CORE_UI_SPLIT_NODE_SPLIT) {
            if (can_draw) {
                switch (split->type) {
                    case NEKO_CORE_UI_SPLIT_LEFT: {
                        r = neko_core_ui_rect(sr->x + sr->w * ratio - NEKO_CORE_UI_SPLIT_SIZE * 0.5f, sr->y + NEKO_CORE_UI_SPLIT_SIZE, NEKO_CORE_UI_SPLIT_SIZE, sr->h - 2.f * NEKO_CORE_UI_SPLIT_SIZE);
                ...
                s16 exp[] = {1, 1, 1, 1};
                neko_core_ui_rect_t expand = neko_core_ui_expand_rect(r, exp);
                bool hover = !valid_hover && !ctx->focus && !ctx->prev_dockable_root && neko_core_ui_rect_overlaps_vec2(expand, ctx->mouse_pos);
                if (hover) ctx->next_hover_split = split;
                if (hover && ctx->mouse_down == NEKO_CORE_UI_MOUSE_LEFT) {
                    if (!ctx->focus_split) ctx->focus_split = split;
                }
            ...
            neko_core_ui_split_t* child = neko_slot_array_getp(ctx->splits, split->children[i].split);
            neko_core_ui_draw_splits(ctx, child);
        }
    }
    if (ctx->focus_split == split && ctx->mouse_down != NEKO_CORE_UI_MOUSE_LEFT) {
        ctx->focus_split = NULL;
    }
}

static void neko_core_ui_get_split_lowest_zindex(neko_core_ui_context_t* ctx, neko_core_ui_split_t* split, s32* index) {
    if (!split) return;

    if (split->children[0].type == NEKO_CORE_UI_SPLIT_NODE_CONTAINER && split->children[0].container->zindex < *index) {
        *index = split->children[0].container->zindex;
    }
    if (split->children[0].type == NEKO_CORE_UI_SPLIT_NODE_CONTAINER && split->children[0].container->tab_bar) {
        neko_core_ui_tab_bar_t* tab_bar = neko_slot_array_getp(ctx->tab_bars, split->children[0].container->tab_bar);
        for (u32 i = 0; i < tab_bar->size; ++i) {
            if (((neko_core_ui_container_t*)tab_bar->items[i].data)->zindex < *index) *index = ((neko_core_ui_container_t*)tab_bar->items[i].data)->zindex;
        }
    }

    if (split->children[1].type == NEKO_CORE_UI_SPLIT_NODE_CONTAINER && split->children[1].container->zindex < *index) {
        *index = split->children[1].container->zindex;
    }
    if (split->children[1].type == NEKO_CORE_UI_SPLIT_NODE_CONTAINER && split->children[1].container->tab_bar) {
        neko_core_ui_tab_bar_t* tab_bar = neko_slot_array_getp(ctx->tab_bars, split->children[1].container->tab_bar);
        for (u32 i = 0; i < tab_bar->size; ++i) {
            if (((neko_core_ui_container_t*)tab_bar->items[i].data)->zindex < *index) *index = ((neko_core_ui_container_t*)tab_bar->items[i].data)->zindex;
        }
    }

    if (split->children[0].type == NEKO_CORE_UI_SPLIT_NODE_SPLIT) {
        neko_core_ui_get_split_lowest_zindex(ctx, neko_slot_array_getp(ctx->splits, split->children[0].split), index);
    }

    if (split->children[1].type == NEKO_CORE_UI_SPLIT_NODE_SPLIT) {
        neko_core_ui_get_split_lowest_zindex(ctx, neko_slot_array_getp(ctx->splits, split->children[1].split), index);
    }
}

NEKO_API_DECL void neko_core_ui_begin(neko_core_ui_context_t* ctx, const neko_core_ui_hints_t* hints) {
    neko_core_ui_hints_t default_hints = neko_default_val();
    default_hints.framebuffer_size = neko_platform_framebuffer_sizev(ctx->window_hndl);
    default_hints.viewport = neko_core_ui_rect(0.f, 0.f, default_hints.framebuffer_size.x, default_hints.framebuffer_size.y);
    neko_core_ui_hints_t hint = hints ? *hints : default_hints;

    // Set up viewport
    neko_core_ui_rect_t vp = hint.viewport;
    if (~hint.flags & NEKO_CORE_UI_HINT_FLAG_NO_INVERT_Y) {
    ...
    if (hint.flags & NEKO_CORE_UI_HINT_FLAG_NO_SCALE_BIAS_MOUSE) {
    ...
    while (neko_platform_poll_events(&evt, false)) {
        switch (evt.type) {
            case NEKO_PLATFORM_EVENT_MOUSE: {
                switch (evt.mouse.action) {
                    ...
                    case NEKO_PLATFORM_MOUSE_WHEEL: {
                        neko_core_ui_input_scroll(ctx, (s32)(-evt.mouse.wheel.x * 30.f), (s32)(-evt.mouse.wheel.y * 30.f));
                    } break;

                    case NEKO_PLATFORM_MOUSE_BUTTON_DOWN:
                    case NEKO_PLATFORM_MOUSE_BUTTON_PRESSED: {
                        s32 code = 1 << evt.mouse.button;
                        neko_core_ui_input_mousedown(ctx, (s32)mouse_pos.x, (s32)mouse_pos.y, code);
                    } break;

                    case NEKO_PLATFORM_MOUSE_BUTTON_RELEASED: {
                        s32 code = 1 << evt.mouse.button;
                        neko_core_ui_input_mouseup(ctx, (s32)mouse_pos.x, (s32)mouse_pos.y, code);
                    } break;
            ...
            case NEKO_PLATFORM_EVENT_TEXT: {
                ...
                neko_core_ui_input_text(ctx, txt);
            } break;
            ...
            case NEKO_PLATFORM_EVENT_KEY: {
                switch (evt.key.action) {
                    ...
                    case NEKO_PLATFORM_KEY_PRESSED: {
                        neko_core_ui_input_keydown(ctx, key_map[evt.key.keycode & 511]);
                    } break;

                    case NEKO_PLATFORM_KEY_RELEASED: {
                        neko_core_ui_input_keyup(ctx, key_map[evt.key.keycode & 511]);
                    } break;
    ...
    ctx->viewport = neko_core_ui_rect(hint.viewport.x, hint.viewport.y, hint.viewport.w, hint.viewport.h);
    ...
    for (neko_slot_array_iter it = neko_slot_array_iter_new(ctx->splits); neko_slot_array_iter_valid(ctx->splits, it); neko_slot_array_iter_advance(ctx->splits, it)) {
        ...
        if (!split->parent) {
            ...
            neko_core_ui_window_begin_ex(ctx, TMP, r, NULL, NULL, opt);
            {
                ...
                // Draw splits
                neko_core_ui_draw_splits(ctx, split);
                ...
                // W
                {
                    ...
                    if (id == ctx->focus && ctx->mouse_down == NEKO_CORE_UI_MOUSE_LEFT) {
                        ...
                        ctx->next_hover_root = top;
                        neko_core_ui_request_t req = neko_default_val();
                        req.type = NEKO_CORE_UI_SPLIT_RESIZE_W;
                        req.split = split;
                        neko_dyn_array_push(ctx->requests, req);
                    }
                }
                ...
                {
                    // Cache rect
                    neko_core_ui_rect_t rr = neko_core_ui_rect(fr.x + fr.w + NEKO_CORE_UI_SPLIT_SIZE, fr.y, NEKO_CORE_UI_SPLIT_SIZE, fr.h);
                    neko_core_ui_rect_t ex = rr;
                    ex.w += 20.f;
                    ...
                    if (id == ctx->focus && ctx->mouse_down == NEKO_CORE_UI_MOUSE_LEFT) {
                        ...
                        ctx->next_hover_root = top;
                        neko_core_ui_request_t req = neko_default_val();
                        req.type = NEKO_CORE_UI_SPLIT_RESIZE_E;
                        req.split = split;
                        neko_dyn_array_push(ctx->requests, req);
                    }
                }
                ...
                {
                    // Cache rect
                    neko_core_ui_rect_t tr = neko_core_ui_rect(fr.x, fr.y - 2.f * NEKO_CORE_UI_SPLIT_SIZE, fr.w, NEKO_CORE_UI_SPLIT_SIZE);
                    neko_core_ui_rect_t ex = tr;
                    ex.y -= 10.f;
                    ex.h += 20.f;
                    ...
                    if (id == ctx->focus && ctx->mouse_down == NEKO_CORE_UI_MOUSE_LEFT) {
                        ...
                        ctx->next_hover_root = top;
                        neko_core_ui_request_t req = neko_default_val();
                        req.type = NEKO_CORE_UI_SPLIT_RESIZE_N;
                        req.split = split;
                        neko_dyn_array_push(ctx->requests, req);
                    }
                }
                ...
                {
                    // Cache rect
                    neko_core_ui_rect_t br = neko_core_ui_rect(fr.x, fr.y + fr.h + NEKO_CORE_UI_SPLIT_SIZE, fr.w, NEKO_CORE_UI_SPLIT_SIZE);
                    neko_core_ui_rect_t ex = br;
                    ex.h += 20.f;
                    ...
                    if (id == ctx->focus && ctx->mouse_down == NEKO_CORE_UI_MOUSE_LEFT) {
                        ...
                        ctx->next_hover_root = top;
                        neko_core_ui_request_t req = neko_default_val();
                        req.type = NEKO_CORE_UI_SPLIT_RESIZE_S;
                        req.split = split;
                        neko_dyn_array_push(ctx->requests, req);
                    }
                }
...
static void neko_core_ui_docking(neko_core_ui_context_t* ctx) {
    if (ctx->undock_root) {
        neko_core_ui_undock_ex_cnt(ctx, ctx->undock_root);
    }

    if (!ctx->focus_root || ctx->focus_root->opt & NEKO_CORE_UI_OPT_NODOCK) return;

    if (ctx->dockable_root || ctx->prev_dockable_root) {
        neko_core_ui_container_t* cnt = ctx->dockable_root ? ctx->dockable_root : ctx->prev_dockable_root;

        if (ctx->prev_dockable_root && !ctx->dockable_root && ctx->mouse_down != NEKO_CORE_UI_MOUSE_LEFT) {
            s32 b = 0;
        }
        ...
        neko_color_t def_col = neko_color_alpha(ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_BUTTON][NEKO_CORE_UI_ELEMENT_STATE_FOCUS].colors[NEKO_CORE_UI_COLOR_BACKGROUND], 100);
        neko_color_t hov_col = neko_color_alpha(ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_BUTTON][NEKO_CORE_UI_ELEMENT_STATE_FOCUS].colors[NEKO_CORE_UI_COLOR_BACKGROUND], 150);

        neko_core_ui_rect_t center = neko_core_ui_rect(c.x, c.y, sz, sz);
        neko_core_ui_rect_t left = neko_core_ui_rect(c.x - off, c.y, sz, sz);
        neko_core_ui_rect_t right = neko_core_ui_rect(c.x + off, c.y, sz, sz);
        neko_core_ui_rect_t top = neko_core_ui_rect(c.x, c.y - off, sz, sz);
        neko_core_ui_rect_t bottom = neko_core_ui_rect(c.x, c.y + off, sz, sz);

        s32 hov_c = (neko_core_ui_rect_overlaps_vec2(center, ctx->mouse_pos));
        s32 hov_l = neko_core_ui_rect_overlaps_vec2(left, ctx->mouse_pos);
        s32 hov_r = neko_core_ui_rect_overlaps_vec2(right, ctx->mouse_pos);
        s32 hov_t = neko_core_ui_rect_overlaps_vec2(top, ctx->mouse_pos);
        s32 hov_b = neko_core_ui_rect_overlaps_vec2(bottom, ctx->mouse_pos);
        s32 hov_w = neko_core_ui_rect_overlaps_vec2(cnt->rect, ctx->mouse_pos);

        bool can_dock = true;

        // Can't dock one editor_dockspace into another
        if (ctx->focus_root->opt & NEKO_CORE_UI_OPT_DOCKSPACE) {
            can_dock = false;
        }

        if (ctx->focus_root->tab_bar) {
            neko_core_ui_container_t* tcmp = ctx->dockable_root ? ctx->dockable_root : ctx->prev_dockable_root;
            neko_core_ui_tab_bar_t* tab_bar = neko_slot_array_getp(ctx->tab_bars, ctx->focus_root->tab_bar);
            for (u32 i = 0; i < tab_bar->size; ++i) {
                neko_core_ui_container_t* tcnt = (neko_core_ui_container_t*)tab_bar->items[i].data;
                if (tcnt == tcmp) {
                    can_dock = false;
                }
            }
        ...
        if (ctx->dockable_root && can_dock) {
            ...
            bool is_dockspace = ctx->dockable_root->opt & NEKO_CORE_UI_OPT_DOCKSPACE;
            ...
            if (is_dockspace) {
                if (hov_c) {
                    center = neko_core_ui_rect(cnt->rect.x, cnt->rect.y, cnt->rect.w, cnt->rect.h);
                ...
                if (hov_c && !ctx->focus_root->split) {
                    ...
                    left = neko_core_ui_rect(cnt->rect.x, cnt->rect.y, cnt->rect.w * d + hs, cnt->rect.h);
                    neko_idraw_rectvd(dl, neko_v2(left.x, left.y), neko_v2(left.w, left.h), neko_v2s(0.f), neko_v2s(1.f), hov_col, NEKO_GRAPHICS_PRIMITIVE_LINES);
                    ...
                    right = neko_core_ui_rect(cnt->rect.x + cnt->rect.w * d + hs, cnt->rect.y, cnt->rect.w * (1.f - d) - hs, cnt->rect.h);
                    neko_idraw_rectvd(dl, neko_v2(right.x, right.y), neko_v2(right.w, right.h), neko_v2s(0.f), neko_v2s(1.f), hov_col, NEKO_GRAPHICS_PRIMITIVE_LINES);
                    ...
                    bottom = neko_core_ui_rect(cnt->rect.x, cnt->rect.y + cnt->rect.h * d + hs, cnt->rect.w, cnt->rect.h * (1.f - d) - hs);
                    neko_idraw_rectvd(dl, neko_v2(bottom.x, bottom.y), neko_v2(bottom.w, bottom.h), neko_v2s(0.f), neko_v2s(1.f), hov_col, NEKO_GRAPHICS_PRIMITIVE_LINES);
                    ...
                    top = neko_core_ui_rect(cnt->rect.x, cnt->rect.y, cnt->rect.w, cnt->rect.h * d + hs);
                    neko_idraw_rectvd(dl, neko_v2(top.x, top.y), neko_v2(top.w, top.h), neko_v2s(0.f), neko_v2s(1.f), hov_col, NEKO_GRAPHICS_PRIMITIVE_LINES);
        ...
        // Handle docking
        if (ctx->prev_dockable_root && !ctx->dockable_root && ctx->mouse_down != NEKO_CORE_UI_MOUSE_LEFT) {
            neko_core_ui_container_t* parent = ctx->prev_dockable_root;
            neko_core_ui_container_t* child = ctx->focus_root;

            bool is_dockspace = ctx->prev_dockable_root->opt & NEKO_CORE_UI_OPT_DOCKSPACE;

            if (is_dockspace) {
                if (hov_c) neko_core_ui_dock_ex_cnt(ctx, child, parent, NEKO_CORE_UI_SPLIT_TOP, 1.0f);
            } else {
                if (hov_c && !ctx->focus_root->split)
                    neko_core_ui_dock_ex_cnt(ctx, child, parent, NEKO_CORE_UI_SPLIT_TAB, 0.5f);
                else if (hov_l)
                    neko_core_ui_dock_ex_cnt(ctx, child, parent, NEKO_CORE_UI_SPLIT_LEFT, 0.5f);
                else if (hov_r)
                    neko_core_ui_dock_ex_cnt(ctx, child, parent, NEKO_CORE_UI_SPLIT_RIGHT, 0.5f);
                else if (hov_t)
                    neko_core_ui_dock_ex_cnt(ctx, child, parent, NEKO_CORE_UI_SPLIT_TOP, 0.5f);
                else if (hov_b)
                    neko_core_ui_dock_ex_cnt(ctx, child, parent, NEKO_CORE_UI_SPLIT_BOTTOM, 0.5f);
            }
        }
    }
}
...
NEKO_API_DECL void neko_core_ui_end(neko_core_ui_context_t* ctx, b32 update) {
    ...
    neko_core_ui_docking(ctx);

    for (u32 r = 0; r < neko_dyn_array_size(ctx->requests); ++r) {
        const neko_core_ui_request_t* req = &ctx->requests[r];

        // If split moved, update position for next frame
        switch (req->type) {
            case NEKO_CORE_UI_CNT_MOVE: {
                if (!update) break;
                if (req->cnt) {
                    ...
                    if (req->cnt->tab_bar) {
                        neko_core_ui_tab_bar_t* tb = neko_slot_array_getp(ctx->tab_bars, req->cnt->tab_bar);
                        neko_assert(tb);
            ...
            case NEKO_CORE_UI_CNT_FOCUS: {
                if (!update) break;
                if (!req->cnt) break;

                neko_core_ui_container_t* cnt = (neko_core_ui_container_t*)req->cnt;
                neko_assert(cnt);

                neko_core_ui_split_t* rs = neko_core_ui_get_root_split(ctx, cnt);

                if (cnt->tab_bar) {
                    if (rs) {
                        neko_core_ui_bring_split_to_front(ctx, rs);
                    }

                    neko_core_ui_tab_bar_t* tab_bar = neko_slot_array_getp(ctx->tab_bars, cnt->tab_bar);
                    neko_core_ui_tab_item_t* tab_item = &tab_bar->items[cnt->tab_item];
                    neko_core_ui_container_t* fcnt = (neko_core_ui_container_t*)tab_bar->items[tab_bar->focus].data;
                    fcnt->opt |= NEKO_CORE_UI_OPT_NOHOVER;
                    fcnt->opt |= NEKO_CORE_UI_OPT_NOINTERACT;
                    fcnt->flags &= ~NEKO_CORE_UI_WINDOW_FLAGS_VISIBLE;
                    ...
                    for (u32 i = 0; i < tab_bar->size; ++i) {
                        neko_core_ui_bring_to_front(ctx, (neko_core_ui_container_t*)tab_bar->items[i].data);
                    }
                    neko_core_ui_bring_to_front(ctx, cnt);
                }
            ...
            case NEKO_CORE_UI_SPLIT_MOVE: {
                if (req->split) {
                    req->split->rect.x += ctx->mouse_delta.x;
                    req->split->rect.y += ctx->mouse_delta.y;
                    neko_core_ui_update_split(ctx, req->split);
                }

            } break;

            case NEKO_CORE_UI_SPLIT_RESIZE_SE: {
                if (req->split) {
                    neko_core_ui_rect_t* r = &req->split->rect;
            ...
            case NEKO_CORE_UI_SPLIT_RESIZE_W: {
                if (req->split) {
                    neko_core_ui_rect_t* r = &req->split->rect;
                    float w = r->w;
                    float max_x = r->x + r->w;
            ...
            case NEKO_CORE_UI_SPLIT_RESIZE_E: {
                if (req->split) {
                    neko_core_ui_rect_t* r = &req->split->rect;
                    r->w = neko_max(r->w + ctx->mouse_delta.x, 40);
                    neko_core_ui_update_split(ctx, req->split);
            ...
            case NEKO_CORE_UI_SPLIT_RESIZE_N: {
                if (req->split) {
                    neko_core_ui_rect_t* r = &req->split->rect;
                    float h = r->h;
                    float max_y = h + r->y;
            ...
            case NEKO_CORE_UI_SPLIT_RESIZE_NE: {
                if (req->split) {
                    neko_core_ui_rect_t* r = &req->split->rect;
                    r->w = neko_max(r->w + ctx->mouse_delta.x, 40);
                    float h = r->h;
            ...
            case NEKO_CORE_UI_SPLIT_RESIZE_NW: {
                if (req->split) {
                    neko_core_ui_rect_t* r = &req->split->rect;
                    float h = r->h;
                    float max_y = h + r->y;
            ...
            case NEKO_CORE_UI_SPLIT_RESIZE_S: {
                if (req->split) {
                    neko_core_ui_rect_t* r = &req->split->rect;
                    r->h = neko_max(r->h + ctx->mouse_delta.y, 40);
                    neko_core_ui_update_split(ctx, req->split);
            ...
            case NEKO_CORE_UI_SPLIT_RESIZE_SW: {
                if (req->split) {
                    neko_core_ui_rect_t* r = &req->split->rect;
                    float h = r->h;
                    float max_y = h + r->y;
            ...
            case NEKO_CORE_UI_SPLIT_RATIO: {
                ...
                // Bring to font
                neko_core_ui_bring_split_to_front(ctx, neko_core_ui_get_root_split_from_split(ctx, split));

            } break;

            case NEKO_CORE_UI_TAB_SWAP_LEFT: {
                neko_core_ui_tab_item_swap(ctx, req->cnt, -1);
            } break;

            case NEKO_CORE_UI_TAB_SWAP_RIGHT: {
                neko_core_ui_tab_item_swap(ctx, req->cnt, +1);
            } break;
        }
    }
    ...
    if (update && ctx->mouse_pressed && ctx->next_hover_root && ctx->next_hover_root->zindex < ctx->last_zindex && ctx->next_hover_root->zindex >= 0) {
        // Root split
        neko_core_ui_split_t* split = neko_core_ui_get_root_split(ctx, ctx->next_hover_root);

        // Need to bring entire editor_dockspace to front
        if (split) {
            neko_core_ui_bring_split_to_front(ctx, split);
        } else if (~ctx->next_hover_root->opt & NEKO_CORE_UI_OPT_NOFOCUS) {
            neko_core_ui_bring_to_front(ctx, ctx->next_hover_root);
        }
    }

    if (ctx->mouse_pressed && (!ctx->next_hover_root || ctx->next_hover_root->opt & NEKO_CORE_UI_OPT_NOFOCUS)) {
    ...
    n = ctx->root_list.idx;
    qsort(ctx->root_list.items, n, sizeof(neko_core_ui_container_t*), neko_core_ui_compare_zindex);

    // Set root container jump commands
    for (i = 0; i < n; i++) {
        neko_core_ui_container_t* cnt = ctx->root_list.items[i];

        // If this is the first container then make the first command jump to it.
        // otherwise set the previous container's tail to jump to this one
        if (i == 0) {
            neko_core_ui_command_t* cmd = (neko_core_ui_command_t*)ctx->command_list.items;
            cmd->jump.dst = (char*)cnt->head + sizeof(neko_core_ui_jumpcommand_t);
        } else {
            neko_core_ui_container_t* prev = ctx->root_list.items[i - 1];
            prev->tail->jump.dst = (char*)cnt->head + sizeof(neko_core_ui_jumpcommand_t);
        }
...
NEKO_API_DECL void neko_core_ui_render(neko_core_ui_context_t* ctx, neko_command_buffer_t* cb) {
    ...
    const neko_core_ui_rect_t* viewport = &ctx->viewport;
    ...
    neko_core_ui_command_t* cmd = NULL;
    while (neko_core_ui_next_command(ctx, &cmd)) {
        switch (cmd->type) {
            case NEKO_CORE_UI_COMMAND_CUSTOM: {
                neko_idraw_defaults(&ctx->gui_idraw);
            ...
            case NEKO_CORE_UI_COMMAND_PIPELINE: {
            ...
            case NEKO_CORE_UI_COMMAND_UNIFORMS: {
                ...
                buffer.capacity = NEKO_CORE_UI_COMMANDLIST_SIZE;
            ...
            case NEKO_CORE_UI_COMMAND_TEXT: {
            ...
            case NEKO_CORE_UI_COMMAND_SHAPE: {
                ...
                switch (cmd->shape.type) {
                    case NEKO_CORE_UI_SHAPE_RECT: {
                        neko_core_ui_rect_t* r = &cmd->shape.rect;
                    ...
                    case NEKO_CORE_UI_SHAPE_CIRCLE: {
                    ...
                    case NEKO_CORE_UI_SHAPE_TRIANGLE: {
            ...
            case NEKO_CORE_UI_COMMAND_CLIP: {
...
NEKO_API_DECL void neko_core_ui_renderpass_submit(neko_core_ui_context_t* ctx, neko_command_buffer_t* cb, neko_color_t c) {
    neko_vec2 fbs = ctx->framebuffer_size;
...
NEKO_API_DECL neko_core_ui_id neko_core_ui_get_id(neko_core_ui_context_t* ctx, const void* data, s32 size) {
    ...
    neko_core_ui_id res = (idx > 0) ? ctx->id_stack.items[idx - 1] : NEKO_CORE_UI_HASH_INITIAL;
    neko_core_ui_hash(&res, data, size);
    ctx->last_id = res;

    return res;
}

NEKO_API_DECL neko_core_ui_id neko_core_ui_get_id_hash(neko_core_ui_context_t* ctx, const void* data, s32 size, neko_core_ui_id hash) {
    neko_core_ui_id res = hash;
    neko_core_ui_hash(&res, data, size);
    ctx->last_id = res;
    return res;
}

NEKO_API_DECL void neko_core_ui_push_id(neko_core_ui_context_t* ctx, const void* data, s32 size) { neko_core_ui_stack_push(ctx->id_stack, neko_core_ui_get_id(ctx, data, size)); }

NEKO_API_DECL void neko_core_ui_pop_id(neko_core_ui_context_t* ctx) { neko_core_ui_stack_pop(ctx->id_stack); }

NEKO_API_DECL void neko_core_ui_push_clip_rect(neko_core_ui_context_t* ctx, neko_core_ui_rect_t rect) {
    neko_core_ui_rect_t last = neko_core_ui_get_clip_rect(ctx);
    neko_core_ui_stack_push(ctx->clip_stack, neko_core_ui_intersect_rects(rect, last));
}

NEKO_API_DECL void neko_core_ui_pop_clip_rect(neko_core_ui_context_t* ctx) { neko_core_ui_stack_pop(ctx->clip_stack); }
...
NEKO_API_DECL s32 neko_core_ui_check_clip(neko_core_ui_context_t* ctx, neko_core_ui_rect_t r) {
    neko_core_ui_rect_t cr = neko_core_ui_get_clip_rect(ctx);

    if (r.x > cr.x + cr.w || r.x + r.w < cr.x || r.y > cr.y + cr.h || r.y + r.h < cr.y) {
        return NEKO_CORE_UI_CLIP_ALL;
    }
...
NEKO_API_DECL neko_core_ui_container_t* neko_core_ui_get_container(neko_core_ui_context_t* ctx, const char* name) {
    neko_core_ui_id id = neko_core_ui_get_id(ctx, name, strlen(name));
    return neko_core_ui_get_container_ex(ctx, id, 0);
}

NEKO_API_DECL void neko_core_ui_bring_to_front(neko_core_ui_context_t* ctx, neko_core_ui_container_t* cnt) {
    neko_core_ui_container_t* root = neko_core_ui_get_root_container(ctx, cnt);
    if (root->opt & NEKO_CORE_UI_OPT_NOBRINGTOFRONT) {
        if (cnt->opt & NEKO_CORE_UI_OPT_DOCKSPACE)
            cnt->zindex = 0;
        else
            cnt->zindex = 2;
        if (cnt->tab_bar) {
            neko_core_ui_tab_bar_t* tab_bar = neko_slot_array_getp(ctx->tab_bars, cnt->tab_bar);
            for (u32 i = 0; i < tab_bar->size; ++i) {
                ((neko_core_ui_container_t*)tab_bar->items[i].data)->zindex = cnt->zindex + i;
            }
        }
    } else {
        ...
        if (cnt->tab_bar) {
            neko_core_ui_tab_bar_t* tab_bar = neko_slot_array_getp(ctx->tab_bars, cnt->tab_bar);
            for (u32 i = 0; i < tab_bar->size; ++i) {
                ((neko_core_ui_container_t*)tab_bar->items[i].data)->zindex = ++ctx->last_zindex;
            }
        }
    }
}
...
NEKO_API_DECL s32 neko_core_ui_pool_init(neko_core_ui_context_t* ctx, neko_core_ui_pool_item_t* items, s32 len, neko_core_ui_id id) {
    ...
    neko_expect(n > -1);
    items[n].id = id;
    neko_core_ui_pool_update(ctx, items, n);
...
NEKO_API_DECL void neko_core_ui_pool_update(neko_core_ui_context_t* ctx, neko_core_ui_pool_item_t* items, s32 idx) { items[idx].last_update = ctx->frame; }

/*============================================================================
** input handlers
**============================================================================*/

NEKO_API_DECL void neko_core_ui_input_mousemove(neko_core_ui_context_t* ctx, s32 x, s32 y) { ctx->mouse_pos = neko_v2((f32)x, (f32)y); }

NEKO_API_DECL void neko_core_ui_input_mousedown(neko_core_ui_context_t* ctx, s32 x, s32 y, s32 btn) {
    neko_core_ui_input_mousemove(ctx, x, y);
    ctx->mouse_down |= btn;
...
NEKO_API_DECL void neko_core_ui_input_keyup(neko_core_ui_context_t* ctx, s32 key) { ctx->key_down &= ~key; }
...
NEKO_API_DECL neko_core_ui_command_t* neko_core_ui_push_command(neko_core_ui_context_t* ctx, s32 type, s32 size) {
    neko_core_ui_command_t* cmd = (neko_core_ui_command_t*)(ctx->command_list.items + ctx->command_list.idx);
    neko_expect(ctx->command_list.idx + size < NEKO_CORE_UI_COMMANDLIST_SIZE);
    cmd->base.type = type;
    cmd->base.size = size;
...
NEKO_API_DECL s32 neko_core_ui_next_command(neko_core_ui_context_t* ctx, neko_core_ui_command_t** cmd) {
    if (*cmd) {
        *cmd = (neko_core_ui_command_t*)(((char*)*cmd) + (*cmd)->base.size);
    } else {
        *cmd = (neko_core_ui_command_t*)ctx->command_list.items;
    }

    while ((u8*)*cmd != (u8*)(ctx->command_list.items + ctx->command_list.idx)) {
        if ((*cmd)->type != NEKO_CORE_UI_COMMAND_JUMP) {
            return 1;
        }
        *cmd = (neko_core_ui_command_t*)((*cmd)->jump.dst);
    }
    return 0;
}
...
NEKO_API_DECL void neko_core_ui_set_clip(neko_core_ui_context_t* ctx, neko_core_ui_rect_t rect) {
    neko_core_ui_command_t* cmd;
    cmd = neko_core_ui_push_command(ctx, NEKO_CORE_UI_COMMAND_CLIP, sizeof(neko_core_ui_clipcommand_t));
    cmd->clip.rect = rect;
}

NEKO_API_DECL void neko_core_ui_set_pipeline(neko_core_ui_context_t* ctx, neko_handle(neko_graphics_pipeline_t) pip, void* layout, size_t sz, neko_idraw_layout_type type) {
    neko_core_ui_command_t* cmd;
    cmd = neko_core_ui_push_command(ctx, NEKO_CORE_UI_COMMAND_PIPELINE, sizeof(neko_core_ui_pipelinecommand_t));
...
NEKO_API_DECL void neko_core_ui_bind_uniforms(neko_core_ui_context_t* ctx, neko_graphics_bind_uniform_desc_t* uniforms, size_t uniforms_sz) {
    neko_core_ui_command_t* cmd;
    cmd = neko_core_ui_push_command(ctx, NEKO_CORE_UI_COMMAND_UNIFORMS, sizeof(neko_core_ui_binduniformscommand_t));
...
NEKO_API_DECL void neko_core_ui_draw_line(neko_core_ui_context_t* ctx, neko_vec2 start, neko_vec2 end, neko_color_t color) {
    neko_core_ui_command_t* cmd;
    neko_core_ui_rect_t rect = neko_default_val();
    neko_vec2 s = start.x < end.x ? start : end;
    neko_vec2 e = start.x < end.x ? end : start;
    neko_core_ui_rect(s.x, s.y, e.x - s.x, e.y - s.y);
    rect = neko_core_ui_intersect_rects(rect, neko_core_ui_get_clip_rect(ctx));
    ...
    if (clipped == NEKO_CORE_UI_CLIP_PART) {
        neko_core_ui_set_clip(ctx, neko_core_ui_get_clip_rect(ctx));
    }

    cmd = neko_core_ui_push_command(ctx, NEKO_CORE_UI_COMMAND_SHAPE, sizeof(neko_core_ui_shapecommand_t));
    cmd->shape.type = NEKO_CORE_UI_SHAPE_LINE;
...
NEKO_API_DECL void neko_core_ui_draw_rect(neko_core_ui_context_t* ctx, neko_core_ui_rect_t rect, neko_color_t color) {
    ...
    if (rect.w > 0 && rect.h > 0) {
        cmd = neko_core_ui_push_command(ctx, NEKO_CORE_UI_COMMAND_SHAPE, sizeof(neko_core_ui_shapecommand_t));
        cmd->shape.type = NEKO_CORE_UI_SHAPE_RECT;
        cmd->shape.rect = rect;
        cmd->shape.color = color;
    }
}
...
NEKO_API_DECL void neko_core_ui_draw_circle(neko_core_ui_context_t* ctx, neko_vec2 position, float radius, neko_color_t color) {
    neko_core_ui_command_t* cmd;
    neko_core_ui_rect_t rect = neko_core_ui_rect(position.x - radius, position.y - radius, 2.f * radius, 2.f * radius);
    ...
    // do shape command
    cmd = neko_core_ui_push_command(ctx, NEKO_CORE_UI_COMMAND_SHAPE, sizeof(neko_core_ui_shapecommand_t));
    cmd->shape.type = NEKO_CORE_UI_SHAPE_CIRCLE;
...
NEKO_API_DECL void neko_core_ui_draw_triangle(neko_core_ui_context_t* ctx, neko_vec2 a, neko_vec2 b, neko_vec2 c, neko_color_t color) {
    ...
    s32 clipped = 0x00;
    neko_core_ui_rect_t clip = neko_core_ui_get_clip_rect(ctx);
    s32 ca = neko_core_ui_rect_overlaps_vec2(clip, a);
    s32 cb = neko_core_ui_rect_overlaps_vec2(clip, b);
    s32 cc = neko_core_ui_rect_overlaps_vec2(clip, c);

    if (ca && cb && cc)
        clipped = 0x00;  // No clip
    else if (!ca && !cb && !cc)
        clipped = NEKO_CORE_UI_CLIP_ALL;
    else if (ca || cb || cc)
        clipped = NEKO_CORE_UI_CLIP_PART;

    if (clipped == NEKO_CORE_UI_CLIP_ALL) {
        return;
    }
    if (clipped == NEKO_CORE_UI_CLIP_PART) {
        neko_core_ui_set_clip(ctx, clip);
    }

    cmd = neko_core_ui_push_command(ctx, NEKO_CORE_UI_COMMAND_SHAPE, sizeof(neko_core_ui_shapecommand_t));
    cmd->shape.type = NEKO_CORE_UI_SHAPE_TRIANGLE;
...
NEKO_API_DECL void neko_core_ui_draw_box(neko_core_ui_context_t* ctx, neko_core_ui_rect_t rect, s16* w, neko_color_t color) {
    neko_immediate_draw_t* dl = &ctx->overlay_draw_list;
    ...
    neko_core_ui_draw_rect(ctx, neko_core_ui_rect(rect.x + l, rect.y, rect.w - r - l, t), color);               // top
    neko_core_ui_draw_rect(ctx, neko_core_ui_rect(rect.x + l, rect.y + rect.h - b, rect.w - r - l, b), color);  // bottom
    neko_core_ui_draw_rect(ctx, neko_core_ui_rect(rect.x, rect.y, l, rect.h), color);                           // left
    neko_core_ui_draw_rect(ctx, neko_core_ui_rect(rect.x + rect.w - r, rect.y, r, rect.h), color);              // right
}
...
#define DRAW_TEXT(TEXT, RECT, COLOR)                                                                               \
    ...
    if (shadow_x || shadow_y && shadow_color.a) {
        DRAW_TEXT(str, neko_core_ui_rect(pos.x + (float)shadow_x, pos.y + (float)shadow_y, td.x, td.y), shadow_color);
    }

    // Draw text
    { DRAW_TEXT(str, neko_core_ui_rect(pos.x, pos.y, td.x, td.y), color); }
}
...
NEKO_API_DECL void neko_core_ui_draw_image(neko_core_ui_context_t* ctx, neko_handle(neko_graphics_texture_t) hndl, neko_core_ui_rect_t rect, neko_vec2 uv0, neko_vec2 uv1, neko_color_t color) {
    ...
    /* do image command */
    cmd = neko_core_ui_push_command(ctx, NEKO_CORE_UI_COMMAND_IMAGE, sizeof(neko_core_ui_imagecommand_t));
    cmd->image.hndl = hndl;
    cmd->image.rect = rect;
...
NEKO_API_DECL void neko_core_ui_draw_custom(neko_core_ui_context_t* ctx, neko_core_ui_rect_t rect, neko_core_ui_draw_callback_t cb, void* data, size_t sz) {
    ...
    neko_core_ui_rect_t viewport = rect;
    ...
    /* do custom command */
    cmd = neko_core_ui_push_command(ctx, NEKO_CORE_UI_COMMAND_CUSTOM, sizeof(neko_core_ui_customcommand_t));
...
NEKO_API_DECL void neko_core_ui_draw_nine_rect(neko_core_ui_context_t* ctx, neko_handle(neko_graphics_texture_t) hndl, neko_core_ui_rect_t rect, neko_vec2 uv0, neko_vec2 uv1, u32 left, u32 right,
    ...
    // tl
    {
        u32 w = left;
        u32 h = top;
        neko_core_ui_rect_t r = neko_core_ui_rect(rect.x, rect.y, (f32)w, (f32)h);
        neko_vec2 st0 = neko_v2(uv0.x, uv0.y);
        neko_vec2 st1 = neko_v2(uv0.x + ((float)left / (float)width), uv0.y + ((float)top / (float)height));
        neko_core_ui_draw_image(ctx, hndl, r, st0, st1, color);
    }
    ...
    // tr
    {
        u32 w = right;
        u32 h = top;
        neko_core_ui_rect_t r = neko_core_ui_rect(rect.x + rect.w - w, rect.y, (f32)w, (f32)h);
    ...
    // br
    {
        u32 w = right;
        u32 h = bottom;
        neko_core_ui_rect_t r = neko_core_ui_rect(rect.x + rect.w - (f32)w, rect.y + rect.h - (f32)h, (f32)w, (f32)h);
    ...
    // bl
    {
        u32 w = left;
        u32 h = bottom;
        neko_core_ui_rect_t r = neko_core_ui_rect(rect.x, rect.y + rect.h - (f32)h, (f32)w, (f32)h);
    ...
    // top
    {
        ...
        u32 h = top;
        neko_core_ui_rect_t r = neko_core_ui_rect(rect.x + left, rect.y, (f32)w, (f32)h);
    ...
    // bottom
    {
        u32 w = (u32)rect.w - left - right;
        u32 h = bottom;
        neko_core_ui_rect_t r = neko_core_ui_rect(rect.x + left, rect.y + rect.h - (f32)h, (f32)w, (f32)h);
    ...
    // left
    {
        ...
        neko_core_ui_rect_t r = neko_core_ui_rect(rect.x, rect.y + top, (f32)w, (f32)h);
    ...
    // right
    {
        u32 w = right;
        u32 h = (u32)rect.h - top - bottom;
        neko_core_ui_rect_t r = neko_core_ui_rect(rect.x + rect.w - (f32)w, rect.y + top, (f32)w, (f32)h);
    ...
    // center
    {
        ...
        u32 h = (u32)rect.h - top - bottom;
        neko_core_ui_rect_t r = neko_core_ui_rect(rect.x + left, rect.y + top, (f32)w, (f32)h);
...
** layout
**============================================================================*/
enum { NEKO_CORE_UI_RELATIVE = 1, NEKO_CORE_UI_ABSOLUTE = 2 };

NEKO_API_DECL neko_core_ui_rect_t neko_core_ui_layout_anchor(const neko_core_ui_rect_t* p, s32 width, s32 height, s32 xoff, s32 yoff, neko_core_ui_layout_anchor_type type) {
    float w = (float)width;
    float h = (float)height;
    neko_core_ui_rect_t r = neko_core_ui_rect(p->x, p->y, w, h);

    switch (type) {
        default:
        case NEKO_CORE_UI_LAYOUT_ANCHOR_TOPLEFT: {
        } break;

        case NEKO_CORE_UI_LAYOUT_ANCHOR_TOPCENTER: {
            r.x = p->x + (p->w - w) * 0.5f;
        } break;

        case NEKO_CORE_UI_LAYOUT_ANCHOR_TOPRIGHT: {
            r.x = p->x + (p->w - w);
        } break;

        case NEKO_CORE_UI_LAYOUT_ANCHOR_LEFT: {
            r.y = p->y + (p->h - h) * 0.5f;
        } break;

        case NEKO_CORE_UI_LAYOUT_ANCHOR_CENTER: {
            r.x = p->x + (p->w - w) * 0.5f;
            r.y = p->y + (p->h - h) * 0.5f;
        } break;

        case NEKO_CORE_UI_LAYOUT_ANCHOR_RIGHT: {
            r.x = p->x + (p->w - w);
            r.y = p->y + (p->h - h) * 0.5f;
        } break;

        case NEKO_CORE_UI_LAYOUT_ANCHOR_BOTTOMLEFT: {
            r.y = p->y + (p->h - h);
        } break;

        case NEKO_CORE_UI_LAYOUT_ANCHOR_BOTTOMCENTER: {
            r.x = p->x + (p->w - w) * 0.5f;
            r.y = p->y + (p->h - h);
        } break;

        case NEKO_CORE_UI_LAYOUT_ANCHOR_BOTTOMRIGHT: {
            r.x = p->x + (p->w - w);
            r.y = p->y + (p->h - h);
        } break;
    }
...
NEKO_API_DECL void neko_core_ui_layout_column_begin(neko_core_ui_context_t* ctx) { neko_core_ui_push_layout(ctx, neko_core_ui_layout_next(ctx), neko_v2(0, 0)); }

NEKO_API_DECL void neko_core_ui_layout_column_end(neko_core_ui_context_t* ctx) {
    neko_core_ui_layout_t *a, *b;
    b = neko_core_ui_get_layout(ctx);
    neko_core_ui_stack_pop(ctx->layout_stack);

    /* inherit position/next_row/max from child layout if they are greater */
    a = neko_core_ui_get_layout(ctx);
...
NEKO_API_DECL void neko_core_ui_layout_row(neko_core_ui_context_t* ctx, s32 items, const s32* widths, s32 height) {
    neko_core_ui_style_t* style = ctx->style;
    neko_core_ui_layout_t* layout = neko_core_ui_get_layout(ctx);

    if (widths) {
        neko_expect(items <= NEKO_CORE_UI_MAX_WIDTHS);
...
NEKO_API_DECL void neko_core_ui_layout_row_ex(neko_core_ui_context_t* ctx, s32 items, const s32* widths, s32 height, s32 justification) {
    neko_core_ui_layout_row(ctx, items, widths, height);
    neko_core_ui_layout_t* layout = neko_core_ui_get_layout(ctx);
    ...
    switch (justification) {
        ...
        case NEKO_CORE_UI_JUSTIFY_CENTER: {
...
NEKO_API_DECL void neko_core_ui_layout_width(neko_core_ui_context_t* ctx, s32 width) { neko_core_ui_get_layout(ctx)->size.x = (f32)width; }

NEKO_API_DECL void neko_core_ui_layout_height(neko_core_ui_context_t* ctx, s32 height) { neko_core_ui_get_layout(ctx)->size.y = (f32)height; }

NEKO_API_DECL void neko_core_ui_layout_set_next(neko_core_ui_context_t* ctx, neko_core_ui_rect_t r, s32 relative) {
    neko_core_ui_layout_t* layout = neko_core_ui_get_layout(ctx);
    layout->next = r;
    layout->next_type = relative ? NEKO_CORE_UI_RELATIVE : NEKO_CORE_UI_ABSOLUTE;
}
...
NEKO_API_DECL neko_core_ui_rect_t neko_core_ui_layout_peek_next(neko_core_ui_context_t* ctx) {
    ...
    if (layout.next_type) {
        /* handle rect set by `neko_core_ui_layout_set_next` */
        s32 type = layout.next_type;
        res = layout.next;
        if (type == NEKO_CORE_UI_ABSOLUTE) {
            return res;
        }
        ...
        if (layout.item_index == layout.items) {
            neko_core_ui_layout_row(ctx, layout.items, NULL, (s32)layout.size.y);
        }

        const s32 items = layout.items;
        const s32 idx = layout.item_index;

        s32 ml = style->margin[NEKO_CORE_UI_MARGIN_LEFT];
        s32 mr = style->margin[NEKO_CORE_UI_MARGIN_RIGHT];
        s32 mt = style->margin[NEKO_CORE_UI_MARGIN_TOP];
        s32 mb = style->margin[NEKO_CORE_UI_MARGIN_BOTTOM];
    ...
    /* update position */
    layout.position.x += res.w + style->margin[NEKO_CORE_UI_MARGIN_RIGHT];
    layout.next_row = (s32)neko_max(layout.next_row, res.y + res.h + style->margin[NEKO_CORE_UI_MARGIN_BOTTOM]);
...
NEKO_API_DECL neko_core_ui_rect_t neko_core_ui_layout_next(neko_core_ui_context_t* ctx) {
    ...
    /* update position */
    layout->position.x += res.w + style->margin[NEKO_CORE_UI_MARGIN_RIGHT];
    layout->next_row = (s32)neko_max(layout->next_row, res.y + res.h + style->margin[NEKO_CORE_UI_MARGIN_BOTTOM]);  //  + style->margin[NEKO_CORE_UI_MARGIN_TOP] * 0.5f);
...
static s32 neko_core_ui_in_hover_root(neko_core_ui_context_t* ctx) {
...
NEKO_API_DECL void neko_core_ui_draw_control_frame(neko_core_ui_context_t* ctx, neko_core_ui_id id, neko_core_ui_rect_t rect, s32 elementid, u64 opt) {
    if (opt & NEKO_CORE_UI_OPT_NOFRAME) {
        return;
    }
    s32 state = ctx->focus == id ? NEKO_CORE_UI_ELEMENT_STATE_FOCUS : ctx->hover == id ? NEKO_CORE_UI_ELEMENT_STATE_HOVER : 0x00;
    neko_core_ui_draw_frame(ctx, rect, &ctx->style_sheet->styles[elementid][state]);
}
...
NEKO_API_DECL void neko_core_ui_draw_control_text(neko_core_ui_context_t* ctx, const char* str, neko_core_ui_rect_t rect, const neko_core_ui_style_t* style, u64 opt) {
    ...
    neko_vec2 td = neko_core_ui_text_dimensions(font, str, -1);
    s32 tw = (s32)td.x;
    s32 th = (s32)td.y;

    neko_core_ui_push_clip_rect(ctx, rect);

    // Grab stylings
    const s32 padding_left = style->padding[NEKO_CORE_UI_PADDING_LEFT];
    const s32 padding_top = style->padding[NEKO_CORE_UI_PADDING_TOP];
    const s32 padding_right = style->padding[NEKO_CORE_UI_PADDING_RIGHT];
    const s32 padding_bottom = style->padding[NEKO_CORE_UI_PADDING_BOTTOM];
    ...
    switch (justify) {
        default:
        case NEKO_CORE_UI_JUSTIFY_START: {
    ...
    switch (align) {
        default:
        case NEKO_CORE_UI_ALIGN_START: {
            pos.y = rect.y + padding_top;
        } break;

        case NEKO_CORE_UI_ALIGN_CENTER: {
            pos.y = rect.y + (rect.h - th) * 0.5f;
        } break;

        case NEKO_CORE_UI_ALIGN_END: {
            pos.y = rect.y + (rect.h - th) - padding_bottom;
        } break;
    }

    bool is_content = (opt & NEKO_CORE_UI_OPT_ISCONTENT);
    s32 bg_color = is_content ? NEKO_CORE_UI_COLOR_CONTENT_BACKGROUND : NEKO_CORE_UI_COLOR_BACKGROUND;
    s32 sh_color = is_content ? NEKO_CORE_UI_COLOR_CONTENT_SHADOW : NEKO_CORE_UI_COLOR_SHADOW;
    s32 bd_color = is_content ? NEKO_CORE_UI_COLOR_CONTENT_BORDER : NEKO_CORE_UI_COLOR_BORDER;
    ...
    if (bc->a && ~opt & NEKO_CORE_UI_OPT_NOSTYLEBORDER) {
        neko_core_ui_pop_clip_rect(ctx);
        neko_core_ui_rect_t border_rect = neko_core_ui_expand_rect(rect, (s16*)style->border_width);
        neko_core_ui_push_clip_rect(ctx, border_rect);
        neko_core_ui_draw_box(ctx, border_rect, (s16*)style->border_width, *bc);
    }

    // Background
    if (~opt & NEKO_CORE_UI_OPT_NOSTYLEBACKGROUND) {
        neko_core_ui_draw_rect(ctx, rect, style->colors[bg_color]);
    }

    // Text
    neko_core_ui_draw_text(ctx, font, str, -1, pos, style->colors[NEKO_CORE_UI_COLOR_CONTENT], sx, sy, *sc);

    neko_core_ui_pop_clip_rect(ctx);
}

NEKO_API_DECL s32 neko_core_ui_mouse_over(neko_core_ui_context_t* ctx, neko_core_ui_rect_t rect) {
    return neko_core_ui_rect_overlaps_vec2(rect, ctx->mouse_pos) && !ctx->hover_split && !ctx->lock_hover_id && neko_core_ui_rect_overlaps_vec2(neko_core_ui_get_clip_rect(ctx), ctx->mouse_pos) &&
           neko_core_ui_in_hover_root(ctx);
}
...
NEKO_API_DECL void neko_core_ui_update_control(neko_core_ui_context_t* ctx, neko_core_ui_id id, neko_core_ui_rect_t rect, u64 opt) {
    ...
    neko_core_ui_id prev_hov = ctx->prev_hover;
    neko_core_ui_id prev_foc = ctx->prev_focus;

    // I should do updates in here

    if (opt & NEKO_CORE_UI_OPT_FORCEFOCUS) {
        mouseover = neko_core_ui_rect_overlaps_vec2(neko_core_ui_get_clip_rect(ctx), ctx->mouse_pos);
    } else {
        mouseover = neko_core_ui_mouse_over(ctx, rect);
    }
    ...
    // Do state check
    if (~opt & NEKO_CORE_UI_OPT_NOSWITCHSTATE) {
        if (ctx->focus == id) {
            if (ctx->prev_focus != id)
                ctx->last_focus_state = NEKO_CORE_UI_ELEMENT_STATE_ON_FOCUS;
            else
                ctx->last_focus_state = NEKO_CORE_UI_ELEMENT_STATE_FOCUS;
        } else {
            if (ctx->prev_focus == id)
                ctx->last_focus_state = NEKO_CORE_UI_ELEMENT_STATE_OFF_FOCUS;
            else
                ctx->last_focus_state = NEKO_CORE_UI_ELEMENT_STATE_DEFAULT;
        }

        if (ctx->hover == id) {
            if (ctx->prev_hover != id)
                ctx->last_hover_state = NEKO_CORE_UI_ELEMENT_STATE_ON_HOVER;
            else
                ctx->last_hover_state = NEKO_CORE_UI_ELEMENT_STATE_HOVER;
        } else {
            if (ctx->prev_hover == id)
                ctx->last_hover_state = NEKO_CORE_UI_ELEMENT_STATE_OFF_HOVER;
            else
                ctx->last_hover_state = NEKO_CORE_UI_ELEMENT_STATE_DEFAULT;
        }

        if (ctx->prev_focus == id && !ctx->mouse_down && ~opt & NEKO_CORE_UI_OPT_HOLDFOCUS) {
            ctx->prev_focus = ctx->focus;
        }
...
NEKO_API_DECL s32 neko_core_ui_text_ex(neko_core_ui_context_t* ctx, const char* text, s32 wrap, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    s32 res = 0, elementid = NEKO_CORE_UI_ELEMENT_TEXT;
    neko_core_ui_id id = neko_core_ui_get_id(ctx, text, strlen(text));
    neko_immediate_draw_t* dl = &ctx->overlay_draw_list;
    neko_core_ui_style_t style = neko_default_val();
    neko_core_ui_animation_t* anim = neko_core_ui_get_animation(ctx, id, desc, elementid);
    ...
    neko_core_ui_style_t* save = neko_core_ui_push_style(ctx, &style);
    neko_asset_ascii_font_t* font = ctx->style->font;
    neko_color_t* color = &ctx->style->colors[NEKO_CORE_UI_COLOR_CONTENT];
    s32 sx = ctx->style->shadow_x;
    s32 sy = ctx->style->shadow_y;
    if (opt & NEKO_CORE_UI_OPT_NOSTYLESHADOW) {
        sx = 0;
        sy = 0;
    }
    neko_color_t* sc = &ctx->style->colors[NEKO_CORE_UI_COLOR_SHADOW];
    s32 th = neko_core_ui_font_height(font);
    neko_core_ui_layout_column_begin(ctx);
    neko_core_ui_layout_row(ctx, 1, &width, th);

    neko_core_ui_rect_t tr = neko_core_ui_layout_next(ctx);
    neko_core_ui_layout_set_next(ctx, tr, 0);
    neko_core_ui_rect_t r = neko_core_ui_layout_next(ctx);
    neko_core_ui_rect_t bg = r;
    do {
        s32 w = 0;
        start = end = p;
        do {
            ...
            if (wrap) w += neko_core_ui_text_width(font, word, p - word);
            if (w > r.w && end != start) {
                break;
            }

            if (wrap) w += neko_core_ui_text_width(font, p, 1);
            end = p++;
        ...
        neko_core_ui_rect_t txtrct = r;
        bg = r;
        if (*end) {
            r = neko_core_ui_layout_next(ctx);
            bg.h = r.y - bg.y;
        } else {
            s32 th = neko_core_ui_text_height(font, start, end - start);
            bg.h = r.h + (float)th / 2.f;
        }

        // Draw frame here for background if applicable (need to do this to account for space between wrap)
        if (ctx->style->colors[NEKO_CORE_UI_COLOR_BACKGROUND].a && ~opt & NEKO_CORE_UI_OPT_NOSTYLEBACKGROUND) {
            neko_core_ui_draw_rect(ctx, bg, style.colors[NEKO_CORE_UI_COLOR_BACKGROUND]);
        }

        // Draw text
        neko_core_ui_draw_text(ctx, font, start, end - start, neko_v2(txtrct.x, txtrct.y), *color, sx, sy, *sc);
        p = end + 1;

    } while (*end);

    // draw border
    if (style.colors[NEKO_CORE_UI_COLOR_BORDER].a && ~opt & NEKO_CORE_UI_OPT_NOSTYLEBORDER) {
        neko_core_ui_draw_box(ctx, neko_core_ui_expand_rect(tr, (s16*)style.border_width), (s16*)style.border_width, style.colors[NEKO_CORE_UI_COLOR_BORDER]);
    }

    neko_core_ui_update_control(ctx, id, tr, 0x00);

    // handle click
    if (ctx->mouse_down != NEKO_CORE_UI_MOUSE_LEFT && ctx->hover == id && ctx->last_focus_state == NEKO_CORE_UI_ELEMENT_STATE_OFF_FOCUS) {
        res |= NEKO_CORE_UI_RES_SUBMIT;
    }

    neko_core_ui_layout_column_end(ctx);
    neko_core_ui_pop_style(ctx, save);
...
NEKO_API_DECL s32 neko_core_ui_label_ex(neko_core_ui_context_t* ctx, const char* label, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    // Want to push animations here for styles
    s32 res = 0;
    s32 elementid = NEKO_CORE_UI_ELEMENT_LABEL;
    neko_core_ui_id id = neko_core_ui_get_id(ctx, label, neko_strlen(label));
    ...
    neko_core_ui_style_t* save = neko_core_ui_push_style(ctx, &style);
    neko_core_ui_rect_t r = neko_core_ui_layout_next(ctx);
    neko_core_ui_update_control(ctx, id, r, 0x00);
    neko_core_ui_draw_control_text(ctx, label_tag, r, &style, 0x00);
    neko_core_ui_pop_style(ctx, save);
    if (id_tag) neko_core_ui_pop_id(ctx);
...
NEKO_API_DECL s32 neko_core_ui_image_ex(neko_core_ui_context_t* ctx, neko_handle(neko_graphics_texture_t) hndl, neko_vec2 uv0, neko_vec2 uv1, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    s32 res = 0;
    neko_core_ui_id id = neko_core_ui_get_id(ctx, &hndl, sizeof(hndl));
    const s32 elementid = NEKO_CORE_UI_ELEMENT_IMAGE;
    ...
    neko_core_ui_style_t* save = neko_core_ui_push_style(ctx, &style);
    neko_core_ui_rect_t r = neko_core_ui_layout_next(ctx);
    neko_core_ui_update_control(ctx, id, r, opt);
    ...
    // draw border
    if (style.colors[NEKO_CORE_UI_COLOR_BORDER].a) {
        neko_core_ui_draw_box(ctx, neko_core_ui_expand_rect(r, (s16*)style.border_width), (s16*)style.border_width, style.colors[NEKO_CORE_UI_COLOR_BORDER]);
    }

    neko_core_ui_draw_image(ctx, hndl, r, uv0, uv1, style.colors[NEKO_CORE_UI_COLOR_CONTENT]);
...
NEKO_API_DECL s32 neko_core_ui_combo_begin_ex(neko_core_ui_context_t* ctx, const char* id, const char* current_item, s32 max_items, neko_core_ui_selector_desc_t* desc, u64 opt) {
    s32 res = 0;
    opt = NEKO_CORE_UI_OPT_NOMOVE | NEKO_CORE_UI_OPT_NORESIZE | NEKO_CORE_UI_OPT_NOTITLE | NEKO_CORE_UI_OPT_FORCESETRECT;

    if (neko_core_ui_button(ctx, current_item)) {
        neko_core_ui_popup_open(ctx, id);
    }

    s32 ct = max_items > 0 ? max_items : 0;
    neko_core_ui_rect_t rect = ctx->last_rect;
    rect.y += rect.h;
    rect.h = ct ? (ct + 1) * ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_BUTTON][0x00].size[1] : rect.h;
    return neko_core_ui_popup_begin_ex(ctx, id, rect, NULL, opt);
}
...
NEKO_API_DECL s32 neko_core_ui_combo_item_ex(neko_core_ui_context_t* ctx, const char* label, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    s32 res = neko_core_ui_button_ex(ctx, label, desc, opt);
    if (res) {
        neko_core_ui_current_container_close(ctx);
    }
    return res;
}
...
NEKO_API_DECL s32 neko_core_ui_button_ex(neko_core_ui_context_t* ctx, const char* label, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    ...
    neko_core_ui_style_t style = neko_default_val();
    neko_core_ui_animation_t* anim = neko_core_ui_get_animation(ctx, id, desc, NEKO_CORE_UI_ELEMENT_BUTTON);

    // Push id if tag available
    if (id_tag) {
        neko_core_ui_push_id(ctx, id_tag, strlen(id_tag));
    }
    ...
    if (anim) {
        ...
        // Get blended style based on animation
        style = neko_core_ui_animation_get_blend_style(ctx, anim, desc, NEKO_CORE_UI_ELEMENT_BUTTON);
    } else {
        style = ctx->focus == id   ? neko_core_ui_get_current_element_style(ctx, desc, NEKO_CORE_UI_ELEMENT_BUTTON, 0x02)
                : ctx->hover == id ? neko_core_ui_get_current_element_style(ctx, desc, NEKO_CORE_UI_ELEMENT_BUTTON, 0x01)
                                   : neko_core_ui_get_current_element_style(ctx, desc, NEKO_CORE_UI_ELEMENT_BUTTON, 0x00);
    }
    ...
    opt |= NEKO_CORE_UI_OPT_ISCONTENT;
    neko_core_ui_draw_rect(ctx, r, style.colors[NEKO_CORE_UI_COLOR_BACKGROUND]);
    if (label) {
        neko_core_ui_draw_control_text(ctx, label_tag, r, &style, opt);
    }
...
NEKO_API_DECL s32 neko_core_ui_checkbox_ex(neko_core_ui_context_t* ctx, const char* label, s32* state, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    s32 res = 0;
    neko_core_ui_id id = neko_core_ui_get_id(ctx, &state, sizeof(state));
    neko_core_ui_rect_t r = neko_core_ui_layout_next(ctx);
    neko_core_ui_rect_t box = neko_core_ui_rect(r.x, r.y, r.h, r.h);
    s32 ox = (s32)(box.w * 0.2f), oy = (s32)(box.h * 0.2f);
    neko_core_ui_rect_t inner_box = neko_core_ui_rect(box.x + ox, box.y + oy, box.w - 2 * ox, box.h - 2 * oy);
    neko_core_ui_update_control(ctx, id, r, 0);

    s32 elementid = NEKO_CORE_UI_ELEMENT_BUTTON;
    neko_core_ui_style_t style = neko_default_val();
    style = ctx->focus == id   ? neko_core_ui_get_current_element_style(ctx, desc, elementid, 0x02)
            : ctx->hover == id ? neko_core_ui_get_current_element_style(ctx, desc, elementid, 0x01)
                               : neko_core_ui_get_current_element_style(ctx, desc, elementid, 0x00);
    ...
    /* handle click */
    if ((ctx->mouse_pressed == NEKO_CORE_UI_MOUSE_LEFT || (ctx->mouse_pressed && ~opt & NEKO_CORE_UI_OPT_LEFTCLICKONLY)) && ctx->focus == id) {
        res |= NEKO_CORE_UI_RES_CHANGE;
        *state = !*state;
    }

    /* draw */
    neko_core_ui_draw_control_frame(ctx, id, box, NEKO_CORE_UI_ELEMENT_INPUT, 0);
    if (*state) {
        // Draw in a filled rect
        neko_core_ui_draw_rect(ctx, inner_box, style.colors[NEKO_CORE_UI_COLOR_BACKGROUND]);
    }

    r = neko_core_ui_rect(r.x + box.w, r.y, r.w - box.w, r.h);
    neko_core_ui_draw_control_text(ctx, label, r, &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_TEXT][0], 0);
    return res;
}

NEKO_API_DECL s32 neko_core_ui_textbox_raw(neko_core_ui_context_t* ctx, char* buf, s32 bufsz, neko_core_ui_id id, neko_core_ui_rect_t rect, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    s32 res = 0;

    s32 elementid = NEKO_CORE_UI_ELEMENT_INPUT;
    neko_core_ui_style_t style = neko_default_val();
    neko_core_ui_animation_t* anim = neko_core_ui_get_animation(ctx, id, desc, elementid);
    ...
    neko_core_ui_update_control(ctx, id, rect, opt | NEKO_CORE_UI_OPT_HOLDFOCUS);
    ...
    // Textbox bg
    neko_core_ui_draw_control_frame(ctx, id, rect, NEKO_CORE_UI_ELEMENT_INPUT, opt);

    // Text and carret
    if (ctx->focus == id) {
        neko_core_ui_style_t* sp = &style;
        neko_color_t* color = &sp->colors[NEKO_CORE_UI_COLOR_CONTENT];
        s32 sx = sp->shadow_x;
        s32 sy = sp->shadow_y;
        neko_color_t* sc = &sp->colors[NEKO_CORE_UI_COLOR_SHADOW];
        neko_asset_ascii_font_t* font = sp->font;
        s32 textw = neko_core_ui_text_width(font, buf, -1);
        s32 texth = neko_core_ui_font_height(font);
        s32 ofx = (s32)(rect.w - sp->padding[NEKO_CORE_UI_PADDING_RIGHT] - textw - 1);
        s32 textx = (s32)(rect.x + neko_min(ofx, sp->padding[NEKO_CORE_UI_PADDING_LEFT]));
        s32 texty = (s32)(rect.y + (rect.h - texth) / 2);
        s32 cary = (s32)(rect.y + 1);
        neko_core_ui_push_clip_rect(ctx, rect);

        // Draw text
        neko_core_ui_draw_control_text(ctx, buf, rect, &style, opt);

        // Draw caret (control alpha based on frame)
        static bool on = true;
        static float ct = 0.f;
        if (~opt & NEKO_CORE_UI_OPT_NOCARET) {
            neko_vec2 pos = neko_v2(rect.x, rect.y);

            // Grab stylings
            const s32 padding_left = sp->padding[NEKO_CORE_UI_PADDING_LEFT];
            const s32 padding_top = sp->padding[NEKO_CORE_UI_PADDING_TOP];
            const s32 padding_right = sp->padding[NEKO_CORE_UI_PADDING_RIGHT];
            const s32 padding_bottom = sp->padding[NEKO_CORE_UI_PADDING_BOTTOM];
            ...
            neko_core_ui_rect_t cr = neko_core_ui_rect(pos.x + textw + padding_right, (f32)rect.y + 5.f, 1.f, (f32)rect.h - 10.f);

            if (ctx->last_focus_state == NEKO_CORE_UI_ELEMENT_STATE_ON_FOCUS) {
                on = true;
                ct = 0.f;
            }
            ct += 0.1f;
            ...
            col.a = on ? col.a : 0;
            neko_core_ui_draw_rect(ctx, cr, col);
        }
        ...
        neko_color_t* sc = &sp->colors[NEKO_CORE_UI_COLOR_SHADOW];
        s32 textw = neko_core_ui_text_width(font, buf, -1);
        s32 texth = neko_core_ui_text_height(font, buf, -1);
        s32 textx = (s32)(rect.x + sp->padding[NEKO_CORE_UI_PADDING_LEFT]);
        s32 texty = (s32)(rect.y + (rect.h - texth) / 2);
        neko_core_ui_push_clip_rect(ctx, rect);
        neko_core_ui_draw_control_text(ctx, buf, rect, &style, opt);
        neko_core_ui_pop_clip_rect(ctx);
...
static s32 neko_core_ui_number_textbox(neko_core_ui_context_t* ctx, neko_core_ui_real* value, neko_core_ui_rect_t r, neko_core_ui_id id, const neko_core_ui_selector_desc_t* desc) {
    if (ctx->mouse_pressed == NEKO_CORE_UI_MOUSE_LEFT && ctx->key_down & NEKO_CORE_UI_KEY_SHIFT && ctx->hover == id) {
        ctx->number_edit = id;
        neko_snprintf(ctx->number_edit_buf, NEKO_CORE_UI_MAX_FMT, NEKO_CORE_UI_REAL_FMT, *value);
    }
    if (ctx->number_edit == id) {
        // This is broken for some reason...
        s32 res = neko_core_ui_textbox_raw(ctx, ctx->number_edit_buf, sizeof(ctx->number_edit_buf), id, r, desc, 0);

        if (res & NEKO_CORE_UI_RES_SUBMIT || ctx->focus != id) {
...
NEKO_API_DECL s32 neko_core_ui_textbox_ex(neko_core_ui_context_t* ctx, char* buf, s32 bufsz, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    ...
    s32 res = 0;
    neko_core_ui_id id = neko_core_ui_get_id(ctx, &buf, sizeof(buf));
    s32 elementid = NEKO_CORE_UI_ELEMENT_INPUT;
    neko_core_ui_style_t style = neko_default_val();
    neko_core_ui_animation_t* anim = neko_core_ui_get_animation(ctx, id, desc, elementid);
    ...
    neko_core_ui_style_t* save = neko_core_ui_push_style(ctx, &style);
    neko_core_ui_rect_t r = neko_core_ui_layout_next(ctx);
    neko_core_ui_update_control(ctx, id, r, opt | NEKO_CORE_UI_OPT_HOLDFOCUS);
    res |= neko_core_ui_textbox_raw(ctx, buf, bufsz, id, r, desc, opt);
    neko_core_ui_pop_style(ctx, save);
...
NEKO_API_DECL s32 neko_core_ui_slider_ex(neko_core_ui_context_t* ctx, neko_core_ui_real* value, neko_core_ui_real low, neko_core_ui_real high, neko_core_ui_real step, const char* fmt,
    ...
    char buf[NEKO_CORE_UI_MAX_FMT + 1];
    neko_core_ui_rect_t thumb;
    s32 x, w, res = 0;
    neko_core_ui_real last = *value, v = last;
    neko_core_ui_id id = neko_core_ui_get_id(ctx, &value, sizeof(value));
    s32 elementid = NEKO_CORE_UI_ELEMENT_INPUT;
    neko_core_ui_style_t style = neko_default_val();
    neko_core_ui_animation_t* anim = neko_core_ui_get_animation(ctx, id, desc, elementid);
    s32 state = ctx->focus == id ? NEKO_CORE_UI_ELEMENT_STATE_FOCUS : ctx->hover == id ? NEKO_CORE_UI_ELEMENT_STATE_HOVER : NEKO_CORE_UI_ELEMENT_STATE_DEFAULT;
    ...
    if (anim) {
        ...
        style = neko_core_ui_get_current_element_style(ctx, desc, elementid, state);
    }

    // Temporary copy of style
    neko_core_ui_style_t* save = neko_core_ui_push_style(ctx, &style);
    neko_core_ui_rect_t base = neko_core_ui_layout_next(ctx);

    /* handle text input mode */
    if (neko_core_ui_number_textbox(ctx, &v, base, id, desc)) {
        return res;
    }

    /* handle normal mode */
    neko_core_ui_update_control(ctx, id, base, opt);

    /* handle input */
    if (ctx->focus == id && (ctx->mouse_down | ctx->mouse_pressed) == NEKO_CORE_UI_MOUSE_LEFT) {
    ...
    *value = v = neko_clamp(v, low, high);
    ...
    /* draw base */
    neko_core_ui_draw_control_frame(ctx, id, base, NEKO_CORE_UI_ELEMENT_INPUT, opt);

    /* draw control */
    w = style.thumb_size;  // Don't like this...
    x = (s32)((v - low) * (base.w - w) / (high - low));
    thumb = neko_core_ui_rect((f32)base.x + (f32)x, base.y, (f32)w, base.h);
    neko_core_ui_draw_control_frame(ctx, id, thumb, NEKO_CORE_UI_ELEMENT_BUTTON, opt);

    /* draw text    */
    style.colors[NEKO_CORE_UI_COLOR_BACKGROUND] = ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_TEXT][state].colors[NEKO_CORE_UI_COLOR_BACKGROUND];
    neko_snprintf(buf, NEKO_CORE_UI_MAX_FMT, fmt, v);
    neko_core_ui_draw_control_text(ctx, buf, base, &style, opt);  // oh...bg
...
NEKO_API_DECL s32 neko_core_ui_number_ex(neko_core_ui_context_t* ctx, neko_core_ui_real* value, neko_core_ui_real step, const char* fmt, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    ...
    neko_core_ui_style_t* save = neko_core_ui_push_style(ctx, &style);
    neko_core_ui_rect_t base = neko_core_ui_layout_next(ctx);
    neko_core_ui_real last = *value;

    /* handle text input mode */
    if (neko_core_ui_number_textbox(ctx, value, base, id, desc)) {
        neko_core_ui_pop_style(ctx, save);
        return res;
    }
    ...
    /* draw text    */
    neko_snprintf(buf, NEKO_CORE_UI_MAX_FMT, fmt, *value);
    neko_core_ui_draw_control_text(ctx, buf, base, &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_TEXT][0], opt);
...
static s32 _neko_core_ui_header(neko_core_ui_context_t* ctx, const char* label, s32 istreenode, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    ...
    active = (idx >= 0);
    expanded = (opt & NEKO_CORE_UI_OPT_EXPANDED) ? !active : active;
    r = neko_core_ui_layout_next(ctx);
    neko_core_ui_update_control(ctx, id, r, 0);

    /* handle click */
    active ^= (ctx->mouse_pressed == NEKO_CORE_UI_MOUSE_LEFT && ctx->focus == id);

    /* update pool ref */
    if (idx >= 0) {
        if (active) {
            neko_core_ui_pool_update(ctx, ctx->treenode_pool, idx);
        } else {
            memset(&ctx->treenode_pool[idx], 0, sizeof(neko_core_ui_pool_item_t));
        }

    } else if (active) {
        neko_core_ui_pool_init(ctx, ctx->treenode_pool, NEKO_CORE_UI_TREENODEPOOL_SIZE, id);
    }

    /* draw */
    if (istreenode) {
        if (ctx->hover == id) {
            neko_core_ui_draw_frame(ctx, r, &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_BUTTON][NEKO_CORE_UI_ELEMENT_STATE_HOVER]);
        }
    } else {
        neko_core_ui_draw_control_frame(ctx, id, r, NEKO_CORE_UI_ELEMENT_BUTTON, 0);
    }
    ...
    if (expanded) {
        ...
        neko_core_ui_draw_triangle(ctx, a, b, c, ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_TEXT][0x00].colors[NEKO_CORE_UI_COLOR_CONTENT]);
    } else {
        neko_vec2 a = {r.x + sz / 2.f, r.y + (r.h - sz) / 2.f};
    ...
    r.x += r.h - ctx->style->padding[NEKO_CORE_UI_PADDING_TOP];
    r.w -= r.h - ctx->style->padding[NEKO_CORE_UI_PADDING_BOTTOM];
    neko_core_ui_draw_control_text(ctx, label_tag, r, &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_TEXT][0x00], 0);

    if (id_tag) neko_core_ui_pop_id(ctx);

    return expanded ? NEKO_CORE_UI_RES_ACTIVE : 0;
}

NEKO_API_DECL s32 neko_core_ui_header_ex(neko_core_ui_context_t* ctx, const char* label, const neko_core_ui_selector_desc_t* desc, u64 opt) { return _neko_core_ui_header(ctx, label, 0, desc, opt); }
...
NEKO_API_DECL s32 neko_core_ui_treenode_begin_ex(neko_core_ui_context_t* ctx, const char* label, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    s32 res = _neko_core_ui_header(ctx, label, 1, desc, opt);
    if (res & NEKO_CORE_UI_RES_ACTIVE) {
        neko_core_ui_get_layout(ctx)->indent += ctx->style->indent;
        neko_core_ui_stack_push(ctx->id_stack, ctx->last_id);
    }
...
NEKO_API_DECL void neko_core_ui_tab_item_swap(neko_core_ui_context_t* ctx, neko_core_ui_container_t* cnt, s32 direction) {
    ...
    neko_core_ui_container_t* scnt = (neko_core_ui_container_t*)tab_bar->items[idx].data;

    neko_core_ui_tab_item_t* cti = &tab_bar->items[cnt->tab_item];
    neko_core_ui_tab_item_t* sti = &tab_bar->items[idx];
    neko_core_ui_tab_item_t tmp = *cti;
...
NEKO_API_DECL s32 neko_core_ui_window_begin_ex(neko_core_ui_context_t* ctx, const char* title, neko_core_ui_rect_t rect, bool* open, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    ...
    neko_core_ui_id id = neko_core_ui_get_id(ctx, title, strlen(title));
    neko_core_ui_container_t* cnt = neko_core_ui_get_container_ex(ctx, id, opt);
    ...
    s32 state = ctx->active_root == cnt ? NEKO_CORE_UI_ELEMENT_STATE_FOCUS : ctx->hover_root == cnt ? NEKO_CORE_UI_ELEMENT_STATE_HOVER : NEKO_CORE_UI_ELEMENT_STATE_DEFAULT;

    const float split_size = NEKO_CORE_UI_SPLIT_SIZE;

    neko_core_ui_stack_push(ctx->id_stack, id);

    // Get splits
    neko_core_ui_split_t* split = neko_core_ui_get_split(ctx, cnt);
    neko_core_ui_split_t* root_split = neko_core_ui_get_root_split(ctx, cnt);

    // Get root container
    neko_core_ui_container_t* root_cnt = neko_core_ui_get_root_container(ctx, cnt);

    // Cache rect
    if ((cnt->rect.w == 0.f || opt & NEKO_CORE_UI_OPT_FORCESETRECT || opt & NEKO_CORE_UI_OPT_FULLSCREEN || cnt->flags & NEKO_CORE_UI_WINDOW_FLAGS_FIRST_INIT) && new_frame) {
        if (opt & NEKO_CORE_UI_OPT_FULLSCREEN) {
            neko_vec2 fb = ctx->framebuffer_size;
            cnt->rect = neko_core_ui_rect(0, 0, fb.x, fb.y);
        ...
        cnt->flags = cnt->flags & ~NEKO_CORE_UI_WINDOW_FLAGS_FIRST_INIT;
    }
    neko_core_ui_begin_root_container(ctx, cnt, opt);
    rect = body = cnt->rect;
    cnt->opt = opt;

    if (opt & NEKO_CORE_UI_OPT_DOCKSPACE) {
        cnt->zindex = 0;
    }

    // If parent cannot move/resize, set to this opt as well
    if (root_cnt->opt & NEKO_CORE_UI_OPT_NOMOVE) {
        cnt->opt |= NEKO_CORE_UI_OPT_NOMOVE;
    }

    if (root_cnt->opt & NEKO_CORE_UI_OPT_NORESIZE) {
        cnt->opt |= NEKO_CORE_UI_OPT_NORESIZE;
    }

    // If in a tab view, then title has to be handled differently...
    neko_core_ui_tab_bar_t* tab_bar = cnt->tab_bar ? neko_slot_array_getp(ctx->tab_bars, cnt->tab_bar) : NULL;
    neko_core_ui_tab_item_t* tab_item = tab_bar ? &tab_bar->items[cnt->tab_item] : NULL;

    if (tab_item && tab_item) {
        if (tab_bar->focus == tab_item->idx) {
            cnt->flags |= NEKO_CORE_UI_WINDOW_FLAGS_VISIBLE;
            cnt->opt &= !NEKO_CORE_UI_OPT_NOINTERACT;
            cnt->opt &= !NEKO_CORE_UI_OPT_NOHOVER;
        } else {
            cnt->flags &= ~NEKO_CORE_UI_WINDOW_FLAGS_VISIBLE;
            cnt->opt |= NEKO_CORE_UI_OPT_NOINTERACT;
            cnt->opt |= NEKO_CORE_UI_OPT_NOHOVER;
    ...
    if (tab_bar) {
        for (u32 i = 0; i < tab_bar->size; ++i) {
            if (ctx->hover_root == (neko_core_ui_container_t*)tab_bar->items[i].data) {
                in_root = true;
                break;
            }
        }
    ...
    neko_core_ui_container_t* s_cnt = cnt;
    if (tab_bar && split) {
        for (u32 i = 0; i < tab_bar->size; ++i) {
            if (((neko_core_ui_container_t*)tab_bar->items[i].data)->split) {
                s_cnt = (neko_core_ui_container_t*)tab_bar->items[i].data;
            }
        }
    }
    ...
    if (split) {
        const neko_core_ui_style_t* cstyle = &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_CONTAINER][state];
        const neko_core_ui_rect_t* sr = &split->rect;
        const float ratio = split->ratio;
        ...
        switch (split->type) {
            case NEKO_CORE_UI_SPLIT_LEFT: {
                if (split->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].container == s_cnt) {
                    cnt->rect = neko_core_ui_rect(sr->x + shsz, sr->y + shsz, sr->w * ratio - 2.f * shsz, sr->h - 2.f * shsz);
                } else {
                    cnt->rect = neko_core_ui_rect(sr->x + sr->w * ratio + shsz, sr->y + shsz, sr->w * (1.f - ratio) - 2.f * shsz, sr->h - 2.f * shsz);
                }

            } break;

            case NEKO_CORE_UI_SPLIT_RIGHT: {
                if (split->children[NEKO_CORE_UI_SPLIT_NODE_PARENT].container == s_cnt) {
                    cnt->rect = neko_core_ui_rect(sr->x + shsz, sr->y + shsz, sr->w * (1.f - ratio) - 2.f * shsz, sr->h - 2.f * shsz);
                } else {
                    cnt->rect = neko_core_ui_rect(sr->x + sr->w * (1.f - ratio) + shsz, sr->y + shsz, sr->w * ratio - 2.f * shsz, sr->h - 2.f * shsz);
                }
            } break;

            case NEKO_CORE_UI_SPLIT_TOP: {
                if (split->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].container == s_cnt) {
                    cnt->rect = neko_core_ui_rect(sr->x + shsz, sr->y + shsz, sr->w - 2.f * shsz, sr->h * ratio - 2.f * shsz);
                } else {
                    cnt->rect = neko_core_ui_rect(sr->x + shsz, sr->y + sr->h * ratio + shsz, sr->w - 2.f * shsz, sr->h * (1.f - ratio) - 2.f * shsz);
                }
            } break;

            case NEKO_CORE_UI_SPLIT_BOTTOM: {
                if (split->children[NEKO_CORE_UI_SPLIT_NODE_CHILD].container == s_cnt) {
                    cnt->rect = neko_core_ui_rect(sr->x + shsz, sr->y + sr->h * (1.f - ratio) + shsz, sr->w - 2.f * shsz, sr->h * (ratio)-2.f * shsz);
                } else {
                    cnt->rect = neko_core_ui_rect(sr->x + shsz, sr->y + shsz, sr->w - 2.f * shsz, sr->h * (1.f - ratio) - 2.f * shsz);
                }
            } break;
        }
    }
    ...
    // Calculate movement
    if (~cnt->opt & NEKO_CORE_UI_OPT_NOTITLE && new_frame) {
        neko_core_ui_rect_t* rp = root_split ? &root_split->rect : &cnt->rect;

        // Cache rect
        neko_core_ui_rect_t tr = cnt->rect;
        tr.h = ctx->style->title_height;
        tr.x += split_size;
        neko_core_ui_id id = neko_core_ui_get_id(ctx, "!title", 6);
        neko_core_ui_update_control(ctx, id, tr, opt);

        // Need to move the entire thing
        if ((id == ctx->focus || id == ctx->hover) && ctx->mouse_down == NEKO_CORE_UI_MOUSE_LEFT) {
            // This log_lock id is what I need...

            ctx->active_root = cnt;

            if (tab_bar) {
                ctx->next_focus_root = (neko_core_ui_container_t*)(tab_bar->items[tab_bar->focus].data);
                neko_core_ui_bring_to_front(ctx, (neko_core_ui_container_t*)tab_bar->items[tab_bar->focus].data);
            ...
            if (root_split) {
                neko_core_ui_request_t req = neko_default_val();
                req.type = NEKO_CORE_UI_SPLIT_MOVE;
                req.split = root_split;
                neko_dyn_array_push(ctx->requests, req);
            } else {
                neko_core_ui_request_t req = neko_default_val();
                req.type = NEKO_CORE_UI_CNT_MOVE;
                req.cnt = cnt;
                neko_dyn_array_push(ctx->requests, req);
            }
        }
        ...
        id = neko_core_ui_get_id(ctx, "!split_tab", 10);
        const float hp = 0.8f;
        tr.x += split_size;
        float h = tr.h * hp;
        ...
        if (tab_item) {
            // Get tab bar
            neko_core_ui_rect_t* r = &tab_bar->rect;
        ...
        neko_core_ui_rect_t r = neko_core_ui_rect(tr.x + split_size, y, (f32)tw, h);

        neko_core_ui_update_control(ctx, id, r, opt);

        // Need to move the entire thing
        if ((id == ctx->hover || id == ctx->focus) && ctx->mouse_down == NEKO_CORE_UI_MOUSE_LEFT) {
            neko_core_ui_set_focus(ctx, id);
            ctx->next_focus_root = cnt;
            ctx->active_root = cnt;
            ...
            if (tab_item) {
                ...
                if (tab_bar->focus != tab_item->idx) {
                    neko_core_ui_request_t req = neko_default_val();
                    req.type = NEKO_CORE_UI_CNT_FOCUS;
                    req.cnt = cnt;
                    neko_dyn_array_push(ctx->requests, req);
                }
            }
    ...
    if (~root_cnt->opt & NEKO_CORE_UI_OPT_NOMOVE && ~cnt->opt & NEKO_CORE_UI_OPT_NOMOVE && ~cnt->opt & NEKO_CORE_UI_OPT_NOINTERACT && ~cnt->opt & NEKO_CORE_UI_OPT_NOHOVER && new_frame &&
        cnt->flags & NEKO_CORE_UI_WINDOW_FLAGS_VISIBLE) {
        // Cache rect
        neko_core_ui_rect_t br = cnt->rect;

        if (~cnt->opt & NEKO_CORE_UI_OPT_NOTITLE) {
            br.y += ctx->style->title_height;
            br.h -= ctx->style->title_height;
        }
        neko_core_ui_id id = neko_core_ui_get_id(ctx, "!body", 5);
        // neko_core_ui_update_control(ctx, id, br, (opt | NEKO_CORE_UI_OPT_NOSWITCHSTATE));

        // Need to move the entire thing
        if (ctx->hover_root == cnt && !ctx->focus_split && !ctx->focus && !ctx->lock_focus && !ctx->hover && ctx->mouse_down == NEKO_CORE_UI_MOUSE_LEFT) {
            ctx->active_root = cnt;
            ctx->next_focus_root = cnt;
            if (root_split) {
                neko_core_ui_request_t req = neko_default_val();
    ...
    if (~opt & NEKO_CORE_UI_OPT_NOTITLE) {
        neko_core_ui_rect_t tr = cnt->rect;
        tr.h = ctx->style->title_height;
        if (split) {
            const float sh = split_size * 0.5f;
    ...
    s32 zindex = INT32_MAX - 1;
    if (root_split) {
        neko_core_ui_get_split_lowest_zindex(ctx, root_split, &zindex);
        if (zindex == cnt->zindex) {
            neko_core_ui_style_t* style = &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_CONTAINER][state];
            neko_core_ui_draw_rect(ctx, root_split->rect, style->colors[NEKO_CORE_UI_COLOR_BACKGROUND]);
            neko_core_ui_draw_splits(ctx, root_split);
        }
    }

    // draw body frame
    if (~opt & NEKO_CORE_UI_OPT_NOFRAME && cnt->flags & NEKO_CORE_UI_WINDOW_FLAGS_VISIBLE) {
        neko_core_ui_style_t* style = &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_CONTAINER][state];
        ...
        neko_core_ui_draw_rect(ctx, body, style->colors[NEKO_CORE_UI_COLOR_BACKGROUND]);

        // draw border (get root cnt and check state of that)
        if (split) {
            s32 root_state = ctx->active_root == root_cnt ? NEKO_CORE_UI_ELEMENT_STATE_FOCUS : ctx->hover_root == root_cnt ? NEKO_CORE_UI_ELEMENT_STATE_HOVER : NEKO_CORE_UI_ELEMENT_STATE_DEFAULT;

            bool share_split = ctx->active_root && neko_core_ui_get_root_container(ctx, ctx->active_root) == root_cnt ? true : false;

            // Have to look and see if hovered root shares split...
            neko_core_ui_style_t* root_style = style;
            if (share_split) {
                root_style = &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_CONTAINER][NEKO_CORE_UI_ELEMENT_STATE_FOCUS];
            } else {
            ...
            if (~opt & NEKO_CORE_UI_OPT_NOBORDER && style->colors[NEKO_CORE_UI_COLOR_BORDER].a) {
                neko_core_ui_draw_box(ctx, neko_core_ui_expand_rect(cnt->rect, (s16*)style->border_width), (s16*)style->border_width, style->colors[NEKO_CORE_UI_COLOR_BORDER]);
            }
        }
    }

    if (split && ~opt & NEKO_CORE_UI_OPT_NOCLIP && cnt->flags & NEKO_CORE_UI_WINDOW_FLAGS_VISIBLE) {
        s16 exp[] = {1, 1, 1, 1};
        neko_core_ui_push_clip_rect(ctx, neko_core_ui_expand_rect(cnt->rect, exp));
    }
    ...
    // do title bar
    if (~opt & NEKO_CORE_UI_OPT_NOTITLE) {
        neko_core_ui_style_t* cstyle = &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_CONTAINER][state];
        neko_core_ui_rect_t tr = cnt->rect;
        tr.h = ctx->style->title_height;
        if (split) {
            const float sh = split_size * 0.5f;
        ...
        if (tab_bar) {
            ...
            {
                for (u32 i = 0; i < tab_bar->size; ++i) {
                    if (cnt->zindex > ((neko_core_ui_container_t*)(tab_bar->items[i].data))->zindex) {
                        lowest = false;
                        break;
                    }
                }
                if (lowest) {
                    neko_core_ui_draw_frame(ctx, tr, &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_PANEL][0x00]);
                    // neko_core_ui_draw_box(ctx, neko_core_ui_expand_rect(tr, (s16*)cstyle->border_width), (s16*)cstyle->border_width, cstyle->colors[NEKO_CORE_UI_COLOR_BORDER]);
                }
            }
        }
            ...
            neko_core_ui_draw_frame(ctx, tr, &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_PANEL][0x00]);
            // neko_core_ui_draw_box(ctx, neko_core_ui_expand_rect(tr, (s16*)cstyle->border_width), cstyle->border_width, cstyle->colors[NEKO_CORE_UI_COLOR_BORDER]);
        }
        ...
        // Draw tab control
        {
            ...
            if (in_root && neko_core_ui_rect_overlaps_vec2(r, ctx->mouse_pos)) {
                neko_immediate_draw_t* dl = &ctx->overlay_draw_list;
            ...
            if (!other_root_active && hovered && ctx->mouse_down == NEKO_CORE_UI_MOUSE_LEFT && !ctx->lock_focus) {
                // This is an issue...
                neko_core_ui_set_focus(ctx, id);
                ctx->lock_focus = id;
            ...
            if (!other_root_active && ctx->mouse_down == NEKO_CORE_UI_MOUSE_LEFT && ctx->focus == id) {
                if (ctx->mouse_pos.x < r.x) {
                    neko_core_ui_request_t req = neko_default_val();
                    req.type = NEKO_CORE_UI_TAB_SWAP_LEFT;
                    req.cnt = cnt;
                    neko_dyn_array_push(ctx->requests, req);
                }
                if (ctx->mouse_pos.x > r.x + r.w) {
                    neko_core_ui_request_t req = neko_default_val();
                    req.type = NEKO_CORE_UI_TAB_SWAP_RIGHT;
                    req.cnt = cnt;
                    neko_dyn_array_push(ctx->requests, req);
                }
            }
            ...
            neko_color_t def = ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_BUTTON][0x00].colors[NEKO_CORE_UI_COLOR_BACKGROUND];
            neko_color_t hov = ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_BUTTON][0x01].colors[NEKO_CORE_UI_COLOR_BACKGROUND];
            neko_color_t act = ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_BUTTON][0x02].colors[NEKO_CORE_UI_COLOR_BACKGROUND];
            neko_color_t inactive = neko_color(10, 10, 10, 50);

            s16 exp[] = {1, 1, 1, 1};
            neko_core_ui_push_clip_rect(ctx, neko_core_ui_expand_rect(cnt->rect, exp));

            neko_core_ui_push_clip_rect(ctx, r);

            neko_core_ui_draw_rect(ctx, r, id == ctx->focus ? act : hovered ? hov : tab_focus ? def : inactive);
            neko_core_ui_draw_control_text(ctx, label_tag, r, &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_CONTAINER][state], opt);
        ...
        // do `close` button
        /*
        if (~opt & NEKO_CORE_UI_OPT_NOCLOSE && false)
        {
            neko_core_ui_id id = neko_core_ui_get_id(ctx, "!close", 6);
            neko_core_ui_rect_t r = neko_core_ui_rect(tr.x + tr.w - tr.h, tr.y, tr.h, tr.h);
            tr.w -= r.w;
            neko_core_ui_draw_icon(ctx, NEKO_CORE_UI_ICON_CLOSE, r, ctx->style->colors[NEKO_CORE_UI_COLOR_TITLETEXT]);
            neko_core_ui_update_control(ctx, id, r, opt);
            if (ctx->mouse_pressed == NEKO_CORE_UI_MOUSE_LEFT && id == ctx->focus)
            {
                cnt->open = 0;
            }
        }
    ...
    if (opt & NEKO_CORE_UI_OPT_AUTOSIZE && !split) {
        /*
        neko_core_ui_rect_t r = neko_core_ui_get_layout(ctx)->body;
    ...
    // Draw border
    if (~opt & NEKO_CORE_UI_OPT_NOFRAME && cnt->flags & NEKO_CORE_UI_WINDOW_FLAGS_VISIBLE) {
        const int* w = (int*)ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_CONTAINER][0x00].border_width;
        const neko_color_t* bc = &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_CONTAINER][0x00].colors[NEKO_CORE_UI_COLOR_BORDER];
        // neko_core_ui_draw_box(ctx, neko_core_ui_expand_rect(cnt->rect, w), w, *bc);
    }

    neko_core_ui_push_container_body(ctx, cnt, body, desc, opt);

    /* close if this is a popup window and elsewhere was clicked */
    if (opt & NEKO_CORE_UI_OPT_POPUP && ctx->mouse_pressed && ctx->hover_root != cnt) {
        cnt->open = 0;
    }

    if (~opt & NEKO_CORE_UI_OPT_NOCLIP) {
        if (cnt->flags & NEKO_CORE_UI_WINDOW_FLAGS_VISIBLE) {
            neko_core_ui_push_clip_rect(ctx, cnt->body);
        } else {
            neko_core_ui_push_clip_rect(ctx, neko_core_ui_rect(0, 0, 0, 0));
        }
    }

    return NEKO_CORE_UI_RES_ACTIVE;
}
...
NEKO_API_DECL void neko_core_ui_window_end(neko_core_ui_context_t* ctx) {
    ...
    // Pop clip for rect
    if (~cnt->opt & NEKO_CORE_UI_OPT_NOCLIP) {
        neko_core_ui_pop_clip_rect(ctx);
    }

    if (~cnt->opt & NEKO_CORE_UI_OPT_NOCLIP) {
        neko_core_ui_push_clip_rect(ctx, cnt->rect);
    }

    // do `resize` handle
    if (~cnt->opt & NEKO_CORE_UI_OPT_NORESIZE && ~root_cnt->opt & NEKO_CORE_UI_OPT_NORESIZE && new_frame && ~cnt->opt & NEKO_CORE_UI_OPT_DOCKSPACE) {
        s32 sz = ctx->style->title_height;
        neko_core_ui_id id = neko_core_ui_get_id(ctx, "!resize", 7);
        neko_core_ui_rect_t r = neko_core_ui_rect(cnt->rect.x + cnt->rect.w - (f32)sz, cnt->rect.y + cnt->rect.h - (f32)sz, (f32)sz, (f32)sz);
        neko_core_ui_update_control(ctx, id, r, opt);
        if (id == ctx->focus && ctx->mouse_down == NEKO_CORE_UI_MOUSE_LEFT) {
            ctx->active_root = cnt;
            if (root_split) {
                neko_core_ui_request_t req = neko_default_val();
                req.type = NEKO_CORE_UI_SPLIT_RESIZE_SE;
                req.split = root_split;
                neko_dyn_array_push(ctx->requests, req);
            } else {
        ...
        neko_color_t col = ctx->focus == id   ? ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_BUTTON][0x02].colors[NEKO_CORE_UI_COLOR_BACKGROUND]
                           : ctx->hover == id ? ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_BUTTON][0x01].colors[NEKO_CORE_UI_COLOR_BACKGROUND]
                                              : ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_BUTTON][0x00].colors[NEKO_CORE_UI_COLOR_BACKGROUND];

        neko_core_ui_draw_rect(ctx, neko_core_ui_rect(r.x + w * grid - o, r.y + h * (grid - 2) - o, w - m, h - m), col);
        neko_core_ui_draw_rect(ctx, neko_core_ui_rect(r.x + w * grid - o, r.y + h * (grid - 1) - o, w - m, h - m), col);
        neko_core_ui_draw_rect(ctx, neko_core_ui_rect(r.x + w * (grid - 1) - o, r.y + h * (grid - 1) - o, w - m, h - m), col);
        neko_core_ui_draw_rect(ctx, neko_core_ui_rect(r.x + w * grid - o, r.y + h * grid - o, w - m, h - m), col);
        neko_core_ui_draw_rect(ctx, neko_core_ui_rect(r.x + w * (grid - 1) - o, r.y + h * grid - o, w - m, h - m), col);
        neko_core_ui_draw_rect(ctx, neko_core_ui_rect(r.x + w * (grid - 2) - o, r.y + h * grid - o, w - m, h - m), col);
    }
    ...
    // draw shadow
    if (~opt & NEKO_CORE_UI_OPT_NOFRAME && cnt->flags & NEKO_CORE_UI_WINDOW_FLAGS_VISIBLE) {
        neko_core_ui_rect_t* r = &cnt->rect;
        u32 ssz = (u32)(split ? NEKO_CORE_UI_SPLIT_SIZE : 5);

        neko_core_ui_draw_rect(ctx, neko_core_ui_rect(r->x, r->y + r->h, r->w + 1, 1), ctx->style->colors[NEKO_CORE_UI_COLOR_SHADOW]);
        neko_core_ui_draw_rect(ctx, neko_core_ui_rect(r->x, r->y + r->h, r->w + (f32)ssz, (f32)ssz), ctx->style->colors[NEKO_CORE_UI_COLOR_SHADOW]);
        neko_core_ui_draw_rect(ctx, neko_core_ui_rect(r->x + r->w, r->y, 1, r->h), ctx->style->colors[NEKO_CORE_UI_COLOR_SHADOW]);
        neko_core_ui_draw_rect(ctx, neko_core_ui_rect(r->x + r->w, r->y, (f32)ssz, r->h), ctx->style->colors[NEKO_CORE_UI_COLOR_SHADOW]);
    }

#define _gui_window_resize_ctrl(ID, RECT, MOUSE, SPLIT_TYPE, MOD_KEY, ...) \
    do {                                                                   \
        if (ctx->key_down == (MOD_KEY)) {                                  \
            neko_core_ui_id _ID = (ID);                                    \
            neko_core_ui_rect_t _R = (RECT);                               \
            neko_core_ui_update_control(ctx, (ID), _R, opt);               \
                                                                           \
            if (_ID == ctx->hover || _ID == ctx->focus) {                  \
                neko_core_ui_draw_rect(ctx, _R, NEKO_COLOR_WHITE);         \
            }                                                              \
                                                                           \
    ...
    if (~opt & NEKO_CORE_UI_OPT_NORESIZE && cnt->flags & NEKO_CORE_UI_WINDOW_FLAGS_VISIBLE) {
        // Cache main rect
        neko_core_ui_rect_t* r = root_split ? &root_split->rect : &cnt->rect;
        neko_core_ui_rect_t* cr = &cnt->rect;

        const float border_ratio = 0.333f;

        if (split) {
            _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!split_w", 8), neko_core_ui_rect(cr->x, cr->y + cr->h * border_ratio, cr->w * border_ratio, cr->h * (1.f - 2.f * border_ratio)),
                                    NEKO_CORE_UI_MOUSE_RIGHT, NEKO_CORE_UI_SPLIT_RESIZE_INVALID, NEKO_CORE_UI_KEY_CTRL, {});

            _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!split_e", 8),
                                    neko_core_ui_rect(cr->x + cr->w * (1.f - border_ratio), cr->y + cr->h * border_ratio, cr->w * border_ratio, cr->h * (1.f - 2.f * border_ratio)),
                                    NEKO_CORE_UI_MOUSE_LEFT, NEKO_CORE_UI_SPLIT_RESIZE_INVALID, NEKO_CORE_UI_KEY_CTRL, {});

            _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!split_n", 8), neko_core_ui_rect(cr->x + cr->w * border_ratio, cr->y, cr->w * (1.f - 2.f * border_ratio), cr->h * border_ratio),
                                    NEKO_CORE_UI_MOUSE_LEFT, NEKO_CORE_UI_SPLIT_RESIZE_INVALID, NEKO_CORE_UI_KEY_CTRL, {});

            _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!split_s", 8),
                                    neko_core_ui_rect(cr->x + cr->w * border_ratio, cr->y + cr->h * (1.f - border_ratio), cr->w * (1.f - 2.f * border_ratio), cr->h * border_ratio),
                                    NEKO_CORE_UI_MOUSE_LEFT, NEKO_CORE_UI_SPLIT_RESIZE_INVALID, NEKO_CORE_UI_KEY_CTRL, {});

            _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!split_se", 9),
                                    neko_core_ui_rect(cr->x + cr->w - cr->w * border_ratio, cr->y + cr->h * (1.f - border_ratio), cr->w * border_ratio, cr->h * border_ratio), NEKO_CORE_UI_MOUSE_LEFT,
                                    NEKO_CORE_UI_SPLIT_RESIZE_INVALID, NEKO_CORE_UI_KEY_CTRL, {});

            _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!split_ne", 9), neko_core_ui_rect(cr->x + cr->w - cr->w * border_ratio, cr->y, cr->w * border_ratio, cr->h * border_ratio),
                                    NEKO_CORE_UI_MOUSE_LEFT, NEKO_CORE_UI_SPLIT_RESIZE_INVALID, NEKO_CORE_UI_KEY_CTRL, {});

            _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!split_nw", 9), neko_core_ui_rect(cr->x, cr->y, cr->w * border_ratio, cr->h * border_ratio), NEKO_CORE_UI_MOUSE_LEFT,
                                    NEKO_CORE_UI_SPLIT_RESIZE_INVALID, NEKO_CORE_UI_KEY_CTRL, {});

            _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!split_sw", 9), neko_core_ui_rect(cr->x, cr->y + cr->h - cr->h * border_ratio, cr->w * border_ratio, cr->h * border_ratio),
                                    NEKO_CORE_UI_MOUSE_LEFT, NEKO_CORE_UI_SPLIT_RESIZE_INVALID, NEKO_CORE_UI_KEY_CTRL, {});
        }

        _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!res_w", 6), neko_core_ui_rect(r->x, r->y + r->h * border_ratio, r->w * border_ratio, r->h * (1.f - 2.f * border_ratio)),
                                NEKO_CORE_UI_MOUSE_LEFT, NEKO_CORE_UI_SPLIT_RESIZE_W, NEKO_CORE_UI_KEY_ALT, {
                                    float w = r->w;
                                    float max_x = r->x + r->w;
                                    r->w = neko_max(r->w - ctx->mouse_delta.x, 40);
                                    if (fabsf(r->w - w) > 0.f) {
        ...
        _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!res_e", 6),
                                neko_core_ui_rect(r->x + r->w * (1.f - border_ratio), r->y + r->h * border_ratio, r->w * border_ratio, r->h * (1.f - 2.f * border_ratio)), NEKO_CORE_UI_MOUSE_LEFT,
                                NEKO_CORE_UI_SPLIT_RESIZE_E, NEKO_CORE_UI_KEY_ALT, { r->w = neko_max(r->w + ctx->mouse_delta.x, 40); });

        _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!res_n", 6), neko_core_ui_rect(r->x + r->w * border_ratio, r->y, r->w * (1.f - 2.f * border_ratio), r->h * border_ratio),
                                NEKO_CORE_UI_MOUSE_LEFT, NEKO_CORE_UI_SPLIT_RESIZE_N, NEKO_CORE_UI_KEY_ALT, {
                                    float h = r->h;
                                    float max_y = h + r->y;
                                    r->h = neko_max(r->h - ctx->mouse_delta.y, 40);
                                    if (fabsf(r->h - h) > 0.f) {
        ...
        _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!res_s", 6),
                                neko_core_ui_rect(r->x + r->w * border_ratio, r->y + r->h * (1.f - border_ratio), r->w * (1.f - 2.f * border_ratio), r->h * border_ratio), NEKO_CORE_UI_MOUSE_LEFT,
                                NEKO_CORE_UI_SPLIT_RESIZE_S, NEKO_CORE_UI_KEY_ALT, { r->h = neko_max(r->h + ctx->mouse_delta.y, 40); });

        _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!res_se", 7),
                                neko_core_ui_rect(r->x + r->w - r->w * border_ratio, r->y + r->h * (1.f - border_ratio), r->w * border_ratio, r->h * border_ratio), NEKO_CORE_UI_MOUSE_LEFT,
                                NEKO_CORE_UI_SPLIT_RESIZE_SE, NEKO_CORE_UI_KEY_ALT, {
                                    r->w = neko_max(r->w + ctx->mouse_delta.x, 40);
                                    r->h = neko_max(r->h + ctx->mouse_delta.y, 40);
                                });

        _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!res_ne", 7), neko_core_ui_rect(r->x + r->w - r->w * border_ratio, r->y, r->w * border_ratio, r->h * border_ratio), NEKO_CORE_UI_MOUSE_LEFT,
                                NEKO_CORE_UI_SPLIT_RESIZE_NE, NEKO_CORE_UI_KEY_ALT, {
                                    r->w = neko_max(r->w + ctx->mouse_delta.x, 40);
                                    float h = r->h;
                                    float max_y = h + r->y;
                                    r->h = neko_max(r->h - ctx->mouse_delta.y, 40);
        ...
        _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!res_nw", 7), neko_core_ui_rect(r->x, r->y, r->w * border_ratio, r->h * border_ratio), NEKO_CORE_UI_MOUSE_LEFT, NEKO_CORE_UI_SPLIT_RESIZE_NW,
                                NEKO_CORE_UI_KEY_ALT, {
                                    float h = r->h;
                                    float max_y = h + r->y;
                                    r->h = neko_max(r->h - ctx->mouse_delta.y, 40);
                                    if (fabsf(r->h - h) > 0.f) {
        ...
        _gui_window_resize_ctrl(neko_core_ui_get_id(ctx, "!res_sw", 7), neko_core_ui_rect(r->x, r->y + r->h - r->h * border_ratio, r->w * border_ratio, r->h * border_ratio), NEKO_CORE_UI_MOUSE_LEFT,
                                NEKO_CORE_UI_SPLIT_RESIZE_SW, NEKO_CORE_UI_KEY_ALT, {
                                    float h = r->h;
                                    float max_y = h + r->y;
                                    r->h = neko_max(r->h + ctx->mouse_delta.y, 40);
        ...
        _gui_window_resize_ctrl(
            neko_core_ui_get_id(ctx, "!res_c", 5),
            neko_core_ui_rect(r->x + r->w * border_ratio, r->y + r->h * border_ratio, r->w * border_ratio, r->h * border_ratio),
    ...
    if (can_dock && ~cnt->opt & NEKO_CORE_UI_OPT_NODOCK && ctx->focus_root && ctx->focus_root != cnt &&
        neko_core_ui_rect_overlaps_vec2(cnt->rect, ctx->mouse_pos) &&  // This is the incorrect part - need to check if this container isn't being overlapped by another
        ctx->mouse_down == NEKO_CORE_UI_MOUSE_LEFT && ~cnt->opt & NEKO_CORE_UI_OPT_NOHOVER && cnt->flags & NEKO_CORE_UI_WINDOW_FLAGS_VISIBLE) {
        neko_core_ui_split_t* focus_split = neko_core_ui_get_root_split(ctx, ctx->focus_root);
        neko_core_ui_split_t* cnt_split = neko_core_ui_get_root_split(ctx, cnt);
    ...
    // Pop root container
    neko_core_ui_root_container_end(ctx);
}
...
NEKO_API_DECL void neko_core_ui_popup_open(neko_core_ui_context_t* ctx, const char* name) {
    ...
    cnt->rect = neko_core_ui_rect(ctx->mouse_pos.x, ctx->mouse_pos.y, 100, 100);
    cnt->open = 1;
    neko_core_ui_bring_to_front(ctx, cnt);
}
...
NEKO_API_DECL s32 neko_core_ui_popup_begin_ex(neko_core_ui_context_t* ctx, const char* name, neko_core_ui_rect_t r, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    opt |= (NEKO_CORE_UI_OPT_POPUP | NEKO_CORE_UI_OPT_NODOCK | NEKO_CORE_UI_OPT_CLOSED);
    return neko_core_ui_window_begin_ex(ctx, name, r, NULL, NULL, opt);
}

NEKO_API_DECL void neko_core_ui_popup_end(neko_core_ui_context_t* ctx) { neko_core_ui_window_end(ctx); }
...
NEKO_API_DECL void neko_core_ui_panel_begin_ex(neko_core_ui_context_t* ctx, const char* name, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    ...
    neko_core_ui_push_id(ctx, name, strlen(name));
    cnt = neko_core_ui_get_container_ex(ctx, ctx->last_id, opt);
    cnt->rect = neko_core_ui_layout_next(ctx);

    const neko_core_ui_id id = neko_core_ui_get_id(ctx, name, strlen(name));
    ...
    if (~opt & NEKO_CORE_UI_OPT_NOFRAME) {
        neko_core_ui_draw_frame(ctx, cnt->rect, &style);
    }

    // Need a way to push/pop syles temp styles
    neko_core_ui_stack_push(ctx->container_stack, cnt);
    neko_core_ui_push_container_body(ctx, cnt, cnt->rect, desc, opt);
    neko_core_ui_push_clip_rect(ctx, cnt->body);
}
...
static u8 uint8_slider(neko_core_ui_context_t* ctx, unsigned char* value, int low, int high, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    static float tmp;
    neko_core_ui_push_id(ctx, &value, sizeof(value));
    tmp = (float)*value;
    int res = neko_core_ui_slider_ex(ctx, &tmp, (neko_core_ui_real)low, (neko_core_ui_real)high, 0, "%.0f", desc, opt);
    *value = (u8)tmp;
    neko_core_ui_pop_id(ctx);
    return res;
}

static s32 int32_slider(neko_core_ui_context_t* ctx, s32* value, s32 low, s32 high, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    static float tmp;
    neko_core_ui_push_id(ctx, &value, sizeof(value));
    tmp = (float)*value;
    int res = neko_core_ui_slider_ex(ctx, &tmp, (neko_core_ui_real)low, (neko_core_ui_real)high, 0, "%.0f", desc, opt);
...
static s16 int16_slider(neko_core_ui_context_t* ctx, s16* value, s32 low, s32 high, const neko_core_ui_selector_desc_t* desc, u64 opt) {
    static float tmp;
    neko_core_ui_push_id(ctx, &value, sizeof(value));
    tmp = (float)*value;
    int res = neko_core_ui_slider_ex(ctx, &tmp, (neko_core_ui_real)low, (neko_core_ui_real)high, 0, "%.0f", desc, opt);
...
typedef struct {
    b32 hit;
    neko_vec3 point;
} neko_core_ui_gizmo_line_intersection_result_t;

enum { NEKO_CORE_UI_AXIS_RIGHT = 0x01, NEKO_CORE_UI_AXIS_UP, NEKO_CORE_UI_AXIS_FORWARD };
...
typedef struct {
    neko_core_ui_axis_t right;
    neko_core_ui_axis_t up;
    neko_core_ui_axis_t forward;
...
#define NEKO_CORE_UI_GIZMO_AXIS_DEFINE_SCALE(MEMBER, OFFSET, DEG, AXIS)                             \
    ...
    NEKO_CORE_UI_GIZMO_AXIS_DEFINE_SCALE(right, neko_v3(-off, 0.f, 0.f), 90.f, NEKO_ZAXIS);
    NEKO_CORE_UI_GIZMO_AXIS_DEFINE_SCALE(up, neko_v3(0.f, off, 0.f), 0.f, NEKO_YAXIS);
    NEKO_CORE_UI_GIZMO_AXIS_DEFINE_SCALE(forward, neko_v3(0.f, 0.f, off), 90.f, NEKO_XAXIS);
...
#define NEKO_CORE_UI_GIZMO_AXIS_DEFINE_TRANSLATE(MEMBER, OFFSET, DEG, AXIS)                         \
    ...
    const float off = 0.6f;
    NEKO_CORE_UI_GIZMO_AXIS_DEFINE_TRANSLATE(right, neko_v3(-off, 0.f, 0.f), 90.f, NEKO_ZAXIS);
    NEKO_CORE_UI_GIZMO_AXIS_DEFINE_TRANSLATE(up, neko_v3(0.f, off, 0.f), 0.f, NEKO_YAXIS);
    NEKO_CORE_UI_GIZMO_AXIS_DEFINE_TRANSLATE(forward, neko_v3(0.f, 0.f, off), 90.f, NEKO_XAXIS);
...
#define NEKO_CORE_UI_GIZMO_AXIS_DEFINE_ROTATE(MEMBER, OFFSET, DEG, AXIS)                          \
    ...
    NEKO_CORE_UI_GIZMO_AXIS_DEFINE_ROTATE(right, neko_v3(0.f, 0.f, 0.f), 90.f, NEKO_YAXIS);
    NEKO_CORE_UI_GIZMO_AXIS_DEFINE_ROTATE(up, neko_v3(0.f, 0.f, 0.f), -90.f, NEKO_XAXIS);
    NEKO_CORE_UI_GIZMO_AXIS_DEFINE_ROTATE(forward, neko_v3(0.f, 0.f, 0.f), 0.f, NEKO_ZAXIS);
...
typedef struct {
    ...
    neko_core_ui_rect_t viewport;
    neko_core_ui_gizmo_line_intersection_result_t li;
} neko_gizmo_desc_t;

static void neko_core_ui_gizmo_render(neko_core_ui_context_t* ctx, neko_core_ui_customcommand_t* cmd) {
    ...
    neko_core_ui_rect_t clip = cmd->clip;
    neko_core_ui_rect_t viewport = desc->viewport;
    neko_camera_t cam = desc->camera;
    const u16 segments = 4;
    const neko_core_ui_gizmo_line_intersection_result_t* li = &desc->li;
...
#define NEKO_CORE_UI_GIZMO_AXIS_ROTATE(ID, AXIS, COLOR)                                                                                                 \
    ...
    switch (desc->op) {
        case NEKO_CORE_UI_GIZMO_TRANSLATE: {
            NEKO_CORE_UI_GIZMO_AXIS_TRANSLATE("#gizmo_trans_right", right, NEKO_COLOR_RED);
            NEKO_CORE_UI_GIZMO_AXIS_TRANSLATE("#gizmo_trans_up", up, NEKO_COLOR_GREEN);
            NEKO_CORE_UI_GIZMO_AXIS_TRANSLATE("#gizmo_trans_forward", forward, NEKO_COLOR_BLUE);
        } break;

        case NEKO_CORE_UI_GIZMO_SCALE: {
            NEKO_CORE_UI_GIZMO_AXIS_SCALE("#gizmo_scale_right", right, NEKO_COLOR_RED);
            NEKO_CORE_UI_GIZMO_AXIS_SCALE("#gizmo_scale_up", up, NEKO_COLOR_GREEN);
            NEKO_CORE_UI_GIZMO_AXIS_SCALE("#gizmo_scale_forward", forward, NEKO_COLOR_BLUE);
        } break;

        case NEKO_CORE_UI_GIZMO_ROTATE: {
            neko_core_ui_id id_r = neko_core_ui_get_id_hash(ctx, "#gizmo_rotate_right", strlen("#gizmo_rotate_right"), cmd->hash);
            neko_core_ui_id id_u = neko_core_ui_get_id_hash(ctx, "#gizmo_rotate_up", strlen("#gizmo_rotate_up"), cmd->hash);
            neko_core_ui_id id_f = neko_core_ui_get_id_hash(ctx, "#gizmo_rotate_forward", strlen("#gizmo_rotate_forward"), cmd->hash);
...
static neko_core_ui_gizmo_line_intersection_result_t neko_core_ui_gizmo_get_line_intersection(const neko_vqs* model, neko_vec3 axis_a, neko_vec3 axis_b, neko_vec3 axis_c, const neko_camera_t* camera,
...
NEKO_API_DECL s32 neko_core_ui_gizmo(neko_core_ui_context_t* ctx, neko_camera_t* camera, neko_vqs* model, neko_core_ui_rect_t viewport, bool invert_view_y, float snap, s32 op, s32 mode, u64 opt) {
    ...
    const neko_vec2 fbs = ctx->framebuffer_size;
    const float t = neko_platform_elapsed_time();
    const bool in_hover_root = neko_core_ui_in_hover_root(ctx);

    neko_immediate_draw_t* dl = &ctx->overlay_draw_list;

    // This doesn't actually work for the clip...
    neko_core_ui_rect_t clip = viewport;
    if (invert_view_y) {
    ...
    // Check for scale and bias
    s32 flags = 0x00;
    if (flags & NEKO_CORE_UI_HINT_FLAG_NO_SCALE_BIAS_MOUSE) {
    ...
    */
    ...
    desc.xform.rotation = (mode == NEKO_CORE_UI_TRANSFORM_LOCAL || op == NEKO_CORE_UI_GIZMO_SCALE) ? model->rotation : neko_quat_default();  // This depends on the mode (local/world)
...
#define UPDATE_GIZMO_CONTROL(ID, RAY, SHAPE, MODEL, FUNC, CAP_SHAPE, CAP_MODEL, CAP_FUNC, INFO)                                                               \
    ...
    switch (op) {
        case NEKO_CORE_UI_GIZMO_TRANSLATE: {
            ...
            neko_core_ui_id id_r = neko_core_ui_get_id(ctx, "#gizmo_trans_right", strlen("#gizmo_trans_right"));
            neko_core_ui_id id_u = neko_core_ui_get_id(ctx, "#gizmo_trans_up", strlen("#gizmo_trans_up"));
            neko_core_ui_id id_f = neko_core_ui_get_id(ctx, "#gizmo_trans_forward", strlen("#gizmo_trans_forward"));
            ...
            // Control
            if (ctx->focus == id_r) {
                desc.li = neko_core_ui_gizmo_get_line_intersection(&desc.xform, NEKO_XAXIS, NEKO_YAXIS, NEKO_ZAXIS, camera, &ray, neko_v3s(0.f), true, false);
                ...
                desc.li = neko_core_ui_gizmo_get_line_intersection(&desc.xform, NEKO_YAXIS, NEKO_XAXIS, NEKO_ZAXIS, camera, &ray, neko_v3s(0.f), true, false);
                ...
                desc.li = neko_core_ui_gizmo_get_line_intersection(&desc.xform, NEKO_ZAXIS, NEKO_XAXIS, NEKO_YAXIS, camera, &ray, neko_v3s(0.f), true, false);
        ...
        case NEKO_CORE_UI_GIZMO_SCALE: {
            ...
            neko_core_ui_id id_r = neko_core_ui_get_id(ctx, "#gizmo_scale_right", strlen("#gizmo_scale_right"));
            neko_core_ui_id id_u = neko_core_ui_get_id(ctx, "#gizmo_scale_up", strlen("#gizmo_scale_up"));
            neko_core_ui_id id_f = neko_core_ui_get_id(ctx, "#gizmo_scale_forward", strlen("#gizmo_scale_forward"));
...
#define UPDATE_GIZMO_CONTROL_ROTATE(ID, RAY, SHAPE, MODEL, AXIS, INFO)                                                                                        \
    do {                                                                                                                                                      \
        ...
        case NEKO_CORE_UI_GIZMO_ROTATE: {
            ...
            neko_core_ui_id id_r = neko_core_ui_get_id(ctx, "#gizmo_rotate_right", strlen("#gizmo_rotate_right"));
            neko_core_ui_id id_u = neko_core_ui_get_id(ctx, "#gizmo_rotate_up", strlen("#gizmo_rotate_up"));
            neko_core_ui_id id_f = neko_core_ui_get_id(ctx, "#gizmo_rotate_forward", strlen("#gizmo_rotate_forward"));
            ...
            if (ctx->focus == id_r) {
                ...
                if (desc.li.hit) {
                    ...
                    if (snap > 0.f) {
                        ...
                        switch (mode) {
                            case NEKO_CORE_UI_TRANSFORM_WORLD:
                                model->rotation = neko_quat_mul(s_delta.rotation, orot);
                                break;
                            case NEKO_CORE_UI_TRANSFORM_LOCAL:
    ...
    neko_core_ui_draw_custom(ctx, clip, neko_core_ui_gizmo_render, &desc, sizeof(desc));
...
NEKO_API_DECL s32 neko_core_ui_style_editor(neko_core_ui_context_t* ctx, neko_core_ui_style_sheet_t* style_sheet, neko_core_ui_rect_t rect, bool* open) {
    if (!style_sheet) {
        style_sheet = &neko_core_ui_default_style_sheet;
    }

    static struct {
        const char* label;
        s32 idx;
    } elements[] = {{"container", NEKO_CORE_UI_ELEMENT_CONTAINER}, {"button", NEKO_CORE_UI_ELEMENT_BUTTON}, {"panel", NEKO_CORE_UI_ELEMENT_PANEL},
                    {"input", NEKO_CORE_UI_ELEMENT_INPUT},         {"label", NEKO_CORE_UI_ELEMENT_LABEL},   {"text", NEKO_CORE_UI_ELEMENT_TEXT},
                    {"scroll", NEKO_CORE_UI_ELEMENT_SCROLL},       {"image", NEKO_CORE_UI_ELEMENT_IMAGE},   {NULL}};
    ...
    static struct {
        const char* label;
        s32 idx;
    } colors[] = {{"background", NEKO_CORE_UI_COLOR_BACKGROUND},
                  {"border", NEKO_CORE_UI_COLOR_BORDER},
                  {"shadow", NEKO_CORE_UI_COLOR_SHADOW},
                  {"content", NEKO_CORE_UI_COLOR_CONTENT},
                  {"content_shadow", NEKO_CORE_UI_COLOR_CONTENT_SHADOW},
                  {"content_background", NEKO_CORE_UI_COLOR_CONTENT_BACKGROUND},
                  {"content_border", NEKO_CORE_UI_COLOR_CONTENT_BORDER},
                  {NULL}};

    if (neko_core_ui_window_begin_ex(ctx, "Style_Editor", rect, open, NULL, 0x00)) {
        for (u32 i = 0; elements[i].label; ++i) {
            s32 idx = elements[i].idx;

            if (neko_core_ui_treenode_begin_ex(ctx, elements[i].label, NULL, 0x00)) {
                for (u32 j = 0; j < NEKO_CORE_UI_ELEMENT_STATE_COUNT; ++j) {
                    neko_core_ui_push_id(ctx, &j, sizeof(j));
                    neko_core_ui_style_t* s = &style_sheet->styles[idx][j];
                    if (neko_core_ui_treenode_begin_ex(ctx, states[j], NULL, 0x00)) {
                        neko_core_ui_style_t* save = neko_core_ui_push_style(ctx, &ctx->style_sheet->styles[NEKO_CORE_UI_ELEMENT_PANEL][0x00]);
                        s32 row[] = {-1};
                        neko_core_ui_layout_row(ctx, 1, row, 300);
                        neko_core_ui_panel_begin(ctx, states[j]);
                        {
                            neko_core_ui_layout_t* l = neko_core_ui_get_layout(ctx);
                            neko_core_ui_rect_t* r = &l->body;
                            ...
                            {
                                s32 row[] = {ls, w, w};
                                neko_core_ui_layout_row(ctx, 3, row, 0);
                            }

                            neko_core_ui_label(ctx, "size:");
                            neko_core_ui_slider(ctx, &s->size[0], 0.f, 500.f);
                            neko_core_ui_slider(ctx, &s->size[1], 0.f, 500.f);

                            w = (s32)((l->body.w - ls) * 0.2f);

                            {
                                s32 row[] = {ls, w, w, w, w};
                                neko_core_ui_layout_row(ctx, 5, row, 0);
                            }

                            neko_core_ui_label(ctx, "border_width:");
                            ...
                            neko_core_ui_label(ctx, "border_radius:");
                            ...
                            // padding/margin
                            neko_core_ui_label(ctx, "padding:");
                            ...
                            int sw = (s32)(l->body.w * 0.14);
                            {
                                ...
                                neko_core_ui_layout_row(ctx, 6, row, 0);
                            }

                            for (u32 c = 0; colors[c].label; ++c) {
                                neko_core_ui_label(ctx, colors[c].label);
                                ...
                                neko_core_ui_draw_rect(ctx, neko_core_ui_layout_next(ctx), s->colors[c]);
                            }
                        }
                        neko_core_ui_panel_end(ctx);
                        neko_core_ui_pop_style(ctx, save);

                        neko_core_ui_treenode_end(ctx);
                    }
                    neko_core_ui_pop_id(ctx);
                }
                neko_core_ui_treenode_end(ctx);
...
NEKO_API_DECL s32 neko_core_ui_demo_window(neko_core_ui_context_t* ctx, neko_core_ui_rect_t rect, bool* open) {
    ...
    if (neko_core_ui_window_begin_ex(ctx, "Demo_Window", rect, open, NULL, 0x00)) {
        neko_core_ui_container_t* win = neko_core_ui_get_current_container(ctx);

        if (neko_core_ui_treenode_begin(ctx, "Help")) {
            {
                s32 row[] = {-10};
                neko_core_ui_layout_row(ctx, 1, row, 170);
            }

            neko_core_ui_panel_begin(ctx, "#!window_info");
            {
                {
                    s32 row[] = {-1};
                    neko_core_ui_layout_row(ctx, 1, row, 0);
                }
                neko_core_ui_label(ctx, "ABOUT THIS DEMO:");
                neko_core_ui_text(ctx, "  - Sections below are demonstrating many aspects of the util.");
                // neko_core_ui_text(ctx, " 测试中文，你好世界");
            }
            neko_core_ui_panel_end(ctx);
            neko_core_ui_treenode_end(ctx);
        }

        if (neko_core_ui_treenode_begin(ctx, "Window Info")) {
            {
                s32 row[] = {-10};
                neko_core_ui_layout_row(ctx, 1, row, 170);
            }
            neko_core_ui_panel_begin(ctx, "#!window_info");
            {
                ...
                {
                    s32 row[] = {65, -1};
                    neko_core_ui_layout_row(ctx, 2, row, 0);
                }

                neko_core_ui_label(ctx, "Position:");
                neko_snprintf(buf, 64, "%.2f, %.2f", win->rect.x, win->rect.y);
                neko_core_ui_label(ctx, buf);

                neko_core_ui_label(ctx, "Size:");
                neko_snprintf(buf, 64, "%.2f, %.2f", win->rect.w, win->rect.h);
                neko_core_ui_label(ctx, buf);

                neko_core_ui_label(ctx, "Title:");
                neko_core_ui_label(ctx, win->name);

                neko_core_ui_label(ctx, "ID:");
                neko_snprintf(buf, 64, "%zu", win->id);
                neko_core_ui_label(ctx, buf);

                neko_core_ui_label(ctx, "Open:");
        ...
        if (neko_core_ui_treenode_begin(ctx, "Context State")) {
            {
                s32 row[] = {-10};
                neko_core_ui_layout_row(ctx, 1, row, 170);
            }
            neko_core_ui_panel_begin(ctx, "#!context_state");
            {
                char buf[64];
                {
                    s32 row[] = {80, -1};
        ...
        if (neko_core_ui_treenode_begin(ctx, "Widgets")) {
            {
                s32 row[] = {-10};
                neko_core_ui_layout_row(ctx, 1, row, 170);
            }
            neko_core_ui_panel_begin(ctx, "#!widgets");
            {
                {
                    s32 row[] = {150, 50};
                    neko_core_ui_layout_row(ctx, 2, row, 0);
                ...
                neko_core_ui_layout_column_begin(ctx);
                {
                    ...
                    neko_core_ui_button(ctx, "Button");

                    // Label
                    neko_core_ui_label(ctx, "Label");
                    ...
                    neko_core_ui_text(ctx, "This is some text");

                    static char buf[64] = {0};
                    neko_core_ui_textbox(ctx, buf, 64);
                }
                neko_core_ui_layout_column_end(ctx);

                neko_core_ui_layout_column_begin(ctx);
                {
                    neko_core_ui_label(ctx, "(?)");
...
typedef enum { NEKO_CORE_UI_SS_DEF_NUMBER = 0x00, NEKO_CORE_UI_SS_DEF_ENUM, NEKO_CORE_UI_SS_DEF_COLOR, NEKO_CORE_UI_SS_DEF_STR } neko_core_ui_ss_var_def_type;

typedef struct {
    neko_core_ui_ss_var_def_type type;
    union {
        s32 number;
        neko_color_t color;
        char str[64];
    } val;
} neko_core_ui_ss_var_def_t;
...
bool _neko_core_ui_style_sheet_parse_attribute_transition(neko_core_ui_context_t* ctx, neko_lexer_t* lex, neko_core_ui_style_sheet_t* ss, const u64 id_tag, s32 elementid, s32 state,
                                                          neko_core_ui_ss_variables_t* variables) {
    ...
    if (id_tag) {
        if (!neko_hash_table_exists(ss->cid_animations, id_tag)) {
            neko_core_ui_animation_property_list_t sl = neko_default_val();
...
#define PARSE_TRANSITION(T)                                                                                                      \
    do {                                                                                                                         \
        ...
        neko_core_ui_animation_property_t prop = neko_default_val();                                                             \
        prop.type = T;                                                                                                           \
    ...
    _NEKO_CORE_UI_SS_GET_TO_VALUES(NEKO_TOKEN_COLON, NEKO_TOKEN_LBRACE);

    if (!neko_hash_table_exists(ss->animations, (neko_core_ui_element_type)elementid)) {
        neko_core_ui_animation_property_list_t list = neko_default_val();
        neko_hash_table_insert(ss->animations, (neko_core_ui_element_type)elementid, list);
    }
    neko_core_ui_animation_property_list_t* list = id_tag ? neko_hash_table_getp(ss->cid_animations, id_tag) : neko_hash_table_getp(ss->animations, (neko_core_ui_element_type)elementid);
...
bool _neko_core_ui_style_sheet_parse_attribute_font(neko_core_ui_context_t* ctx, neko_lexer_t* lex, neko_core_ui_style_sheet_t* ss, u64 id_tag, s32 elementid, s32 state,
                                                    neko_core_ui_ss_variables_t* variables) {
    // the_name of enum attribute
    neko_token_t token = neko_lexer_current_token(lex);
    // neko_token_debug_print(&token);
    //
    neko_core_ui_style_list_t* idsl = NULL;
    if (id_tag) {
        const u64 idhash = id_tag;
        if (!neko_hash_table_exists(ss->cid_styles, idhash)) {
            neko_core_ui_style_list_t sl = neko_default_val();
...
#define SET_FONT(FONT)                                                  \
    ...
    if (neko_token_compare_text(&token, "font")) {
        ...
        neko_core_ui_style_element_t se = neko_default_val();
        se.type = NEKO_CORE_UI_STYLE_FONT;
        token = neko_lexer_current_token(lex);
        ...
        switch (token.type) {
            ...
            case NEKO_TOKEN_DOLLAR: {
                ...
                neko_core_ui_ss_var_def_t* var = neko_hash_table_exists(variables->variables, hash) ? neko_hash_table_getp(variables->variables, hash) : NULL;
                if (!var) {
...
bool _neko_core_ui_style_sheet_parse_attribute_enum(neko_core_ui_context_t* ctx, neko_lexer_t* lex, neko_core_ui_style_sheet_t* ss, u64 id_tag, s32 elementid, s32 state,
                                                    neko_core_ui_ss_variables_t* variables) {
    // the_name of enum attribute
    neko_token_t token = neko_lexer_current_token(lex);
    // neko_token_debug_print(&token);
...
#define SET_ENUM(COMP, VAL)                                       \
    ...
    if (neko_token_compare_text(&token, "justify_content")) {
        neko_core_ui_style_element_t se = neko_default_val();
        se.type = NEKO_CORE_UI_STYLE_JUSTIFY_CONTENT;
        bool ret = neko_lexer_require_token_type(lex, NEKO_TOKEN_COLON);
        ...
        switch (token.type) {
            case NEKO_TOKEN_IDENTIFIER: {
                if (neko_token_compare_text(&token, "start"))
                    SET_ENUM(justify_content, NEKO_CORE_UI_JUSTIFY_START);
                else if (neko_token_compare_text(&token, "end"))
                    SET_ENUM(justify_content, NEKO_CORE_UI_JUSTIFY_END);
                else if (neko_token_compare_text(&token, "center"))
                    SET_ENUM(justify_content, NEKO_CORE_UI_JUSTIFY_CENTER);
            } break;
        ...
        neko_core_ui_style_element_t se = neko_default_val();
        se.type = NEKO_CORE_UI_STYLE_ALIGN_CONTENT;
        bool ret = neko_lexer_require_token_type(lex, NEKO_TOKEN_COLON);
        ret &= (neko_lexer_require_token_type(lex, NEKO_TOKEN_IDENTIFIER) || neko_lexer_require_token_type(lex, NEKO_TOKEN_DOLLAR));
        if (!ret) {
            neko_log_warning("Missing either identifier value or variable.");
        ...
        switch (token.type) {
            case NEKO_TOKEN_IDENTIFIER: {
                if (neko_token_compare_text(&token, "start"))
                    SET_ENUM(align_content, NEKO_CORE_UI_ALIGN_START);
                else if (neko_token_compare_text(&token, "end"))
                    SET_ENUM(align_content, NEKO_CORE_UI_ALIGN_END);
                else if (neko_token_compare_text(&token, "center"))
                    SET_ENUM(align_content, NEKO_CORE_UI_ALIGN_CENTER);
            } break;
...
bool _neko_core_ui_style_sheet_parse_attribute_val(neko_core_ui_context_t* ctx, neko_lexer_t* lex, neko_core_ui_style_sheet_t* ss, u64 id_tag, s32 elementid, s32 state,
                                                   neko_core_ui_ss_variables_t* variables) {
...
#define SET_VAL2(COMP0, COMP1, T, SE0, SE1)                                                                                                                    \
    do {                                                                                                                                                       \
        neko_core_ui_style_element_t se0 = neko_default_val();                                                                                                 \
        neko_core_ui_style_element_t se1 = neko_default_val();                                                                                                 \
        se0.type = SE0;                                                                                                                                        \
        se1.type = SE1;                                                                                                                                        \
        bool ret = neko_lexer_require_token_type(lex, NEKO_TOKEN_COLON);                                                                                       \
        ret &= (neko_lexer_require_token_type(lex, NEKO_TOKEN_DOLLAR) || neko_lexer_require_token_type(lex, NEKO_TOKEN_NUMBER));                               \
...
#define SET_VAL(COMP, T, SE)                                                                                                                                   \
    ...
    if (neko_token_compare_text(&token, "width"))
        SET_VAL(size[0], float, NEKO_CORE_UI_STYLE_WIDTH);
    else if (neko_token_compare_text(&token, "height"))
        SET_VAL(size[1], float, NEKO_CORE_UI_STYLE_HEIGHT);
    else if (neko_token_compare_text(&token, "padding"))
        SET_VAL4(padding, s16, NEKO_CORE_UI_STYLE_PADDING);
    else if (neko_token_compare_text(&token, "padding_left"))
        SET_VAL(padding[NEKO_CORE_UI_PADDING_LEFT], s16, NEKO_CORE_UI_STYLE_PADDING_LEFT);
    else if (neko_token_compare_text(&token, "padding_right"))
        SET_VAL(padding[NEKO_CORE_UI_PADDING_RIGHT], s16, NEKO_CORE_UI_STYLE_PADDING_RIGHT);
    else if (neko_token_compare_text(&token, "padding_top"))
        SET_VAL(padding[NEKO_CORE_UI_PADDING_TOP], s16, NEKO_CORE_UI_STYLE_PADDING_TOP);
    else if (neko_token_compare_text(&token, "padding_bottom"))
        SET_VAL(padding[NEKO_CORE_UI_PADDING_BOTTOM], s16, NEKO_CORE_UI_STYLE_PADDING_BOTTOM);
    else if (neko_token_compare_text(&token, "margin"))
        SET_VAL4(margin, s16, NEKO_CORE_UI_STYLE_MARGIN);
    else if (neko_token_compare_text(&token, "margin_left"))
        SET_VAL(margin[NEKO_CORE_UI_MARGIN_LEFT], s16, NEKO_CORE_UI_STYLE_MARGIN_LEFT);
    else if (neko_token_compare_text(&token, "margin_right"))
        SET_VAL(margin[NEKO_CORE_UI_MARGIN_RIGHT], s16, NEKO_CORE_UI_STYLE_MARGIN_RIGHT);
    else if (neko_token_compare_text(&token, "margin_top"))
        SET_VAL(margin[NEKO_CORE_UI_MARGIN_TOP], s16, NEKO_CORE_UI_STYLE_MARGIN_TOP);
    else if (neko_token_compare_text(&token, "margin_bottom"))
        SET_VAL(margin[NEKO_CORE_UI_MARGIN_BOTTOM], s16, NEKO_CORE_UI_STYLE_MARGIN_BOTTOM);
    else if (neko_token_compare_text(&token, "border"))
        SET_VAL4(border_width, s16, NEKO_CORE_UI_STYLE_BORDER_WIDTH);
    else if (neko_token_compare_text(&token, "border_left"))
        SET_VAL(border_width[0], s16, NEKO_CORE_UI_STYLE_BORDER_WIDTH_LEFT);
    else if (neko_token_compare_text(&token, "border_right"))
        SET_VAL(border_width[1], s16, NEKO_CORE_UI_STYLE_BORDER_WIDTH_RIGHT);
    else if (neko_token_compare_text(&token, "border_top"))
        SET_VAL(border_width[2], s16, NEKO_CORE_UI_STYLE_BORDER_WIDTH_TOP);
    else if (neko_token_compare_text(&token, "border_bottom"))
        SET_VAL(border_width[3], s16, NEKO_CORE_UI_STYLE_BORDER_WIDTH_BOTTOM);
    else if (neko_token_compare_text(&token, "shadow"))
        SET_VAL2(shadow_x, shadow_y, s16, NEKO_CORE_UI_STYLE_SHADOW_X, NEKO_CORE_UI_STYLE_SHADOW_Y);

    return true;
}

bool _neko_core_ui_style_sheet_parse_attribute_color(neko_core_ui_context_t* ctx, neko_lexer_t* lex, neko_core_ui_style_sheet_t* ss, u64 id_tag, s32 elementid, s32 state,
                                                     neko_core_ui_ss_variables_t* variables) {
    ...
    neko_core_ui_style_element_type type = (neko_core_ui_style_element_type)0x00;
    s32 color = NEKO_CORE_UI_COLOR_BACKGROUND;
    if (neko_token_compare_text(&token, "color_background")) {
        color = NEKO_CORE_UI_COLOR_BACKGROUND;
        type = NEKO_CORE_UI_STYLE_COLOR_BACKGROUND;
    } else if (neko_token_compare_text(&token, "color_border")) {
        color = NEKO_CORE_UI_COLOR_BORDER;
        type = NEKO_CORE_UI_STYLE_COLOR_BORDER;
    } else if (neko_token_compare_text(&token, "color_shadow")) {
        color = NEKO_CORE_UI_COLOR_SHADOW;
        type = NEKO_CORE_UI_STYLE_COLOR_SHADOW;
    } else if (neko_token_compare_text(&token, "color_content")) {
        color = NEKO_CORE_UI_COLOR_CONTENT;
        type = NEKO_CORE_UI_STYLE_COLOR_CONTENT;
    } else if (neko_token_compare_text(&token, "color_content_background")) {
        color = NEKO_CORE_UI_COLOR_CONTENT_BACKGROUND;
        type = NEKO_CORE_UI_STYLE_COLOR_CONTENT_BACKGROUND;
    } else if (neko_token_compare_text(&token, "color_content_border")) {
        color = NEKO_CORE_UI_COLOR_CONTENT_BORDER;
        type = NEKO_CORE_UI_STYLE_COLOR_CONTENT_BORDER;
    } else if (neko_token_compare_text(&token, "color_content_shadow")) {
        color = NEKO_CORE_UI_COLOR_CONTENT_SHADOW;
        type = NEKO_CORE_UI_STYLE_COLOR_CONTENT_SHADOW;
    } else {
        neko_log_warning("Unidentified attribute: %.*s.", token.len, token.text);
        return false;
    }
...
bool _neko_core_ui_style_sheet_parse_attribute(neko_core_ui_context_t* ctx, neko_lexer_t* lex, neko_core_ui_style_sheet_t* ss, u64 id_tag, s32 elementid, s32 state,
                                               neko_core_ui_ss_variables_t* variables) {
    ...
    if (neko_token_compare_text(&token, "color_background") || neko_token_compare_text(&token, "color_border") || neko_token_compare_text(&token, "color_shadow") ||
        ...
        if (!_neko_core_ui_style_sheet_parse_attribute_color(ctx, lex, ss, id_tag, elementid, state, variables)) {
        ...
        if (!_neko_core_ui_style_sheet_parse_attribute_val(ctx, lex, ss, id_tag, elementid, state, variables)) {
        ...
        if (!_neko_core_ui_style_sheet_parse_attribute_enum(ctx, lex, ss, id_tag, elementid, state, variables)) {
        ...
        if (!_neko_core_ui_style_sheet_parse_attribute_font(ctx, lex, ss, id_tag, elementid, state, variables)) {
        ...
        if (!_neko_core_ui_style_sheet_parse_attribute_transition(ctx, lex, ss, id_tag, elementid, state, variables)) {
...
bool _neko_core_ui_style_sheet_parse_element(neko_core_ui_context_t* ctx, neko_lexer_t* lex, neko_core_ui_style_sheet_t* ss, s32 elementid, neko_core_ui_ss_variables_t* variables) {
    s32 state = 0x00;
    s32 bc = 0;
    neko_token_t token = neko_lexer_next_token(lex);
    if (token.type == NEKO_TOKEN_COLON) {
        ...
        if (neko_token_compare_text(&token, "focus"))
            state = NEKO_CORE_UI_ELEMENT_STATE_FOCUS;
        else if (neko_token_compare_text(&token, "hover"))
            state = NEKO_CORE_UI_ELEMENT_STATE_HOVER;
        else {
    ...
    while (neko_lexer_can_lex(lex) && bc) {
        ...
        switch (token.type) {
            ...
            case NEKO_TOKEN_IDENTIFIER: {
                ...
                if (!_neko_core_ui_style_sheet_parse_attribute(ctx, lex, ss, 0, elementid, state, variables)) {
...
bool _neko_core_ui_style_sheet_parse_cid_tag(neko_core_ui_context_t* ctx, neko_lexer_t* lex, neko_core_ui_style_sheet_t* ss, const u64 cid_tag, neko_core_ui_ss_variables_t* variables) {
    s32 state = 0x00;
    s32 bc = 0;
    neko_token_t token = neko_lexer_next_token(lex);
    if (token.type == NEKO_TOKEN_COLON) {
    ...
    while (neko_lexer_can_lex(lex) && bc) {
        ...
        switch (token.type) {
            ...
            case NEKO_TOKEN_IDENTIFIER: {
                // neko_println("Parsing attribute: %.*s", token.len, token.text);
                if (!_neko_core_ui_style_sheet_parse_attribute(ctx, lex, ss, cid_tag, NEKO_CORE_UI_ELEMENT_COUNT, state, variables)) {
                    neko_log_warning("Unable to parse attribute");
                    return false;
                }
            } break;
...
NEKO_API_DECL neko_core_ui_style_sheet_t neko_core_ui_style_sheet_load_from_file(neko_core_ui_context_t* ctx, const char* file_path) {
    // Generate new style sheet based on default element styles
    neko_core_ui_style_sheet_t ss = neko_default_val();
    bool success = true;
    ...
    ss = neko_core_ui_style_sheet_load_from_memory(ctx, fd, sz, &success);
...
static bool _neko_core_ui_style_sheet_parse_variable(neko_core_ui_context_t* ctx, neko_lexer_t* lex, neko_core_ui_style_sheet_t* ss, char* name_buf, size_t sz, neko_core_ui_ss_var_def_t* out) {
        ...
        switch (token.type) {
            case NEKO_TOKEN_IDENTIFIER: {
                if (neko_token_compare_text(&token, "rgb")) {
                    ...
                    out->type = NEKO_CORE_UI_SS_DEF_COLOR;
                    ...
                    out->type = NEKO_CORE_UI_SS_DEF_ENUM;
                    out->val.number = (s32)NEKO_CORE_UI_JUSTIFY_CENTER;
                } else if (neko_token_compare_text(&token, "start")) {
                    out->type = NEKO_CORE_UI_SS_DEF_ENUM;
                    out->val.number = (s32)NEKO_CORE_UI_JUSTIFY_START;
                } else if (neko_token_compare_text(&token, "end")) {
                    out->type = NEKO_CORE_UI_SS_DEF_ENUM;
                    out->val.number = (s32)NEKO_CORE_UI_JUSTIFY_END;
                } else {
            ...
            case NEKO_TOKEN_NUMBER: {
                neko_snprintfc(VAL, 32, "%.*s", token.len, token.text);
                s32 v = (s32)atoi(VAL);
                out->type = NEKO_CORE_UI_SS_DEF_NUMBER;
                out->val.number = v;
            } break;

            case NEKO_TOKEN_STRING: {
                neko_snprintfc(VAL, 64, "%.*s", token.len - 2, token.text + 1);
                out->type = NEKO_CORE_UI_SS_DEF_STR;
...
NEKO_API_DECL neko_core_ui_style_sheet_t neko_core_ui_style_sheet_load_from_memory(neko_core_ui_context_t* ctx, const char* fd, size_t sz, bool* sp) {
    // Generate new style sheet based on default element styles
    neko_core_ui_style_sheet_t ss = neko_default_val();
    bool success = true;

    neko_core_ui_ss_variables_t variables = neko_default_val();

    // Copy all default styles
    NEKO_CORE_UI_COPY_STYLES(ss.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_CONTAINER);
    NEKO_CORE_UI_COPY_STYLES(ss.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_LABEL);
    NEKO_CORE_UI_COPY_STYLES(ss.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_TEXT);
    NEKO_CORE_UI_COPY_STYLES(ss.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_PANEL);
    NEKO_CORE_UI_COPY_STYLES(ss.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_INPUT);
    NEKO_CORE_UI_COPY_STYLES(ss.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_BUTTON);
    NEKO_CORE_UI_COPY_STYLES(ss.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_SCROLL);
    NEKO_CORE_UI_COPY_STYLES(ss.styles, neko_core_ui_default_style_sheet.styles, NEKO_CORE_UI_ELEMENT_IMAGE);

#define PARSE_ELEMENT(TYPE, TYPESTR)                                                      \
    do {                                                                                  \
        if (!_neko_core_ui_style_sheet_parse_element(ctx, &lex, &ss, TYPE, &variables)) { \
        ...
        switch (token.type) {
            case NEKO_TOKEN_IDENTIFIER: {
                if (neko_token_compare_text(&token, "button"))
                    PARSE_ELEMENT(NEKO_CORE_UI_ELEMENT_BUTTON, "button");
                else if (neko_token_compare_text(&token, "text"))
                    PARSE_ELEMENT(NEKO_CORE_UI_ELEMENT_TEXT, "text");
                else if (neko_token_compare_text(&token, "label"))
                    PARSE_ELEMENT(NEKO_CORE_UI_ELEMENT_LABEL, "label");
                else if (neko_token_compare_text(&token, "image"))
                    PARSE_ELEMENT(NEKO_CORE_UI_ELEMENT_IMAGE, "image");
                else if (neko_token_compare_text(&token, "scroll"))
                    PARSE_ELEMENT(NEKO_CORE_UI_ELEMENT_SCROLL, "scroll");
                else if (neko_token_compare_text(&token, "panel"))
                    PARSE_ELEMENT(NEKO_CORE_UI_ELEMENT_PANEL, "panel");
                else if (neko_token_compare_text(&token, "container"))
                    PARSE_ELEMENT(NEKO_CORE_UI_ELEMENT_CONTAINER, "container");
                else if (neko_token_compare_text(&token, "input"))
                    PARSE_ELEMENT(NEKO_CORE_UI_ELEMENT_INPUT, "input");
                else {
            ...
            case NEKO_TOKEN_PERIOD: {
                ...
                if (!_neko_core_ui_style_sheet_parse_cid_tag(ctx, &lex, &ss, cls_hash, &variables)) {
            ...
            case NEKO_TOKEN_HASH: {
                ...
                if (!_neko_core_ui_style_sheet_parse_cid_tag(ctx, &lex, &ss, id_hash, &variables)) {
            ...
            case NEKO_TOKEN_DOLLAR: {
                neko_core_ui_ss_var_def_t variable = neko_default_val();
                char variable_name[256] = neko_default_val();
                if (!_neko_core_ui_style_sheet_parse_variable(ctx, &lex, &ss, variable_name, sizeof(variable_name), &variable)) {
                ...
                typedef enum
                {
                    NEKO_CORE_UI_SS_DEF_VALUE = 0x00,
                    NEKO_CORE_UI_SS_DEF_COLOR,
                    NEKO_CORE_UI_SS_DEF_STR
                } neko_core_ui_ss_var_def_type;
