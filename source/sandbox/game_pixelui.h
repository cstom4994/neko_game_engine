...
void putpixel(neko_fallsand_render* fallsand, s32 x, s32 y) {
    if (pixelui_in_bounds(x, y)) {
        fallsand->ui_buffer[pixelui_compute_idx(x, y)] = cell_color_t{255, 255, 255, 255};
    }
}

// Function to put pixels
// at subsequence points
void drawCircle(neko_fallsand_render* fallsand, s32 xc, s32 yc, s32 x, s32 y) {
    putpixel(fallsand, xc + x, yc + y);
    putpixel(fallsand, xc - x, yc + y);
    putpixel(fallsand, xc + x, yc - y);
    putpixel(fallsand, xc - x, yc - y);
    putpixel(fallsand, xc + y, yc + x);
    putpixel(fallsand, xc - y, yc + x);
    putpixel(fallsand, xc + y, yc - x);
    putpixel(fallsand, xc - y, yc - x);
}
...
void circleBres(neko_fallsand_render* fallsand, s32 xc, s32 yc, s32 r) {
    s32 x = 0, y = r;
    s32 d = 3 - 2 * r;
    drawCircle(fallsand, xc, yc, x, y);
    while (y >= x) {
...
#define __gui_interaction(x, y, w, h, c, str, id)                                                                                                         \
    do {                                                                                                                                                  \
        if ((id) == fallsand->material_selection) {                                                                                                       \
            const s32 b = 2;                                                                                                                              \
            gui_rect(fallsand->ui_buffer, x - b / 2, y - b / 2, w + b, h + b, cell_color_t{200, 150, 20, 255});                                           \
        }                                                                                                                                                 \
        neko_vec2 mp = calculate_mouse_position();                                                                                                        \
        if (in_rect(mp, neko_vec2{(f32)(x), (f32)(y)}, neko_vec2{(w), (h)})) {                                                                            \
            ...
            gui_rect(fallsand->ui_buffer, g_pixelui_texture_width / 2 - 50, 15, 100, 20, r_col);                                                          \
            draw_string_at(&pixel_font, fallsand->ui_buffer, g_pixelui_texture_width / 2 + 1 - (sizeof(str) * 5) / 2, 20 - 1, _str, sizeof(_str), s_col); \
            draw_string_at(&pixel_font, fallsand->ui_buffer, g_pixelui_texture_width / 2 - (sizeof(str) * 5) / 2, 20, _str, sizeof(_str), col);           \
        }                                                                                                                                                 \
        if (gui_rect(fallsand->ui_buffer, x, y, w, h, c)) {                                                                                               \
            fallsand->material_selection = id;                                                                                                            \
        }                                                                                                                                                 \
    } while (0)
...
bool update_ui(neko_fallsand_render* fallsand) {
    ...
    // Do ui stuff
    memset(fallsand->ui_buffer, 0, g_pixelui_texture_width * g_pixelui_texture_height * sizeof(cell_color_t));

    // Material selection panel gui
    if (fallsand->show_material_selection_panel) {
        const s32 offset = 12;
        s32 xoff = 20;
        s32 base = 10;
    ...
    if (fallsand->show_frame_count) {

        char frame_time_str[256];
        neko_snprintf(frame_time_str, sizeof(frame_time_str), "frame: %6.2f ms %llu", neko_platform_frame_time(), fallsand->update_time);
        draw_string_at(&pixel_font, fallsand->ui_buffer, 10, 10, frame_time_str, strlen(frame_time_str), cell_color_t{255, 255, 255, 255});
    ...
    fallsand->update_time = 0;

    // Draw circle around mouse pointer
    s32 R = fallsand->brush_size * fallsand->render_scale;
    circleBres(fallsand, (s32)(mp.x), (s32)(mp.y), R);
    ...
    neko_graphics_texture_update(fallsand->tex_ui, &t_desc);
