...
int lt_poll_event(lua_State *L) {  // init.lua > core.step() wakes on mousemoved || inputtext
    ...
    if ((lt_time_now() - clicks_time) > 400) clicks = 0;
    ...
    for (INPUT_WRAP_event e; input_wrap_next_e(&lt_input_queue, &e); input_wrap_free_e(&e))
        if (e.type) switch (e.type) {
                ...
                case INPUT_WRAP_WINDOW_RESIZED:
                    ...
                    lt_resizesurface(lt_getsurface(), lt_ww, lt_wh);
...
void ren_init(void *win) {
    lt_surface *surf = lt_getsurface();
    ren_set_clip_rect(RenRect{0, 0, surf->w, surf->h});
}

void ren_update_rects(RenRect *rects, int count) { lt_updatesurfacerects(lt_getsurface(), (lt_rect *)rects, count); }
...
RenImage *ren_new_image(int width, int height) {
    neko_assert(width > 0 && height > 0);
    RenImage *image = (RenImage *)mem_alloc(sizeof(RenImage) + width * height * sizeof(RenColor));
...
void ren_free_image(RenImage *image) { mem_free(image); }

static GlyphSet *load_glyphset(RenFont *font, int idx) {
    GlyphSet *set = (GlyphSet *)mem_calloc(1, sizeof(GlyphSet));
...
RenFont *ren_load_font(const char *filename, float size) {
    ...
    // init font
    font = (RenFont *)mem_calloc(1, sizeof(RenFont));
    font->size = size;
    font->data = fontdata;
...
static int f_window_has_focus(lua_State *L) {
    unsigned flags = window_has_focus();
...
static int f_show_confirm_dialog(lua_State *L) {
    ...
    int id = window_prompt(msg, title);  // 0:no, 1:yes
...
static int f_get_clipboard(lua_State *L) {
    const char *text = window_clipboard();
    if (!text) {
        return 0;
    }
...
static int f_set_clipboard(lua_State *L) {
    ...
    window_setclipboard(text);
    return 0;
}

static int f_get_time(lua_State *L) {
    double ss = lt_time_now() / 1000.0;
    lua_pushnumber(L, ss);
    return 1;
}
...
static int f_exec(lua_State *L) {
    ...
    char *buf = (char *)mem_alloc(len + 32);
    if (!buf) {
