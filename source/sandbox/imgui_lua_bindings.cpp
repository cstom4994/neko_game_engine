...
//
// based on https://github.com/casssoft/imgui_lua_bindings
// for neko engine
//
...
static void AddToStack(int type) { endStack.push_back(type); }
...
#define IM_TEXTURE_ID_ARG(name) const ImTextureID name = (ImTextureID)luaL_checkinteger(L, arg++);
...
#define NUMBER_ARG(name) lua_Number name = luaL_checknumber(L, arg++);

#define FLOAT_ARRAY_DEF(name, size) float name[size];

#define FLOAT_ARRAY_ARG(name, it) name[it] = (float)luaL_checknumber(L, arg++);
...
#define INT_ARG(name) const int name = (int)luaL_checknumber(L, arg++);

#define INT_ARRAY_DEF(name, size) int name[size];

#define INT_ARRAY_ARG(name, it) name[it] = (int)luaL_checknumber(L, arg++);
...
#define UINT_ARG(name) const unsigned int name = (unsigned int)luaL_checkinteger(L, arg++);
...
#define BOOL_ARG(name) bool name = lua_toboolean(L, arg++);
...
#define CALLBACK_STUB(name, callback) callback name = NULL;

#define CALL_FUNCTION(name, retType, ...) retType ret = ImGui::name(__VA_ARGS__);

#define DRAW_LIST_CALL_FUNCTION(name, retType, ...) retType ret = ImGui::GetWindowDrawList()->name(__VA_ARGS__);

#define CALL_FUNCTION_NO_RET(name, ...) ImGui::name(__VA_ARGS__);

#define DRAW_LIST_CALL_FUNCTION_NO_RET(name, ...) ImGui::GetWindowDrawList()->name(__VA_ARGS__);
...
#define PUSH_TABLE_TABLE lua_newtable(L);

#define PUSH_TABLE_STRING(name) lua_pushstring(L, name);

#define PUSH_TABLE_NUMBER(name) lua_pushnumber(L, name);

#define PUSH_TABLE_BOOL(name) lua_pushboolean(L, (int)name);

#define SET_TABLE_FIELD(name) lua_setfield(L, -2, name);
...
#define ADD_END_STACK(type) AddToStack(type);

#define POP_END_STACK(type) PopEndStack(type);
