...
#define NEKO_PLATFORM_IMPL_ANDROID
#endif
#endif
...
#ifdef NEKO_PLATFORM_IMPL_ANDROID
...
typedef enum neko_android_message_type {
    NEKO_ANDROID_MSG_CREATE,
    NEKO_ANDROID_MSG_START,
    NEKO_ANDROID_MSG_STOP,
    NEKO_ANDROID_MSG_RESUME,
    NEKO_ANDROID_MSG_PAUSE,
    NEKO_ANDROID_MSG_GAIN_FOCUS,
    NEKO_ANDROID_MSG_LOSE_FOCUS,
    NEKO_ANDROID_MSG_SET_NATIVE_WINDOW,
    NEKO_ANDROID_MSG_SET_INPUT_QUEUE,
    NEKO_ANDROID_MSG_DESTROY
} neko_android_message_type;

typedef struct neko_android_thread_t {
    pthread_t thread;
    pthread_mutex_t mutex;
    pthread_cond_t cond;
    s32 read_from_main_fd;
    s32 write_from_main_fd;
    bool is_thread_started;
    bool is_thread_stopping;
    bool is_thread_stopped;
    bool padding;
} neko_android_thread_t;

typedef struct neko_android_resources_t {
    ANativeWindow* window;
    AInputQueue* input;
} neko_android_resources_t;

typedef struct neko_android_egl_t {
    s32 width;
    s32 height;
    s32 fbwidth;
    s32 fbheight;
...
typedef struct neko_android_state_t {
    bool has_created;
    bool has_resumed;
    bool has_focus;
    bool padding;
} neko_android_state_t;

typedef struct neko_android_t {
    ANativeActivity* activity;
    neko_android_thread_t gt;
    neko_android_resources_t pending;
    neko_android_resources_t current;
    ALooper* looper;
    neko_android_egl_t egl;
    neko_android_state_t state;
    const char* internal_data_path;
} neko_android_t;

#define NEKO_PLATFORM_ANDROID_DATA(...) ((neko_android_t*)neko_subsystem(platform)->user_data)

bool neko_android_should_update(neko_android_t* android) { return (android->state.has_focus && android->state.has_resumed && android->egl.surface != EGL_NO_SURFACE); }

void neko_android_shutdown(neko_android_t* android) { android->gt.is_thread_stopping = true; }

bool neko_android_init_egl(neko_android_t* android) {
    neko_assert(android->egl.display == EGL_NO_DISPLAY);
    neko_assert(android->egl.context == EGL_NO_CONTEXT);
    ...
    const EGLint cfg_attributes[] = {
            EGL_SURFACE_TYPE, EGL_WINDOW_BIT, EGL_RED_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_ALPHA_SIZE, alpha_size, EGL_DEPTH_SIZE, 16, EGL_STENCIL_SIZE, 0, EGL_NONE,
    };
    EGLConfig available_cfgs[32];
    EGLint cfg_count;
    eglChooseConfig(display, cfg_attributes, available_cfgs, 32, &cfg_count);
    neko_assert(cfg_count > 0);
    neko_assert(cfg_count <= 32);
    ...
    for (int i = 0; i < cfg_count; ++i) {
        ...
        EGLint r, g, b, a, d;
        if (eglGetConfigAttrib(display, c, EGL_RED_SIZE, &r) == EGL_TRUE && eglGetConfigAttrib(display, c, EGL_GREEN_SIZE, &g) == EGL_TRUE &&
            eglGetConfigAttrib(display, c, EGL_BLUE_SIZE, &b) == EGL_TRUE && eglGetConfigAttrib(display, c, EGL_ALPHA_SIZE, &a) == EGL_TRUE &&
            eglGetConfigAttrib(display, c, EGL_DEPTH_SIZE, &d) == EGL_TRUE && r == 8 && g == 8 && b == 8 && (alpha_size == 0 || a == alpha_size) && d == 16) {
    ...
    neko_println("EGL: Successfully initialized.");
...
bool neko_android_create_egl_surface(neko_android_t* android, ANativeWindow* window) {
    neko_assert(android);
    neko_assert(android->egl.display != EGL_NO_DISPLAY);
    neko_assert(android->egl.context != EGL_NO_CONTEXT);
    neko_assert(window);
...
bool neko_android_destroy_egl_surface(neko_android_t* android) {
...
void neko_android_update_dimensions(neko_android_t* android, ANativeWindow* window, bool force_update) {
    neko_assert(android->egl.display != EGL_NO_DISPLAY);
    neko_assert(android->egl.context != EGL_NO_CONTEXT);
    neko_assert(android->egl.surface != EGL_NO_SURFACE);
    neko_assert(window);

    const s32 win_w = ANativeWindow_getWidth(window);
    const s32 win_h = ANativeWindow_getHeight(window);
    neko_assert(win_w >= 0 && win_h >= 0);
    ...
    if (win_changed || force_update) {
        if (true) {  // High dpi setting
            /*
            const s32 buf_w = win_w / 2;
            const s32 buf_h = win_h / 2;
            EGLint format;
            EGLBoolean egl_result = eglGetConfigAttrib(android->egl.display, android->egl.config, EGL_NATIVE_VISUAL_ID, &format);
            neko_assert(egl_result == EGL_TRUE);
            s32 result = ANativeWindow_setBuffersGeometry(window, buf_w, buf_h, format);
            neko_assert(result == 0);
            */
        }
    }
    ...
    neko_assert(egl_result_w == EGL_TRUE);
    neko_assert(egl_result_h == EGL_TRUE);
    ...
    neko_println("fb: <%.2f, %.2f>, ws: <%.2f, %.2f>", (float)fb_w, (float)fb_h, (float)win_w, (float)win_h);
...
bool neko_android_touch_event(AInputEvent* evt) {
    neko_android_t* android = NEKO_PLATFORM_ANDROID_DATA();
    neko_assert(android);
    neko_assert(android->current.input);

    if (AInputEvent_getType(evt) != AINPUT_EVENT_TYPE_MOTION) {
        return false;
    }
    // Construct touch events here and call add into platform layer
    s32 action_idx = AMotionEvent_getAction(evt);
    s32 action = action_idx & AMOTION_EVENT_ACTION_MASK;

    // Construct platform event to add
    neko_platform_event_t gsevt = neko_default_val();
    gsevt.type = NEKO_PLATFORM_EVENT_TOUCH;

    switch (action) {

        case AMOTION_EVENT_ACTION_DOWN:
        case AMOTION_EVENT_ACTION_POINTER_DOWN: {
            gsevt.touch.action = NEKO_PLATFORM_TOUCH_DOWN;
        } break;

        case AMOTION_EVENT_ACTION_UP:
        case AMOTION_EVENT_ACTION_POINTER_UP: {
            gsevt.touch.action = NEKO_PLATFORM_TOUCH_UP;
        } break;

        case AMOTION_EVENT_ACTION_MOVE: {
            gsevt.touch.action = NEKO_PLATFORM_TOUCH_MOVE;
        } break;

        case AMOTION_EVENT_ACTION_CANCEL: {
            gsevt.touch.action = NEKO_PLATFORM_TOUCH_CANCEL;
        } break;
    ...
    s32 idx = action_idx >> AMOTION_EVENT_ACTION_POINTER_INDEX_SHIFT;
    u32 num_touches = (u32)AMotionEvent_getPointerCount(evt);

    if (num_touches > NEKO_PLATFORM_MAX_TOUCH) {
        num_touches = NEKO_PLATFORM_MAX_TOUCH;
    }

    for (u32 i = 0; i < num_touches; ++i) {
        ...
        neko_platform_add_event(&gsevt);
    }

    return true;
}

bool neko_android_key_event(AInputEvent* evt) {
    if (AInputEvent_getType(evt) != AINPUT_EVENT_TYPE_KEY) {
        return false;
    }
    // Shutdown event?
    if (AKeyEvent_getKeyCode(evt) == AKEYCODE_BACK) {
        neko_println("Android: back key pressed");
        return false;
    }
...
s32 neko_android_input_cb(s32 fd, s32 events, void* data) {
    neko_android_t* android = (neko_android_t*)data;
    neko_assert(android);
    neko_assert(android->current.input);

    if ((events & ALOOPER_EVENT_INPUT) == 0) {
        neko_println("Android Main CB:Unsupported event: %zu", events);
        return 1;
    }
    ...
    while (AInputQueue_getEvent(android->current.input, &evt) >= 0) {
        ...
        s32 handled = 0;
        if (neko_android_touch_event(evt) || neko_android_key_event(evt)) {
            handled = 1;
        }
        // Finish events
...
s32 neko_android_main_cb(s32 fd, s32 events, void* data) {
    neko_android_t* android = (neko_android_t*)data;
    neko_assert(android);
    ...
    neko_android_message_type msg;
    if (read(fd, &msg, sizeof(msg)) != sizeof(msg)) {
        neko_println("Could not write to read_from_main_fd");
        return 1;
    }
    ...
    switch (msg) {
        case NEKO_ANDROID_MSG_CREATE: {
            neko_println("MSG_CREATE");
            android->state.has_created = true;
        } break;

        case NEKO_ANDROID_MSG_START: {
            neko_println("MSG_START");
            android->state.has_resumed = true;
            if (neko_instance()) {
                gsevt.type = NEKO_PLATFORM_EVENT_APP;
                gsevt.app.action = NEKO_PLATFORM_APP_START;
                neko_platform_add_event(&gsevt);
            }
        } break;

        case NEKO_ANDROID_MSG_STOP: {
            neko_println("MSG_STOP");
            android->state.has_resumed = false;
            if (neko_instance()) {
                gsevt.type = NEKO_PLATFORM_EVENT_APP;
                gsevt.app.action = NEKO_PLATFORM_APP_STOP;
                neko_platform_add_event(&gsevt);
            }
        } break;

        case NEKO_ANDROID_MSG_RESUME: {
            neko_println("MSG_RESUME");
            android->state.has_resumed = true;
            if (neko_instance()) {
                gsevt.type = NEKO_PLATFORM_EVENT_APP;
                gsevt.app.action = NEKO_PLATFORM_APP_RESUME;
                neko_platform_add_event(&gsevt);
            }
        } break;

        case NEKO_ANDROID_MSG_PAUSE: {
            neko_println("MSG_PAUSE");
            android->state.has_resumed = false;
            if (neko_instance()) {
                gsevt.type = NEKO_PLATFORM_EVENT_APP;
                gsevt.app.action = NEKO_PLATFORM_APP_PAUSE;
                neko_platform_add_event(&gsevt);
            }
        } break;

        case NEKO_ANDROID_MSG_GAIN_FOCUS: {
            neko_println("MSG_GAIN_FOCUS");
            android->state.has_focus = true;
            if (neko_instance()) {
                gsevt.type = NEKO_PLATFORM_EVENT_WINDOW;
                gsevt.app.action = NEKO_PLATFORM_WINDOW_GAIN_FOCUS;
                neko_platform_add_event(&gsevt);
            }
        } break;

        case NEKO_ANDROID_MSG_LOSE_FOCUS: {
            neko_println("MSG_NO_FOCUS");
            android->state.has_focus = false;
            if (neko_instance()) {
                gsevt.type = NEKO_PLATFORM_EVENT_WINDOW;
                gsevt.app.action = NEKO_PLATFORM_WINDOW_LOSE_FOCUS;
                neko_platform_add_event(&gsevt);
            }
        } break;

        case NEKO_ANDROID_MSG_SET_NATIVE_WINDOW: {
            if (!neko_instance()) {
                neko_println("Creating engine...");
                neko_app_desc_t app = neko_main(0, NULL);
                ...
                neko_println("MSG_SET_NATIVE_WINDOW");
                if (android->current.window != android->pending.window) {
                    if (android->current.window != NULL) {
                        neko_android_destroy_egl_surface(android);
                    }
                    if (android->pending.window != NULL) {
                        neko_println("Creating egl surface ...");
                        if (neko_android_create_egl_surface(android, android->pending.window)) {
                            neko_println("... ok!");
                            neko_android_update_dimensions(android, android->pending.window, true);
                        } else {
                            neko_println("... failed!");
                            neko_quit();
                        }
        ...
        case NEKO_ANDROID_MSG_SET_INPUT_QUEUE: {
            neko_println("MSG_SET_INPUT_QUEUE");
            if (android->current.input != android->pending.input) {
                ...
                if (android->pending.input != NULL) {
                    AInputQueue_attachLooper(android->pending.input, android->looper, ALOOPER_POLL_CALLBACK, neko_android_input_cb, android); /* data */
                }
            }
            android->current.input = android->pending.input;
        } break;

        case NEKO_ANDROID_MSG_DESTROY: {
            neko_println("MSG_DESTROY");
            if (neko_instance()) {
                gsevt.type = NEKO_PLATFORM_EVENT_WINDOW;
                gsevt.app.action = NEKO_PLATFORM_WINDOW_DESTROY;
                neko_platform_add_event(&gsevt);
            }
            neko_quit();
        } break;
        ...
        default: {
            neko_println("Unknown msg type received");
        } break;
    }
...
void neko_android_msg(neko_android_t* android, neko_android_message_type msg) {
    neko_assert(android);
    if (write(android->gt.write_from_main_fd, &msg, sizeof(msg)) != sizeof(msg)) {
        neko_println("Could not write to write_from_main_fd");
    }
    neko_println("Write message sucessfully");
}

void neko_android_msg_set_native_window(ANativeActivity* activity, ANativeWindow* window) {
    neko_android_t* android = (neko_android_t*)activity->instance;
    neko_assert(android);
    pthread_mutex_lock(&android->gt.mutex);
    android->pending.window = window;
    neko_android_msg(android, NEKO_ANDROID_MSG_SET_NATIVE_WINDOW);
...
void neko_android_msg_set_input_queue(ANativeActivity* activity, AInputQueue* input) {
    neko_android_t* android = (neko_android_t*)activity->instance;
    neko_assert(android);
    pthread_mutex_lock(&android->gt.mutex);
    android->pending.input = input;
    neko_android_msg(android, NEKO_ANDROID_MSG_SET_INPUT_QUEUE);
...
void neko_android_native_window_created_cb(ANativeActivity* activity, ANativeWindow* window) {
    neko_println("NativeActivity:onNativeWindowCreated()");
    neko_android_msg_set_native_window(activity, window);
}

void neko_android_window_focus_change_cb(ANativeActivity* activity, s32 has_focus) {
    neko_android_t* android = (neko_android_t*)activity->instance;
    neko_assert(android);
    neko_println("NativeActivity:onNativeWindowFocusChange()");
    switch (has_focus) {
        case false:
            neko_android_msg(android, NEKO_ANDROID_MSG_LOSE_FOCUS);
            break;
        case true:
            neko_android_msg(android, NEKO_ANDROID_MSG_GAIN_FOCUS);
            break;
    }
}

void neko_android_resume_cb(ANativeActivity* activity) {
    neko_android_t* android = (neko_android_t*)activity->instance;
    neko_assert(android);
    neko_println("NativeActivity:onNativeResume()");
    neko_android_msg(android, NEKO_ANDROID_MSG_RESUME);
}

void neko_android_start_cb(ANativeActivity* activity) {
    neko_android_t* android = (neko_android_t*)activity->instance;
    neko_assert(android);
    neko_println("NativeActivity:onNativeStart()");
    neko_android_msg(android, NEKO_ANDROID_MSG_START);
}

void neko_android_pause_cb(ANativeActivity* activity) {
    neko_android_t* android = (neko_android_t*)activity->instance;
    neko_assert(android);
    neko_println("NativeActivity:onNativePause()");
    neko_android_msg(android, NEKO_ANDROID_MSG_PAUSE);
}

void neko_android_stop_cb(ANativeActivity* activity) {
    neko_android_t* android = (neko_android_t*)activity->instance;
    neko_assert(android);
    neko_println("NativeActivity:onNativeStop()");
    neko_android_msg(android, NEKO_ANDROID_MSG_STOP);
}

void neko_android_input_queue_created_cb(ANativeActivity* activity, AInputQueue* queue) { neko_android_msg_set_input_queue(activity, queue); }

void neko_android_destroy_cb(ANativeActivity* activity) {
    neko_android_t* android = (neko_android_t*)activity->instance;
    neko_assert(android);
    neko_println("NativeActivity:onNativeDestroy()");
    neko_android_msg(android, NEKO_ANDROID_MSG_DESTROY);
}

void neko_android_low_memory_cb(ANativeActivity* activity) { neko_println("NativeActivity:onLowMemory()"); }

void neko_android_config_changed_cb(ANativeActivity* activity) { neko_println("NativeActivity:onConfigurationChanged()"); }

NEKO_API_DECL void* neko_android_loop(void* arg) {
    neko_println("main loop");

    neko_android_t* android = (neko_android_t*)arg;
    neko_assert(android);

    // Set up callback for polling events
    android->looper = ALooper_prepare(0);
    ALooper_addFd(android->looper, android->gt.read_from_main_fd, ALOOPER_POLL_CALLBACK, ALOOPER_EVENT_INPUT, neko_android_main_cb, android);
    ...
    while (!android->gt.is_thread_stopping) {
        if (neko_android_should_update(android)) {
            neko_frame();
        }
        ...
        while (process && !android->gt.is_thread_stopping) {
            bool block_until_evt = !android->gt.is_thread_stopping && !neko_android_should_update(android);
    ...
    pthread_cond_broadcast(&android->gt.cond);
    pthread_mutex_unlock(&android->gt.mutex);
    neko_println("Android:Thread cleanup done.");
...
void ANativeActivity_onCreate(ANativeActivity* activity, void* saved_state, size_t saved_state_size) {
    ...
    activity->instance = neko_malloc_init(neko_android_t);
    neko_android_t* android = (neko_android_t*)activity->instance;
    neko_assert(android);

    // Store internal data path
    neko_println("internal data path: %s", activity->internalDataPath);
    android->internal_data_path = activity->internalDataPath;

    s32 pfd[2];
    if (pipe(pfd) != 0) {
        neko_println("Error:ANativeActivity_onCreate:Could not create thread pipe.");
        return;
    }
    ...
    neko_println("detach state");
    pthread_create(&android->gt.thread, &attr, neko_android_loop, android);
    neko_println("create pthread");
    pthread_attr_destroy(&attr);
    neko_println("destroy attribute");
    ...
    // Send create message
    pthread_mutex_lock(&android->gt.mutex);
    neko_android_msg(android, NEKO_ANDROID_MSG_CREATE);
    ...
    activity->callbacks->onStart = neko_android_stop_cb;
    activity->callbacks->onResume = neko_android_resume_cb;
    activity->callbacks->onPause = neko_android_pause_cb;
    activity->callbacks->onStop = neko_android_stop_cb;
    activity->callbacks->onDestroy = neko_android_destroy_cb;
    //    activity->callbacks->onSaveInstanceState = _sapp_android_on_save_instance_state;
    activity->callbacks->onNativeWindowCreated = neko_android_native_window_created_cb;
    activity->callbacks->onWindowFocusChanged = neko_android_window_focus_change_cb;
    ...
    activity->callbacks->onInputQueueCreated = neko_android_input_queue_created_cb;
    ...
    activity->callbacks->onConfigurationChanged = neko_android_config_changed_cb;
    activity->callbacks->onLowMemory = neko_android_low_memory_cb;

    neko_println("NativeActivity successfully created");
}

NEKO_API_DECL void neko_platform_init(neko_platform_t* platform) {
    // Get from application platform data
    neko_android_t* android = (neko_android_t*)neko_app()->android.activity;
    neko_assert(android);
    neko_app_desc_t* app = neko_app();
    platform->user_data = android;
    neko_println("Android: Init platform");
    neko_android_init_egl(platform->user_data);
    neko_println("Creating egl surface ...");
    if (neko_android_create_egl_surface(android, android->pending.window)) {
        neko_println("... ok!");
        neko_android_update_dimensions(android, android->pending.window, true);
...
NEKO_API_DECL void neko_platform_shutdown(neko_platform_t* platform) { neko_android_shutdown(platform->user_data); }
...
NEKO_API_DECL void neko_platform_sleep(float ms) { usleep(ms * 1000.f); }
...
NEKO_API_DECL u32 neko_platform_key_to_codepoint(neko_platform_keycode code) { return 0; }

NEKO_API_DECL neko_platform_keycode neko_platform_codepoint_to_key(u32 code) { return NEKO_KEYCODE_INVALID; }
...
NEKO_API_DECL neko_vec2 neko_platform_window_sizev(u32 handle) {
    neko_android_t* android = NEKO_PLATFORM_ANDROID_DATA();
    return neko_v2(android->egl.width, android->egl.height);
}
...
NEKO_API_DECL neko_vec2 neko_platform_framebuffer_sizev(u32 handle) {
    neko_android_t* android = NEKO_PLATFORM_ANDROID_DATA();
    return neko_v2(android->egl.fbwidth, android->egl.fbheight);
}
...
#undef NEKO_PLATFORM_IMPL_ANDROID
#endif  // NEKO_PLATFORM_IMPL_ANDROID

#endif  // __NEKO_PLATFORM_IMPL_H__
